...
class TestCase(auto_stub.TestCase):
  ...
  def mock_now_incrementing(self, now, increment):
    incrementer = Incrementer(now, increment)
    self.mock(utils, 'utcnow', incrementer)
    self.mock(ndb.DateTimeProperty, '_now', incrementer)
    self.mock(ndb.DateProperty, '_now', lambda _: incrementer().date())
    return incrementer
