...
def expand_luci_gae_py_vars(body, app_id):
  """Returns a copy of `body` with expanded luci_gae_py_vars."""
  varz = body.get('luci_gae_py_vars', None)
  if not varz:
    return body

  # Grab the mapping "variable name => its value" for the given app_id.
  if not isinstance(varz, dict):
    raise ValueError('luci_gae_py_vars section must be a dict')
  if app_id not in varz:
    raise ValueError(
        'no configuration for %s, it is not not in luci_gae_py_vars' % app_id)
  mapping = varz[app_id]
  if not isinstance(mapping, dict):
    raise ValueError(
        'bad value for %s in luci_gae_py_vars, not a dict' % app_id)
  ...
  def pick_value(match):
    ...
    raise ValueError(
        '%s is not defined in luci_gae_py_vars for %s' % (var, app_id))
...
class Application(object):
  ...
  def update(self, version, services=None):
    ...
    try:
      ...
      # 'application' or 'version'. Additionally, handle hacky luci_gae_py_vars
      # sections by reading vars from them and substituting their values into
      # the rest of the YAML, see expand_luci_gae_py_vars.
      for m in mods:
        modified = m.data.copy()
        modified.pop('application', None)
        modified.pop('version', None)
        try:
          modified = expand_luci_gae_py_vars(modified, self.app_id)
        except ValueError as exc:
          raise ValueError('Bad %s: %s' % (m.path, exc))
        if modified == m.data:
