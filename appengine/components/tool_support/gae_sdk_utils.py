...
class RollOption(collections.namedtuple('RollOption', 'duration')):
  @staticmethod
  def add_option(parser):
    if not USE_GCLOUD:
      return
    parser.add_option(
        '--roll-update', metavar='duration', type='str', nargs=1,
        dest='roll_update', action='callback', callback=RollOption._opt_callback,
        help=(
          'Do a rolling update over over a period of `duration`. The roll '
          'follows a simple quadratic curve and use ip traffic distribution. '
          'Duration may be specified as "[NNh][NNm]" where N are '
          'numbers. Canceling gae with ctrl-C will immediately switch back to '
          '0% traffic for the new version.'))

  _TIME_RE = re.compile(r'(?:(?P<hour>\d+)h)?(?:(?P<min>\d+)m)?')

  @classmethod
  def _parse_string(cls, duration):
    """Parses a RollOption duration."""
    match = cls._TIME_RE.match(duration or '2h')
    if not match:
      raise ValueError(
          "RollOption: cannot parse duration as NNhNNm: %r" % (duration,))
  ...
  @classmethod
  def _opt_callback(cls, option, _opt, value, parser):
    setattr(parser.values, option.dest, cls._parse_string(value))

  def yield_splits(self, starting_split=None):
    ...
    interval = max(total_seconds/100, 5*60)
    start = time.time()
    ...
    if starting_split:
      ...
      offset_seconds = math.sqrt(starting_split) * total_seconds
    ...
    while True:
      ...
      new_weight = min(1, max(
          ((now - start) / self.duration.total_seconds()) ** 2,
          .01
      ))
      if new_weight >= 1:
        break
...
class Application(object):
  ...
  def set_default_version(self, version, services=None, roll_options=None):
    """Switches default version of given |services| to |version|."""

    # pylint: disable=line-too-long
    print "Monitor logs for new version here:", (
      self.app_id.join([
        'https://pantheon.corp.google.com/logs/viewer?project=',
        '&minLogLevel=0&expandAll=false&customFacets=&limitCustomFacetWidth=true&interval=NO_LIMIT&resource=gae_app%2Fmodule_id%2Fbackend%2Fversion_id%2F26051-0d84d1c&logName=projects%2F',
        '%2Flogs%2Fappengine.googleapis.com%252Frequest_log&advancedFilter=resource.type%3D%22gae_app%22%0Aresource.labels.version_id%3D%2226051-0d84d1c%22%0AlogName%3D%22projects%2F',
        '%2Flogs%2Fappengine.googleapis.com%252Frequest_log%22%0Aseverity%3E%3DERROR%20%0ANOT%20%22Request%20was%20aborted%20after%20waiting%20too%20long%20to%20attempt%20to%20service%20your%20request.%22%0ANOT%20%22DeadlineExceededError%22',
      ]))
    ...
    base_cmd = (
      ...
      + services + ['--quiet', '--splits']
    )

    from_version = self.oldest_active_version(services)
    if roll_options and from_version != version:
      ...
      print (
        'Beginning migration, press ctrl-C to cancel migration and reset to %r'
        % (from_version,))

      try:
        for old_weight, new_weight in roll_options.yield_splits(previous_split):
          self.run_gcloud(base_cmd + [
            '%s=%s,%s=%s' % (from_version, old_weight, version, new_weight),
          ])
  ...
  def get_actives(self, services=None):
    ...
    Sorted by (service, ALNUM(id)), where `ALNUM` splits the id by '-', and
    turns any numeral sections to int.
    """
    def _sort_key(info):
      def _maybe_int(val):
        try:
          ...
          return val
      return (info['service'], map(_maybe_int, info['id'].split('-')))

    if self._cached_get_actives is None:
      data = self.run_gcloud(['app', 'versions', 'list', '--hide-no-traffic'])
      # TODO(maruel): There's a lot more data, decide what is generally useful
      # in there.
      self._cached_get_actives = sorted([
        {
    ...
    if services:
      return [
        service
        for service in self._cached_get_actives
