...
resource.labels.version_id="26067-93d16ba"
logName="projects/cr-buildbucket/logs/appengine.googleapis.com%2Frequest_log"
severity>=ERROR
NOT "Request was aborted after waiting too long"
NOT "DeadlineExceededError"
'''
...
class RollOption(collections.namedtuple('RollOption', 'duration')):
  ...
  def add_option(parser):
    ...
    parser.add_option(
        ...
        dest='roll_update',
        action='callback',
        callback=RollOption._opt_callback,
        help=(
  ...
  def yield_splits(self, starting_split=None):
    ...
    while True:
      ...
      new_weight = min(
          1, max(((now - start) / self.duration.total_seconds())**2, .01))
      if new_weight >= 1:
        break
...
class Application(object):
  ...
  def set_default_version(self, version, services=None, roll_options=None):
    ...
    print "Monitor logs for new version here:", (
        'https://pantheon.corp.google.com/logs/viewer?' + urllib.urlencode({
            'project': self.app_id,
            'minLogLevel': 0,
            'customFacets': '',
            ...
            'advancedFilter': _SWITCH_ADVANCED_FILTER.strip(),
        }))
    ...
    base_cmd = (
        ['app', 'services', 'set-traffic'] + services + ['--quiet', '--splits'])
