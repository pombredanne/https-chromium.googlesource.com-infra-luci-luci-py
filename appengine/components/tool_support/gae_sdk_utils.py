...
def expand_files_with_luci_gae_vars(mods, app_id):
  ...
  for m in mods:
    ...
    if modified_body == m.data:
      ...
      new_body = json.dumps(modified_body,
                            sort_keys=True,
                            indent=2,
                            separators=(',', ': '))
      ...
      expanded_mods.append(ModuleFile(path=new_path, data=new_body))
...
class Application(object):
  ...
  def spawn_dev_appserver(self, args, open_ports=False, **kwargs):
    ...
    """
    mods = self._services.values()
    sbp = None
    cleanup = None

    expanded_mods, cleanup = expand_files_with_luci_gae_vars(mods, self.app_id)

    # default should go first always.
    expanded_mods.sort(key=lambda x: '' if x == 'default' else x)
