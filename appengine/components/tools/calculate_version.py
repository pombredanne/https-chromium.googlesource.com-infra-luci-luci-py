...
def get_limited_version(pseudo_revision, mergebase, pristine, user, tag, limit):
  ...
  # Start off assuming we include the full username (just removing '-tainted').
  new_version_len = orig_version_len - 8
  chars_for_user = len(user)

  # If that's still not enough, truncate username, bailing if we try to go
  # go lower than min_user_chars.
  if new_version_len > limit:
    current_chars_without_user = new_version_len - len(user)
