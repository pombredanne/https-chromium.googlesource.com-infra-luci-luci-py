...
class Kubectl(object):
  ...
  COMMIT:
    - Copies the "latest-image" tag to "stable-image".
  """
...
def subcommand_mark_stable(_args, kctl):
  latest = kctl.get_latest_image()
  stable = kctl.get_stable_image()
  logging.info('Updating stable image: %r => %r', stable, latest)
...
def subcommand_status(_args, kctl):
  ...
  if stable != latest:
    print 'NOTE: Stable and latest do not match. Run "mark-stable" to commit.'
  else:
...
def main(args):
  ...
  # Subcommand: mark-stable
  subparser = subparsers.add_parser('mark-stable',
      help='Commit the latest deployed build to "stable".')
  add_cluster_key(subparser)
  subparser.set_defaults(func=subcommand_mark_stable)
