  ...
  def _get_config_v2_async(self, req, allow_not_found=True):
    ...
    try:
      ...
      if rpce.status_code == codes.StatusCode.NOT_FOUND and allow_not_found:
        raise ndb.Return(None)
      logging.warning('RpcError for GetConfig(%s): %s\n' % (req, rpce))
      raise rpce
    ...
    if res.signed_url:
      try:
        ...
        if blob and headers.get('Content-Encoding') == 'gzip':
          res.raw_content = gzip.GzipFile(
              fileobj=cStringIO.StringIO(base64.b64decode(blob))).read()
      except net.Error as ex:
        logging.warning(
  ...
  def _get_configs_multi(self, url_path, cfg_path=None):
    ...
    if self._is_v1_host():
      ...
      try:
        ...
        logging.warning('RpcError for GetProjectConfigs(%s): %s\n' %
                        (cfg_path, rpce))
        raise rpce
    ...
    if self._is_v1_host():
      ...
      try:
        ...
        if rpce.status_code == codes.StatusCode.NOT_FOUND:
          ...
          raise ndb.Return(None)
        logging.warning('RpcError in getting config_set(%s) location: %s\n' %
                        (config_set, rpce))
        raise rpce
