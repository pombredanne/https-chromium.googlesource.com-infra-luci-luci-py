...
class RelevantSubgraphTest(test_case.TestCase):
  ...
  def test_ownership_relations(self):
    b = AuthDBBuilder()
    b.group('a-root', [], [], ['b-inner'])
    b.group('b-inner')
    b.group('c-owned-by-root', [], [], [], owners='a-root')
    b.group('d-includes-owned-by-root', [], [], ['c-owned-by-root'])
  ...
  def test_diamond(self):
    b = AuthDBBuilder()
    b.group('top', [], [], ['middle1', 'middle2'])
    b.group('middle1', [], [], ['bottom'])
    b.group('middle2', [], [], ['bottom'])
    b.group('bottom', [], [], [])
    self.assertEqual({
      0: ('bottom', {'IN': [1, 3]}),
      1: ('middle1', {'IN': [2]}),
