...
def new_auth_db(
  ...
  return api.AuthDB.from_entities(
      ...
      ip_allowlist_assignments=(
          ip_allowlist_assignments or model.AuthIPAllowlistAssignments()),
...
class AuthDBTest(test_case.TestCase):
  ...
  def run_auth_db_fetch_test(self, setup_cb):
    ...
    self.assertEqual(
        {model.Anonymous: 'some ip allowlist'},
        auth_db._ip_allowlist_assignments)
    self.assertEqual(
        {'bots': ['127.0.0.1/32'], 'some ip allowlist': ['127.0.0.1/32']},
