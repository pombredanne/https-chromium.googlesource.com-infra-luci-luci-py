...
def get_access_token_async(
  ...
  if min_lifetime_sec <= 0 or min_lifetime_sec > 30 * 60:
    raise ValueError('"min_lifetime_sec" should be in range (0; 1800], actual: %d'%min_lifetime_sec)
...
def _parse_rfc3339_utc_omit_nanos(s):
  ...
  s, timezone = s[:-1], s[-1]
  idx = s.rfind('.')
  s = s[:idx+6+1] + timezone # cut out everything more granular than microseconds.
  return calendar.timegm(time.strptime(s, '%Y-%m-%dT%H:%M:%S.%fZ'))


@ndb.tasklet
def _generate_oauth_token_async(token_factory, cache_key, email, scopes, min_lifetime_secs=0, delegates=[]):
  ...
  # Cache miss, generate a new token
    ...
    response = yield _call_async(
        url='https://iamcredentials.googleapis.com/v1/projects/-/serviceAccounts/'
            '%s:generateAccessToken' % urllib.quote_plus(email),
        method='POST',
        headers={
          'Accept': 'application/json',
