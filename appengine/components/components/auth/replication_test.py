...
class ShardedAuthDBTest(test_case.TestCase):
  def test_works(self):
    ...
    shard_ids = replication.store_sharded_auth_db(
        auth_db, 'https://primary', 1234, 50)
    self.assertEqual(2, len(shard_ids))

    # Verify keys look OK and the shard size is respected.
    for shard_id in shard_ids:
      chunks = shard_id.split(',')
      self.assertEqual(chunks[0], 'https://primary')
      self.assertEqual(chunks[1], '1234')
      self.assertEqual(len(chunks[2]), 16)
      entity = ndb.Key(model.AuthDBSnapshotShard, shard_id).get()
      self.assertTrue(len(entity.blob) <= 50)

    # Verify it can be reassembled back.
    reassembled = replication.load_sharded_auth_db(shard_ids)
