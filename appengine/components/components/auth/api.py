...
def fetch_auth_db(known_auth_db=None):
  ...
  def fetch():
    ...
    # instance have to pay the cost of fetching AuthDB from Datastore via
    ...
    # since is does some heavy computations. Instead just return all kwargs for
...
def get_latest_auth_db():
  ...
  with _auth_db_fetch_lock:
    ...
    fetched = fetch_auth_db(known_auth_db=known)
...
def _roll_auth_db_cache(candidate):
  ...
  # This may happen when switch the primary server a replica is linked to.
