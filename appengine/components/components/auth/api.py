...
def get_process_auth_db():
  ...
  # Fetch has completed successfully. Update the process cache now, unless some
  # one else (perhaps via 'get_latest_auth_db' fetched a newer version already).
  with _auth_db_lock:
    assert _auth_db_fetching_thread == threading.current_thread()
    _auth_db_fetching_thread = None
...
def get_latest_auth_db():
  ...
  cache. As a downside, this cache  may lag behind the most recent state.
  """
  global _auth_db

  # We just "rush" the update of the internal cache. That way get_latest_auth_db
  # blocks for long only if something in AuthDB changes, when our cached copy
  # becomes stale. By reusing _auth_db, we keep memory footprint smaller.
...
def _roll_auth_db_cache(candidate):
  ...
  # Completely skip the update if fetched version is older than what we've got.
  if candidate.entity_group_version < _auth_db.entity_group_version:
    logging.info(
        'Someone else updated the cached AuthDB already (%d > %d)',
        _auth_db.auth_db_rev, candidate.auth_db_rev)
    return _auth_db

  # Prefer to reuse known copy if it matches the fetched one, it may have some
  # internal caches we want to keep.
  if candidate.entity_group_version > _auth_db.entity_group_version:
    _auth_db = candidate
    logging.info(
  ...
  # Bump the expiration time even if candidate's version same as the current
  # cached one. We've just got confirmation it is still fresh, so we can keep
