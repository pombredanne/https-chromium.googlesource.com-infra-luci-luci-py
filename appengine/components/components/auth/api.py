...
class AuthDB(object):
  ...
  def from_proto(replication_state, auth_db, additional_client_ids):
    ...
    Args:
      replication_state: instance of AuthReplicationState entity.
      auth_db: instance of replication_pb2.AuthDB message.
      additional_client_ids: an additional list of OAuth2 client IDs to trust.
    ...
    # more time and will convert them directly "AuthGroup proto => CachedGroup".
    snap = replication.proto_to_auth_db_snapshot(auth_db, skip_groups=True)

    # Now extract the groups directly into CachedGroup, bypassing entities.
    cached_groups = {}
  ...
  # Note: do not use __init__ directly, use some constructor static method
  # instead.
  def __init__(
  ...
  def _indexes(self):
    ...
    with self._lock:
      ...
      for name, group in sorted(self._groups.iteritems()):
...
def fetch_auth_db(known_auth_db=None):
  ...
  version in the datastore. It they match, the function returns known_auth_db
  unaltered (meaning that there's no need to refetch AuthDB), otherwise it
  fetches a fresh copy of AuthDB and returns it.

  Runs in a transaction to guarantee consistency of the fetched data.
  """
  ...
  if replication_state and replication_state.shard_ids:
    ...
    # corrupted.
    msg = replication.load_sharded_auth_db(replication_state.shard_ids)
