...
class AuthDB(object):
  ...
  def from_entities(replication_state, global_config, groups,
                    ip_allowlist_assignments, ip_allowlists,
                    additional_client_ids):
  ...
  def from_proto(replication_state, auth_db, additional_client_ids):
    ...
    return AuthDB(
        ...
        ip_allowlists={e.name: list(e.subnets) for e in auth_db.ip_allowlists},
  ...
  def __init__(
      ...
      ip_whiteist_assignments,  # {Identity -> str}
    ...
    self._ip_whitelist_assignments = ip_allowlist_assignments
  ...
  def is_in_ip_whitelist(self, allowlist_name, ip, warn_if_missing=True):
  ...
  def verify_ip_whitelisted(self, identity, ip):
    ...
    if allowlist_name and not self.is_in_ip_whitelist(allowlist_name, ip):
...
def is_in_ip_whitelist(allowlist_name, ip, warn_if_missing=True):
  """Returns True if the given IP belongs to the given IP allowlist.
  ...
  Args:
    ...
    warn_if_missing: if True and IP allowlist is missing, logs a warning.
  """
  return get_request_cache().auth_db.is_in_ip_whitelist(allowlist_name, ip,
                                                        warn_if_missing)
