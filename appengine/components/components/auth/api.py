...
class AuthDB(object):
  ...
  def _indexes(self):
    ...
    Returns:
      (
        ...
        Globs index as dict(IndentityGlob => [str with group name],
  ...
  def get_relevant_subgraph(self, principal):
    ...
    Returns it in a graph form where edges represent relations "subset of",
    "superset of" and "owned by". This is basically a union of two subgraphs
    (one is all parent groups of 'principal', another is groups owned by
    'principal'), joined by a bunch of "owned by" edges.
    ...
    # Note: when we say 'add_edge(A, B, IN)' we mean 'A' is a direct subset of
    ...
    def traverse(group):
      ...
      if added:
        for supergroup in nested_idx.get(group, ()):
          add_edge(group_id, traverse(supergroup), Graph.IN)
        for owned in owned_idx.get(group, ()):
          add_edge(group_id, traverse(owned), Graph.OWNS)
      return group_id
...
class Graph(object):
  ...
  Nodes must be comparable between each other and be hashable (since we use them
  ...
  def add_node(self, value):
    """Adds the given not (if not there).
  ...
  def add_edge(self, from_node_id, to_node_id, label):
    """Adds a labeled edge between two nodes given by their IDs."""
    assert from_node_id >= 0 and from_node_id < len(self._nodes)
    assert to_node_id >= 0 and to_node_id < len(self._nodes)
    self._edges[from_node_id][label].add(to_node_id)
  ...
  def describe(self):
    ...
    {label -> set([to_node_id])}, where not all labels may be present.
    """
