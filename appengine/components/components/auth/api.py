...
class AuthDB(object):
  ...
  def _check_realms_available(self):
    ...
    Standalone mode). Tests can populate realms programatically as part of the
  ...
  def _get_realm_or_its_root(self, name, perm):
    ...
    spl = name.split('/', 1)
    if len(spl) != 2 or not spl[0]:
      raise ValueError('Bad realm %r, want "<project>/<name>"' % (name,))
    if not _PROJECT_NAME_RE.match(spl[0]) or not _REALM_NAME_RE.match(spl[1]):
      raise ValueError(
          'Bad realm %r: should be "<project>/<name>" where '
          '<project> is %r and <name> is %r' %
    ...
    root_realm = self._realms.get(root_name)
    if root_realm:
      logging.warning(
...
def check_permission_dryrun(
  ...
  except (ValueError, RealmsError):
    logging.exception('%s: exception', log_pfx)
    return

  if result == bool(expected_result):
    logging.info('%s: match - %s', log_pfx, 'ACCEPT' if result else 'DENY')
  else:
    logging.warning(
        '%s: mismatch - got %s, want %s',
        log_pfx,
        'ACCEPT' if result else 'DENY',
        'ACCEPT' if expected_result else 'DENY')
