...
def root_realm(project):
  ...
  The root realm is used as a fallback when an existing resource points to
  a realm that doesn't exist. Without the root realm, such resources become
...
def check_permission(permission, realms, identity=None):
  ...
  will be used in its place). A non-existing root realm is considered empty.
