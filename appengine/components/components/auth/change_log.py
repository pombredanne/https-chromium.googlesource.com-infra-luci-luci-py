...
def generate_changes(auth_db_rev):
    ...
    def commit():
        if change_log_revision_key(auth_db_rev).get():
            logging.warning(
                'Rev %d was already processed concurrently', auth_db_rev)
            return
        rev = AuthDBLogRev(
...
def diff_global_config(target, old, new):
    ...
    added, removed = diff_lists(
        prev_client_ids, new.oauth_additional_client_ids)
    if added:
...
def diff_realms_globals(target, old, new):
    def to_map(ent): return {} if not ent else {
        p.name: p for p in ent.permissions}
