...
class AuthDBChange(polymodel.PolyModel):
  ...
  # AuthRealmsConfigChange change types.
  CHANGE_REALMS_PERMISSIONS_CHANGED = 9000
...
class AuthRealmsConfigChange(AuthDBChange):
  # Valid for CHANGE_REALMS_PERMISSIONS_CHANGED.
...
def diff_realms_globals(target, old, new):
  ...
  if added or change or removed:
    yield AuthRealmsConfigChange(
        change_type=AuthDBChange.CHANGE_REALMS_PERMISSIONS_CHANGED,
        target=target,
