...
def diff_ip_allowlist_assignments(target, old, new):
  # Helper to reduce amount of typing.
  def change(tp, identity, ip_allowlist, **kwargs):
...
KNOWN_HISTORICAL_ENTITIES = {
  ...
  'AuthIPAllowlistHistory': (model.AuthIPAllowlist, diff_ip_allowlists),
  'AuthIPAllowlistAssignmentsHistory': (
      model.AuthIPAllowlistAssignments, diff_ip_allowlist_assignments),
...
def ensure_initial_snapshot(auth_db_rev):
  ...
  def touch_auth_db():
    ...
    allowlists_future = model.AuthIPAllowlist.query(
        ancestor=model.root_key()).fetch_async()

    # Singleton entities.
    to_process.append(model.root_key().get())
    to_process.append(model.ip_allowlist_assignments_key().get())

    # Finish queries.
    to_process.extend(groups_future.get_result())
    to_process.extend(allowlists_future.get_result())
