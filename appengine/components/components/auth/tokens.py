...
def verify_jwt(jwt, bundle):
  ...
  Checks its header (must be signed using RS256 algo using one of the keys
  from the bundle), its RSA signature, and its issued and expiration times.
  Does not check the audience.
  ...
  # Here token's signature is valid, but it may be expired already.
  try:
  ...
  # wiggle room to account  for possible clock skew between the machine that
