...
def proto_to_auth_db_snapshot(auth_db_proto, skip_groups=False):
...
def store_sharded_auth_db(auth_db, primary_url, auth_db_rev, shard_size):
  ...
  parallel in already memory-constraint (but not performance critical) code
  path.
  ...
  while blob:
    ...
    ids.append('%s,%d,%s' % (
        primary_url,
        auth_db_rev,
        hashlib.sha256(shard).hexdigest()[:16],
    ))
...
def load_sharded_auth_db(shard_ids):
  ...
  shards = ndb.get_multi(
      ndb.Key(model.AuthDBSnapshotShard, shard_id)
      for shard_id in shard_ids)

  missing = [sid for sid, shard in zip(shard_ids, shards) if not shard]
  if missing:
    logging.error(
        'Cannot reconstruct AuthDB due to missing AuthDBSnapshotShard: %r',
        missing)
    return None
