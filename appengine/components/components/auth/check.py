...
# A header to grab to get a value of 'project_header' for check_request.
...
def check_request(
  ...
  if delegation_token and project_header:
    raise api.AuthenticationError(
        'Delegation tokens and X-Luci-Project headers cannot be used together')
  ...
  if delegation_token:
    ...
    if not auth_db.is_group_member(LUCI_SERVICES_GROUP, peer_identity):
      raise api.AuthenticationError(
          'Usage of X-Luci-Project is not allowed for %s,  not a member of %s' %
          (peer_identity, LUCI_SERVICES_GROUP))
...
def _project_identity(proj):
  ...
  try:
    ...
    raise api.AuthenticationError('Bad X-Luci-Project: %s' % exc)
