...
class SubgraphHandler(handler.ApiHandler):
  """Returns groups that include this principal and owned by it."""

  # This is visible in the UI.
  api_doc = [
    {
      'verb': 'GET',
      'doc': 'Returns groups that include this principal and owned by it.',
  ...
  def get(self, principal):
    ...
    self.send_response({
      'subgraph': {
        'nodes': [as_json(node, edges) for node, edges in subgraph.describe()],
        'root': subgraph.root_id,  # note: currently always 0
      },
    })
