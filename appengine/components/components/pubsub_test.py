...
class PubSubTest(test_case.TestCase):
  ...
  def test_full_topic_name(self):
    self.assertEqual('projects/abc/topics/def',
                     pubsub.full_topic_name('abc', 'def'))

  def test_full_subscription_name(self):
    self.assertEqual('projects/abc/subscriptions/def',
  ...
  def test_publish_ok(self):
    self.mock_requests([
        # First attempt. Encounters 404 due to non-existing topic.
        {
            ...
            'response':
                net.NotFoundError('topic not found', 404, ''),
        },
        # Creates the topic.
        {
