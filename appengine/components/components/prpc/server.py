...
class Server(object):
  ...
  def add_service(self, servicer):
    ...
    file_desc = _find_by_name(
        servicer.DESCRIPTION['file_descriptor_set'].file,
        servicer.DESCRIPTION['file_name'])
    desc = _find_by_name(
        file_desc.service, servicer.DESCRIPTION['service_name'])
    ...
    if file_desc.package:
      full_name = '%s.%s' % (file_desc.package, desc.name)
