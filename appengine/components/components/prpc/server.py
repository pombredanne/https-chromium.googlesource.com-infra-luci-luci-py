  ...
  def _handler(self):
    ...
    class Handler(webapp2.RequestHandler):
      ...
      def finalize(self, context, content=''):
        ...
        Args:
          context: context.ServicerContext used to set response headers
        ...
        # TODO(nodir,mknyszek): Set HTTP response code from gRPC response
        # code as well.
      ...
      def post(self, service, method):
        ...
        rpc_handler = server._generic_handlers[service].get(method)
        if rpc_handler is None:
          context.set_code(StatusCode.UNIMPLEMENTED)
          context.set_details('Method %s does not exist' % method)
          return self.finalize(context)

        request_message = rpc_handler[0]
        ...
        try:
          ...
          # canceled.
          handler = rpc_handler[2]
          response = handler(request, context)
        except Exception as e:
          # TODO(nodir,mknyszek): Add really good logging for this. Because
          # we're still returning a valid response to the client, this won't
          # result in stacktraces in logs, so we need to log them ourselves.
        ...
        response_message = rpc_handler[1]
