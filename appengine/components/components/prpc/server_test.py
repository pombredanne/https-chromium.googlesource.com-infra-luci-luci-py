...
class PRPCServerTestCase(test_case.TestCase):
  def setUp(self):
    ...
    self.explicit_origins_app = webttest.TestApp(
  ...
  def test_options_allowed_origins(self):
    ...
    self.assertEqual(allowed_headers['Access-Control-Allow-Origin'],
                     allowed_origin)
    self.assertEqual(allowed__headers['Vary'], 'Origin')
    self.assertEqual(allowed__headers['Access-Control-Allow-Credentials'],
                     'true')
