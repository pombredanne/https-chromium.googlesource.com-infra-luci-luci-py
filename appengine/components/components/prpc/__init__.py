...
class Server(object):
  ...
  def __init__(self, service_modules=None):
    ...
    if service_modules is not None:
      from components.prpc.discovery import discovery_service, service_pb2
      self._discovery_servicer = discovery_service.DiscoveryServicer()
      self.add_service(self._discovery_servicer)
  ...
  def add_service(self, servicer):
    ...
    methods = {
      method.name: types.RpcMethodHandler(
        sym_db.GetSymbol(method.input_type).FromString,
        sym_db.GetSymbol(method.output_type).SerializeToString,
        getattr(servicer, method.name)
          if hasattr(servicer, method.name)
          else self._unimplemented_handler,
      )
    ...
    if self._discovery_servicer:
      self._discovery_servicer.add_service(handler)

  def _unimplemented_handler(self, _request, context):
