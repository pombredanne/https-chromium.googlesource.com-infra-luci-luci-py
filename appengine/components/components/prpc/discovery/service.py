...
def _response_for(services):
  ...
  for description in descriptions:
    full_name = description['descriptor'].name
    if description['package']:
      full_name = '%s.%s' % (description['package'], full_name)
    service_names.append(full_name)

    ensure_file(description['file'])
...
def discovery_service(services):
  """Returns a Discovery service.

  Returns a Discovery service that describes the services and itself.
  This enables RPC Explorer and prpc command line tool.

  Assumes proto filename uniqueness for all files that the services are defined
  in or depend on, directly or indirectly. Protobuf reflection assumes that too.
  """
