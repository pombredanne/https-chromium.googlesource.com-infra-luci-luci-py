...
The *_pb2_grpc.py files generated by the Python protoc compiler contain calls to
three identifiers within the grpc package:
* grpc.unary_unary_rpc_method_handler
* grpc.method_handlers_generic_handler
* grpc.Server.add_generic_rpc_handlers

Unfortunately, these generated files contain `import grpc`, which is unusable
without explicitly adding this implementation of grpc to our path. As a result,
we choose to ignore these generated files altogether, but we still expose this
same functionality to maintain parity.
...
from components.prpc.grpc import helpers
from components.prpc.grpc import types
...
class Server(object):
  ...
  def __init__(self, service_modules=None):
    ...
    if service_modules is not None:
      ...
      # Normally this code is generated by protoc, but since we can't
      # depend on *_pb2_grpc.py files, we re-implement this code here
      # instead.
      discovery_handlers = {
          'Describe': unary_unary_rpc_method_handler(
              self._discovery_servicer.Describe,
              request_deserializer=service_pb2.Void.FromString,
              response_serializer=
                service_pb2.DescribeResponse.SerializeToString,
          ),
      }
      generic_handler = method_handlers_generic_handler(
          'discovery.Discovery', discovery_handlers)
