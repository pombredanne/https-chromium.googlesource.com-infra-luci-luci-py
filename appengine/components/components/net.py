...
TS_MON_CLIENT = "lucipy_components_net"
...
def request_async(
  ...
  Args:
    ...
    metrics_name: the name of the resource for the purpose of metrics collection
      ...
      recorded against the request. If None, then metrics_name will be derived
      from the <method> and the scheme and domain name in <url>
      e.g. if <method> is "GET" and <url> is "https://www.google.com/search?..."
      then metrics_name will be "GET_https://www.google.com"
  ...
  if metrics_name is None:
      ...
      metrics_name = "{method}_{url.scheme}://{url.netloc}".format(
          method=method, url=parsed_url)
  while attempt < max_attempts:
    ...
    try:
      ...
      # TODO: add request size and response size
      if http_metrics is not None:
        elapsed_ms = time.time() - start_timestamp
