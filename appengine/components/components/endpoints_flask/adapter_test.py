...
class EndpointsFlaskTestCase(test_case.TestCase):
  ...
  def test_decode_message_get(self):
    ...
    request.args = MultiDict([("s", "a")])
    request.form = MultiDict([("s2", "b"), ("r", "x"), ("r", "y")])
  ...
  def test_decode_message_get_resource_container(self):
    ...
    request.data = json.dumps({})
    request.args = MultiDict([("s", "a")])
    request.form = MultiDict([("s2", "b"), ("x", "c")])
    request.values = CombinedMultiDict([request.args, request.form])
    request.method = 'GET'
  ...
  def test_handle_403(self):
    app = flask.Flask(adapter.api_routes([EndpointsService], '/_ah/api'))
    request = mock.MagicMock()
