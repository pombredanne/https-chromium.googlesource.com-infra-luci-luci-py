...
class EndpointsFlaskTestCase(test_case.TestCase):
  def test_decode_message_post(self):
    ...
    logging.critical("asdfasdf %r", msg)
  ...
  def test_decode_message_get(self):
    request = mock.MagicMock()
    request.data = json.dumps({"s2": "b"})
    request.args = {"s": "a"}
    request.values = {"s": "a"}
    request.method = 'GET'
    rc = adapter.decode_message(EndpointsService.get.remote, request)
    self.assertEqual(rc.s, 'a')
    self.assertEqual(rc.s2, 'b')

  def test_decode_message_get_resource_container(self):
    request = mock.MagicMock()
    request.data = json.dumps({"s2": "b", "x": "c"})
    request.args = {"s": "a"}
    request.values = {"s": "a"}
    request.method = 'GET'
...
#         debug=True) (use flask.Flask instead)
