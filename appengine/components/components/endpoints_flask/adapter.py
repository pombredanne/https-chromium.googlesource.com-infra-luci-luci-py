...
def decode_message(remote_method_info, request):
  ...
  data = PROTOCOL.decode_message(body_type, request.data)
  if res_container:
  ...
  if res_container or request.method == 'GET':
    ...
    for f in param_fields:
      if f.name in request.route_kwargs:  #TODO(msriniv): rename this
        values = [request.route_kwargs[f.name]]
      else:
        values = request.get_all(f.name)  #TODO(msriniv): rename this
      if values:
