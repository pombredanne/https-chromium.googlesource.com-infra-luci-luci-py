...
def decode_message(remote_method_info, request):
  ...
  body = PROTOCOL.decode_message(body_type, request.data)
  if res_container:
  ...
  if res_container or request.method == 'GET':
    ...
    for f in param_fields:
      if f.name in request.route_kwargs:  #TODO: figure out how to rename this
        values = [request.route_kwargs[f.name]]
      else:
        values = request.get_all(f.name)  #TODO: figure out how to rename this
      if values:
...
def directory_handler_factory(api_classes, base_path):
  ...
  Returns:
    ...
    #TODO: make this return a Flask handler function
  """
