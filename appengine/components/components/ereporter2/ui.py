...
def _records_to_params(categories, ignored_count, request_id_url, report_url):
  ...
  return {
      ...
      'ignored_count':
          ignored_count,
      'occurrence_count':
          sum(e.events.total_count for e in categories),
      'report_url':
          report_url,
      'request_id_url':
          request_id_url,
          ...
          len(
              set(
                  itertools.chain.from_iterable(
                      e.versions for e in categories))),
  }
...
def _email_html(to, subject, body):
  ...
  mail_args = {
      'body':
          saxutils.unescape(re.sub(r'<[^>]+>', r'', body)),
      'html':
          '<html><body>%s</body></html>' % body,
      'sender':
          'no_reply@%s.appspotmail.com' % app_identity.get_application_id(),
      'subject':
          subject,
  }
  try:
...
def _generate_and_email_report(module_versions, recipients, request_id_url,
  ...
  if categories:
    ...
    params.update(
        _records_to_params(categories, sum(
            c.events.total_count for c in ignored), request_id_url, report_url))
    body = template.render('ereporter2/email_report_content.html', params)
    subject_line = template.render('ereporter2/email_report_title.html', params)
  ...
  models.ErrorReportingInfo(
      key=models.ErrorReportingInfo.primary_key(), timestamp=end_time).put()
  logging.info(
      'Processed %d items, ignored %d, reduced to %d categories, sent to %s.',
      sum(c.events.total_count for c in categories),
      sum(c.events.total_count for c in ignored), len(categories), recipients)
  return True
