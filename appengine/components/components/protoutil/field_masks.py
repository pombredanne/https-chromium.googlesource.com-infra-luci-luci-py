...
class Mask(object):
  ...
  @classmethod
  def from_field_mask(cls, field_mask, desc, from_json_names=False):
    ...
    Args:
      ...
      from_json_names: True if field_mask uses json field names for field names,
         e.g. "fooBar" instead of "foo_bar". Field names will be parsed in the
        canonical form.
    ...
    for p in field_mask.paths:
      try:
        parsed_paths.append(
            _parse_path(p, desc, from_json_names=from_json_names))
...
def _parse_path(path, desc, repeated=False, from_json_names=False):
  ...
  Args:
    ...
    repeated: True means that desc is a repeated field. For example,
      ...
      index.
    from_json_names: True if path uses json field names for field names, e.g.
      "fooBar" instead of "foo_bar". Field names will be parsed in the canonical
      form.
  ...
  def read_segment():
    ...
    field = _find_field(ctx.desc, tok, from_json_names)
    if field is None:
      raise ValueError(
