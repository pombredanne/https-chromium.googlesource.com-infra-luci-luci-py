...
def trim_message(message, field_tree):
  ...
  def with_seg(seg):
    """Returns a context manager that adds/removes a seg.
    ...
    included. See include() docstring for possible values.
    """
    seg_stack.append(seg)
    try:
      yield include(field_tree, tuple(seg_stack))
    finally:
      seg_stack.pop()
