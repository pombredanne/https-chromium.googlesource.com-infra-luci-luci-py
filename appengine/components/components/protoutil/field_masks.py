...
def parse_field_tree(field_mask, descriptor):
  ...
  Args:
    ...
    descriptor: a google.protobuf.descriptor.Descriptor for the target message.
  ...
  for p in field_mask.paths:
    try:
      parsed_paths.append(_parse_path(p, descriptor))
...
def trim_message(message, field_tree):
  ...
  Args:
    ...
    field_tree: a field tree parsed using parse_field_tree.
      The desciptor used in parse_field_tree must match message.DESCRIPTOR.
  ...
  def with_seg(seg):
    path.append(seg)
    try:
      yield _must_include(field_tree, tuple(path))
    finally:
      path.pop()
  ...
  def trim_msg(msg):
    for f, v in msg.ListFields():
      with with_seg(f.name) as incl:
        ...
        assert incl == _INCLUDE_PARTIALLY
        ...
        if not f.message_type:
          ...
          # Note that parse_field_tree would fail on such a mask because
...
def _tokenize(path):
  ...
  while i < len(path):
    if path[i] == '`':
      ...
      start = i
      # Scan the string until a delimiter or EOF.
...
_EXCLUDE = 0
_INCLUDE_PARTIALLY = 1
_INCLUDE_ENTIRELY = 2


def _must_include(field_tree, path):
  ...
  Returns:
    _EXCLUDE if the field value must be excluded.
    _INCLUDE_PARTIALLY if some subfields of the field value must be included.
    _INCLUDE_ENTIRELY if the field value must be included entirely.
  """
