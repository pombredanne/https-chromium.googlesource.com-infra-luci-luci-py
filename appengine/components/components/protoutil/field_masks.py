...
class Mask(object):
    ...
    for f, v in message.ListFields():
      ...
      if f.message_type.GetOptions().map_entry:
        for mk, mv in v.items():
          incl = self._includes((f.name, mk), 0)
          if incl == INCLUDE_ENTIRELY:
            pass
          elif incl == EXCLUDE:
            v.pop(mk)
  ...
  def _includes(self, path, start_at):
    """Implements includes()."""
  ...
  def get(self, path):
    ...
    """
    assert path
    return self._get(_parse_path(path, self.desc, repeated=self.repeated), 0)

  def _get(self, path, start_at):
    """Implements get()."""
    if start_at == len(path):
      return self

    child = self.children.get(path[start_at])
    return child and child._get(path, start_at + 1)
