...
class MultilineProtoTest(unittest.TestCase):
  ...
  def test_go_compatibility(self):
    ...
    tests = [
        ('basic', r'something: "this\000 \t is\n\na \"basic\" test\\example"',
         '\n\t\t  '.join([
             'something: << EOF', 'this\x00 \t is', '',
             r'a "basic" test\example', 'EOF'
         ])),
        ('contained', r'something: "A << B"', r'something: "A << B"'),
        ('indent', r'something: "this\n  is indented\n\nwith empty line"',
         '\n'.join([
             'something: << EOF', '\t\t\tthis', '\t\t\t  is indented', '   ',
             '\t\t\twith empty line', '\t\tEOF'
         ])),
        ('col 0 align', r'something: "this\n  is indented\n\nwith empty line"',
         '\n'.join([
             'something: << EOF', 'this', '  is indented', '   ',
             'with empty line', 'EOF'
         ])),
        ('nested', r'something: "<< nerp\nOther\nnerp\nfoo"', '\n\t\t'.join(
            ['something: << EOF', '<< nerp', 'Other', 'nerp', 'foo', 'EOF'])),
        ('multi',
         '\n'.join(['something: "this is something"',
                    'else: "this is else"']), '\n'.join([
                        'something: << EOF', 'this is something', 'EOF',
                        'else: << ELSE', 'this is else', 'ELSE'
                    ])),
        ('indented first line',
         r'something: "  this line\nis indented\n  this too"', '\n\t '.join([
             'something: <<DERP', '  this line', 'is indented', '  this too',
             'DERP'
         ])),
        ('mixed indents are not indents', r'something: "\ttab\n  spaces"',
         '\n'.join(['something: <<DERP', '\ttab', '  spaces', 'DERP'])),
    ]
    for test in tests:
