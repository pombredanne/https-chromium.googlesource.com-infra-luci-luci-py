...
class FromFieldMaskTests(unittest.TestCase):
  def parse(self, paths):
    ...
    return Tree.from_field_mask(fm, TEST_DESC)

  def test_empty(self):
    actual = self.parse([])
    expected = Tree(TEST_DESC)
    self.assertEqual(actual, expected)

  def test_str(self):
    actual = self.parse(['str'])
    expected = Tree(TEST_DESC, children={
        'str': Tree(),
    })
    self.assertEqual(actual, expected)
