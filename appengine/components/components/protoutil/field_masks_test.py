...
class MustIncludeTests(unittest.TestCase):
  def test_all(self):
    ...
    path = ('a', )
    self.assertEqual(
        field_masks._must_include(tree, path),
        field_masks._INCLUDE_ENTIRELY)
  ...
  def test_include_recursively_star(self):
    tree = {'a': {field_masks._STAR_SEG: {'b': {}}}}
    path = ('a', 'x', 'b')
    self.assertEqual(
        field_masks._must_include(tree, path),
        field_masks._INCLUDE_ENTIRELY)
  ...
  def test_include_partially(self):
    ...
    self.assertEqual(
        field_masks._must_include(tree, path),
        field_masks._INCLUDE_PARTIALLY)
...
class TrimTests(unittest.TestCase):
  ...
  def trim(self, msg, *mask_paths):
    field_tree = field_masks.parse_field_tree(
  ...
  def test_map_str_num_leave_key_with_int_key_invalid(self):
    msg = test_proto_pb2.Msg(map_str_num={'1': 1, '2': 2})
    with self.assertRaisesRegexp(ValueError, 'invalid unquoted string "1"'):
      self.trim(msg, 'map_str_num.1')
