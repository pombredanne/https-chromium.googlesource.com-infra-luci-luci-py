...
def has_safe_repr(value):
  ...
  if value is None or value is NotImplemented or value is Ellipsis:
    return True
  if isinstance(value,
                (bool, int, float, complex, range_type, Markup) + string_types):
    return True
...
class Frame(object):
  ...
  def __init__(self, eval_ctx, parent=None):
    ...
    if parent is not None:
      self.identifiers.declared.update(parent.identifiers.declared
                                       | parent.identifiers.declared_parameter
                                       | parent.assigned_names)
      self.identifiers.outer_undeclared.update(parent.identifiers.undeclared -
...
class CodeGenerator(NodeVisitor):
  ...
  def unoptimize_scope(self, frame):
    ...
    if frame.identifiers.declared:
      self.writeline('%sdummy(%s)' %
                     (unoptimize_before_dead_code and 'if 0: ' or '', ', '.join(
  ...
  def function_scoping(self, node, frame, children=None, find_special=True):
    ...
    if overridden_closure_vars:
      self.fail(
          ...
          'derived from an outer scope! (affects: %s)' % ', '.join(
  ...
  def macro_def(self, node, frame):
    ...
    for arg in node.defaults:
      self.visit(arg, frame)
      self.write(', ')
    self.write('), %r, %r, %r)' % (bool(frame.accesses_kwargs),
  ...
  def visit_FromImport(self, node, frame):
    ...
    for name in node.names:
      ...
      self.writeline(
          ...
          'name=%r)' % (alias, 'the template %%r (imported on %s) does '
                        'not export the requested name %s' %
                        (self.position(node), repr(name)), name))
      self.outdent()
      if frame.toplevel:
  ...
  def export_assigned_vars(self, frame, assignment_frame):
    if not frame.toplevel:
      return
    public_names = [
        x for x in assignment_frame.toplevel_assignments
    ...
    if public_names:
      if len(public_names) == 1:
        ...
        self.writeline('context.exported_vars.update((%s))' % ', '.join(
  ...
  def visit_CondExpr(self, node, frame):
    ...
    def write_expr2():
      ...
      self.write('environment.undefined(%r)' %
                 ('the inline if-'
