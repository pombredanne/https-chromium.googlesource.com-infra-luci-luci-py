...
def _load_on_host_maintenance(on_host_maintenance):
  """Loads from the given on_host_maintenance value.

  Args:
    on_host_maintenance:
      proto.config_pb2.InstanceTemplateConfig.InstanceTemplate.OnHostMaintenance

  Returns:
    String indicating host maintenance behavior.
  """
  if on_host_maintenance is None:
    return 'migrate'
  ...
  return host_maintenance_handlers[on_host_maintenance]
...
def compute_template_checksum(template_cfg):
  ...
  identifying_properties = {
      ...
      'on-host-maintenance':
          _load_on_host_maintenance(template_cfg.on_host_maintenance),
...
def ensure_entities_exist(template_cfg, manager_cfgs, max_concurrent=50):
  ...
  if not instance_template_revision:
    ...
    instance_template_revision = models.InstanceTemplateRevision(
        ...
        on_host_maintenance=_load_on_host_maintenance(
