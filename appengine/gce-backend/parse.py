...
def _load_guest_accelerators(guest_accelerators):
  ...
  Args:
    guest_accelerators: A list of "<type>:<count>" strings.

  Returns:
    models.GuestAccelerator instance.
  """
  return [models.GuestAccelerator(accelerator_type=k, accelerator_count=int(v))
          for k, v in _load_dict(guest_accelerators).iteritems()]
