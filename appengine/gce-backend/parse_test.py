...
class ComputeTemplateChecksumTest(test_case.TestCase):
  ...
  def test_checksum_not_dependent_on_presence_of_default_count(self):
    """Ensures checksum not dependent on whether count=1 specified explicitly.
    """
    template1 = config_pb2.InstanceTemplateConfig.InstanceTemplate(
        guest_accelerators=[
            'accelerator-type-1',
...
class EnsureEntityExists(test_case.TestCase):
  ...
  def test_creates_new_entity(self):
    ...
    expected_on_host_maintenance = 'terminate'
