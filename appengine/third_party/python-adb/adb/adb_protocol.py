...
_ord = lambda c: ord(c) if isinstance(c, str) else c

def ID2Wire(name):
  assert len(name) == 4 and isinstance(name, six.binary_type), name
  assert all(ord('A') <= _ord(c) <= ord('Z') for c in name), name
  return sum(_ord(c) << (i * 8) for i, c in enumerate(name))
...
def _CalculateChecksum(data):
  ...
  return sum(_ord(d) for d in data) & 0xFFFFFFFF
