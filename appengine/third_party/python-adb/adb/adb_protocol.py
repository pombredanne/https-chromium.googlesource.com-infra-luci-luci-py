...
_toint = lambda c: c if isinstance(c, int) else ord(c)


def ID2Wire(name):
  assert len(name) == 4 and isinstance(name, six.binary_type), name
  assert all(ord('A') <= _toint(c) <= ord('Z') for c in name), name
  return sum(_toint(c) << (i * 8) for i, c in enumerate(name))
...
def Wire2ID(encoded):
  ...
  if not all(ord('A') <= _toint(c) <= ord('Z') for c in name):
    return b'XXXX'
  return name
...
def _CalculateChecksum(data):
  ...
  return sum(_toint(d) for d in data) & 0xFFFFFFFF
...
class AdbConnectionManager(object):
  ...
  def Open(self, destination, timeout_ms=None):
    ...
    conn._Write(b'OPEN', destination + '\0')
    with self._lock:
