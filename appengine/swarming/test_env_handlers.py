...
# import components/auth modules directly to mock AuthDB.
...
class AppTestBase(test_case.TestCase):
  ...
  def mock_auth_db(self):
    self.mock(auth_api, 'get_request_cache',
              lambda: mock.Mock(auth_db=self.auth_db()))
  ...
  def auth_db():
    return auth_api.AuthDB.from_proto(
        ...
        auth_db=replication_pb2.AuthDB(
            ...
            realms={
                'api_version': auth_realms.API_VERSION,
                'permissions': [
                    {'name': p.name} for p in _ALL_PERMS
                ],
                'realms': [
                    {
                        'name': 'test:@root',
