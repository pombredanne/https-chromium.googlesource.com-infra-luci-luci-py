...
class TaskBackendAPIServiceTest(test_env_handlers.AppTestBase):
  ...
  def test_run_task(self):
    ...
    expected_response = backend_pb2.RunTaskResponse(task=task_pb2.Task(
        ...
        link=
        'https://test-swarming.appspot.com/task?id=4225526b80008610&o=true&w=true',
    ...
    expected_response = backend_pb2.RunTaskResponse(task=task_pb2.Task(
        id=task_pb2.TaskID(id=new_expected_task_id,
                           target='swarming://testing'),
        link=
        'https://test-swarming.appspot.com/task?id=4225526b80008710&o=true&w=true',
        status=common_pb2.SCHEDULED))
    self.assertEqual(actual_resp, expected_response)
