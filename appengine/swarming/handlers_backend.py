...
class TaskExpireTasksHandler(webapp2.RequestHandler):
  ...
  @decorators.require_taskqueue('expire-tasks')
  def post(self):
    payload = json.loads(self.request.body)
    task_scheduler.task_expire_tasks(payload['task_ids'])
...
def get_routes():
  ...
  routes = [
    ...
    ('/internal/taskqueue/important/tasks/expire-tasks', TaskExpireTasksHandler),
