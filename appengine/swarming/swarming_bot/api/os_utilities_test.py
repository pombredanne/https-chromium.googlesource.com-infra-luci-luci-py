...
class TestOsUtilities(auto_stub.TestCase):
  ...
  def test_get_os_name_cases(self):
    ...
    for platform, result in cases:
      self.mock(sys, "platform", lambda: platform)
  ...
  def test_get_os_values(self):
    cases = [
        ('openbsd7', '7.2'['openbsd-7', 'openbsd-7.2']),
        ('netbsd9', '9.3_STABLE',
         ['netbsd-9', 'netbsd-9.3', 'netbsd-9.3_STABLE']),
    ]
    for platform, release, result in cases:
      self.mock(sys, "platform", lambda: platform)
      self.mock(platform, "release", lambda: release)
