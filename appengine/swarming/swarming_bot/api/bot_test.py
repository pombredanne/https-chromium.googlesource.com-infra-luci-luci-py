...
class TestBot(unittest.TestCase):
  def test_get_fudge(self):
    # This test assumes little endian.
    # The following confirms the equivalent code in Bot.get_fudge():
    self.assertEqual(-1., round(struct.unpack('h', '\x00\x80')[0] / 32768., 4))
    self.assertEqual(1., round(struct.unpack('h', '\xff\x7f')[0] / 32768., 4))
    b = make_bot()
    self.assertEqual(-0.7782, b.get_fudge(1.))
    self.assertEqual(-0.0778, b.get_fudge(.1))
