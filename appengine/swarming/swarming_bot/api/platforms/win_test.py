...
class TestWin(auto_stub.TestCase):
  ...
  def test_is_display_attached(self):
    # Whether a display is detected or not when not mocking is going to be
    # dependent on the hardware, but we can at least ensure that we don't hit
    # any errors before being able to determine one way or another.
    display_attached = win.is_display_attached()
