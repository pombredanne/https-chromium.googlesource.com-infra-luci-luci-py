...
class TestWin(auto_stub.TestCase):
  ...
  def test_get_gpu(self):
    ...
    with mock.patch('api.platforms.win._get_wmi_wbem', return_value=SWbemServices):
      actual = win.get_gpu()
      SWbemServices.ExecQuery.assert_called_once_with('SELECT * FROM Win32_VideoController')
      expected = (
        ['1ae0', '1ae0:a002', '1ae0:a002-1.1.1.18'], ['Unknown GGA 1.1.1.18'])
      self.assertEqual(expected, actual)
