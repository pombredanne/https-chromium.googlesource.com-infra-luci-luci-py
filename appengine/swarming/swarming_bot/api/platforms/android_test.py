...
class TestGetState(unittest.TestCase):
  ...
  def test_temperature_fallback_happy_path(self):
    ...
    device.GetTemperatures.return_value = {}
    with mock.patch.object(android,
                           '_parse_thermalservice_output',
                           return_value={'BAT': 20.0}):
      state = android.get_state([device])
    individual_state = state['devices']['serial']
    self.assertIn('temp', individual_state)
    self.assertEqual(individual_state['temp'], {'BAT': 20.0})
...
Temperature static thresholds from HAL:
    ...
    temperature_data = android._parse_thermalservice_output(dumpsys_output)
    self.assertEqual(
        temperature_data, {
            'AP': 19.1,
            'BAT': 18.6,
...
Current temperatures from HAL:
    ...
    with self.assertLogs(level='WARNING') as log_manager:
      temperature_data = android._parse_thermalservice_output(dumpsys_output)
      self.assertIn("WARNING:root:Unable to parse float from 'VeryHot'",
                    log_manager.output)
    self.assertEqual(temperature_data, {
        'AP': 19.1,
        'USB': 19.4,
...
Cached temperatures:
    ...
    with self.assertLogs(level='WARNING') as log_manager:
      temperature_data = android._parse_thermalservice_output(dumpsys_output)
      self.assertIn(
          'WARNING:root:Did not find any data using fallback temperature path',
          log_manager.output)
    self.assertEqual(temperature_data, {})
