...
# at https://docs.microsoft.com/en-us/windows/release-information/ for clients
# and at
...
def get_os_version_names():
  ...
  if version_number == u'10.0':
    ...
    yymm = _WIN10_BUILD_TO_VERSION_MAPPING.get(major_version)
    if yymm:
  ...
  return [marketing_name, raw_version,
          u'%s-%s' % (marketing_name, service_pack)]


@tools.cached
def get_os_product_type():
  """Returns the product_type of the OS.

  The product_type is determined by Python. The return value will be one of
  "workstation", "domain_controller", or "server".
  """
  winversion = sys.getwindowsversion() # pylint: disable=no-member
  return {
      1: u'workstation',
      2: u'domain_controller',
      3: u'server'
  }.get(winversion.product_type, u'unknown')
