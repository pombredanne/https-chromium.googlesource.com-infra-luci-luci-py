...
def get_gpu():
  ...
  try:
    logging.error('start try')
    if not wbem.ExecQuery('SELECT * FROM Win32_VideoController'):
      logging.error('wbem.ExecQuery is empty')
  ...
  logging.error('dimention %s, state %s',sorted(dimensions), sorted(state))
