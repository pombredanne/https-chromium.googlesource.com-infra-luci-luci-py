...
def _get_window_class(hwnd):
  ...
  name_len = ctypes.windll.user32.GetClassNameW(hwnd, buffer, len(buffer))
  if name_len <= 0 or name_len >= len(buffer):
...
def list_top_windows():
  ...
  ctypes.windll.user32.EnumWindows.restype = ctypes.wintypes.BOOL
  ...
  ctypes.windll.user32.IsIconic.restype = ctypes.wintypes.BOOL
  ctypes.windll.user32.IsIconic.argtypes = [ctypes.wintypes.HWND]

  # A collection to hold the discovered class names.
  top_level_window_class_names = []
  ...
  def on_window(hwnd, lparam):
    ...
    if (ctypes.windll.user32.IsWindowVisible(hwnd) and
      ...
      if (class_name and
        ...
        top_level_window_class_names.append(class_name)
    return 1

  ctypes.windll.user32.EnumWindows(window_enum_proc_prototype(on_window), None)
  return top_level_window_class_names
