...
def get_dimensions(devices):
  ...
  # If only one device_type is to be reported, product.device be read
  # before build.product because the latter is deprecated.
      ...
      for dim, props in dimension_properties.iteritems():
        for prop in props:
          ...
          if real_prop in properties:
            ...
            if p and p not in dimensions[dim] and p != "<missing>":
