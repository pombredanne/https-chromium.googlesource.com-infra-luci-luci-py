...
class TestTaskRunnerBase(auto_stub.TestCase):
  ...
  def expectTask(self, **kwargs):
    ...
    self.assertLessEqual(0., actual.pop('bot_overhead'))
    self.assertTrue(actual.pop('cost_usd'))
...
class TestTaskRunnerTimeMocked(TestTaskRunnerBase):
  ...
  def test_run_command_env_prefix_1(self):
    ...
    self.expectTask(
        output=u'L3Zhci9mb2xkZXJzL3FfL3lkeWpkbGMxNjkzYmtkcW16Y3k1cngzMDAwMW5qM'
               u'i9UL3Rhc2tfcnVubmVyQTR3elBNL3cvaXIvbG9jYWwvc211cmYK')
...
class TestTaskRunnerNoTimeMock(TestTaskRunnerBase):
  ...
  def test_hard_signal(self):
    ...
    self.expectTask(hard_timeout=True, output=u'aGkKZ290IHNpZ25hbCAxNQpieWUK')
  ...
  def test_io_signal(self):
    ...
    # Now look at the updates sent by the bot as seen by the server.
    #    output_re='^hi\ngot signal %d\nbye\n$' % task_runner.SIG_BREAK_OR_TERM)
    self.expectTask(io_timeout=True, output=u'aGkKZ290IHNpZ25hbCAxNQpieWUK')
  ...
  def test_hard_signal_no_grace(self):
    ...
    self.expectTask(
        hard_timeout=True,
        exit_code=exit_code,
        output=u'aGkKZ290IHNpZ25hbCAxNQpieWUK')
