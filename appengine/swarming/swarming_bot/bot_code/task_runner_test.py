...
class TestTaskRunnerKilled(TestTaskRunnerBase):
  ...
  def test_signal(self):
    ...
    # Windows reports a SIGBREAK whereas UNIX reports SIGTERM.
    if sys.platform == "win32":
      expected['internal_error'] = 'task_runner received signal 21'
    else:
      expected['internal_error'] = 'task_runner received signal 15'
...
class TaskRunnerNoServer(auto_stub.TestCase):
  ...
  def test_load_and_run_raw_with_bb_agent_context(self):
    local_auth_ctx = {
        'accounts': [{
            'id': 'a'
        }, {
    ...
    def _run_command(remote, rbe_session, task_details, work_dir, cost_usd_hour,
                     start, run_isolated_flags, bot_file, ctx_file):
      self.assertNotEqual(os.environ['BUILDBUCKET_AGENT_CONTEXT_FILE'], '')
      self.assertTrue(
          os.environ['BUILDBUCKET_AGENT_CONTEXT_FILE'].endswith(".json"))
      self.assertTrue(
          'bb_agent_ctx.' in os.environ['BUILDBUCKET_AGENT_CONTEXT_FILE'])
      self.assertTrue(remote.uses_auth)  # mainly to avoid "unused arg" warning
      self.assertIsNone(rbe_session)
      self.assertTrue(isinstance(task_details, task_runner.TaskDetails))
      # Necessary for OSX.
    ...
    self.assertEqual(expected, actual)
    self.assertEqual(os.environ.get('BUILDBUCKET_AGENT_CONTEXT_FILE'), None)

  def test_load_and_run_raw_with_bb_agent_context_failure(self):
    local_auth_ctx = {
        'accounts': [{
            'id': 'a'
        }, {
