...
EXIT_CODE_SIGTERM = -signal.SIGTERM
# Actually 0xc000013a, unsigned in python3 on windows
if sys.platform == 'win32':
  if six.PY3:
    EXIT_CODE_SIGTERM = 3221225786
  else:
    EXIT_CODE_SIGTERM = -1073741510
...
class TestTaskRunner(TestTaskRunnerBase):
  ...
  def test_run_command_large(self):
    ...
    self.assertEqual(
      base64.b64encode(expect_out * 100002), updates[1][u'output'].encode())
    self.assertEqual(
      base64.b64encode(expect_out), updates[2][u'output'].encode())
...
class TestTaskRunnerKilled(TestTaskRunnerBase):
  ...
  def test_killed_later(self):
    ...
    # Now look at the updates sent by the bot as seen by the server.
    self.expectTask(task_details.task_id, exit_code=EXIT_CODE_SIGTERM)
    t.join()
  ...
  def test_hard(self):
    ...
    # Now look at the updates sent by the bot as seen by the server.
    self.expectTask(
        task_details.task_id, hard_timeout=True, exit_code=EXIT_CODE_SIGTERM)
  ...
  def test_io(self):
    ...
    # Now look at the updates sent by the bot as seen by the server.
    self.expectTask(
      task_details.task_id, io_timeout=True, exit_code=EXIT_CODE_SIGTERM)
