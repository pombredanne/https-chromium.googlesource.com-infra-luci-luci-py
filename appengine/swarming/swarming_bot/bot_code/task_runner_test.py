...
class TestTaskRunner(TestTaskRunnerBase):
  ...
  def test_run_command_large(self):
    ...
    # - buffer filled with the 3 first yeild
    # - last yield
...
class TestTaskRunnerKilled(TestTaskRunnerBase):
  ...
  def test_killed_later(self):
    # Have a task killed by the server.

    # Make the task update a busy loop to reduce the duration of this test case.
    self.mock(task_runner._OutputBuffer, '_MIN_PACKET_INTERVAL', 0.1)
    self.mock(task_runner._OutputBuffer, '_MAX_PACKET_INTERVAL', 0.1)

    # We need to 'prime' the server before starting the thread.
    self.assertTruel(self.server.url)
    ...
    self.expectTask(
        ...
        isolated_stats={
          u'download': {
            ...
            u'initial_number_items': 0,
            u'initial_size': 0,
            u'items_cold': u'eJybwMjWzigGAAUwATY=',
            u'items_hot': u'',
          },
          u'upload': {
            u'duration': 0.,
