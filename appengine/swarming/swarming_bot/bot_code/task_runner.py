...
def tmp_bb_agent_context_file(secret_bytes, task_id, workdir):
  ...
  try:
    ...
    os.environ['BUILDBUCKET_AGENT_CONTEXT_FILE'] = tf.name
    yield
    ...
    try:
      os.unlink(tf.name)
      os.environ.pop('BUILDBUCKET_AGENT_CONTEXT_FILE', None)
  ...
  try:
    with subprocess42.set_signal_handler([SIG_BREAK_OR_TERM], handler):
      ...
      with contextlib.ExitStack() as stack:
        ...
        ctx_file = stack.enter_context(
            luci_context.stage(_tmpdir=work_dir, **context_edits))
        ...
        if task_details.is_using_bb_agent_context:
          stack.enter_context(
              tmp_bb_agent_context_file(task_details.secret_bytes,
