...
class RemoteClientGrpc(object):
  ...
  def __init__(self, server, fake_proxy=None):
    ...
    if fake_proxy:
      ...
      self._proxy_bs = grpc_proxy.Proxy(server, bytestream_pb2_grpc.ByteStreamStub)
    self._server = server
  ...
  def do_handshake(self, attributes):
    ...
    resp = {
        ...
        'bot_group_cfg': {
            'dimensions': _worker_to_bot_group_cfg(attributes, self._session.worker),
        },
  ...
  def poll(self, attributes):
    ...
    if not self._session:
      raise ValueError("session is unset")
    ...
    if len(self._session.leases) == 1:
      ...
      assert len(self._session.leases) == 0
    ...
    for lease in self._session.leases:
      if lease.state is bots_pb2.PENDING:
        if new_lease is not None:
          raise ValueError("Multiple new leases are not supported")
        new_lease = lease
        # This will be picked up the next time we call UpdateBotSession (in
        # post_bot_event).
        lease.state = bots_pb2.ACTIVE
      elif lease.state is bots_pb2.ACTIVE:
        raise ValueError("Server thinks lease is already running")
      elif lease.state is bots_pb2.COMPLETED:
        pass
      elif lease.state is bots_pb2.CANCELLED:
        pass
    ...
    self._log_is_asleep = False
    if not lease.inline_assignment:
      raise ValueError('task must be included in lease')

    return ('run', self._process_lease(lease))
  ...
  def post_task_update(self, task_id, bot_id, params,
    ...
    # TODO: send normal BotSessionUpdate and verify that this lease is not
    # CANCELLED.
  ...
  def _send_stdout_chunk(self, bot_id, task_id, stdout_and_chunk, finished):
    ...
    if not stdout_and_chunk and not finished:
      raise ValueError("missing stdout, but finished is False")
    ...
    try:
      ...
      logging.error('error during stdout update: %s' % e)
      raise e

    if res is not None and res.committed_size != self._stdout_offset:
      raise ValueError('%s: incorrect size written (got %d, want %d)' % (
...
def _worker_to_bot_group_cfg(attributes, worker):
