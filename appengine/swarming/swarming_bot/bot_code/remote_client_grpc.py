...
class RemoteClientGrpc(object):
  ...
  def _process_lease(self, lease):
    ...
    if t == 'type.googleapis.com/google.devtools.remoteworkers.v1test2.AdminTemp':
      return self._process_admin_lease(lease)
    if t == 'type.googleapis.com/google.devtools.remoteworkers.v1test2.Task':
  ...
  def _stdout_resource_name_from_ids(self, bot_id, task_id):
    ...
    res = '%s/logs/%s/%s/stdout' % (project_id, bot_id, real_task_id)
    logging.info('Will send stdout to: %s', res)
    return res
