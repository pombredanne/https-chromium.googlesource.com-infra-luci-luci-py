...
class RemoteClientGrpc(object):
  ...
  def __init__(self, server, fake_proxy=None):
    ...
    if fake_proxy:
      ...
      self._proxy_wm = grpc_proxy.Proxy(server, worker_manager_pb2_grpc.WorkerManagerStub)
      self._proxy_rm = grpc_proxy.Proxy(server, reservation_manager_pb2_grpc.ReservationManagerStub)
      self._proxy_tm = grpc_proxy.Proxy(server, task_manager_pb2_grpc.TaskManagerStub)
    self._server = server
  ...
  def post_task_update(self, task_id, bot_id, params,
    ...
    logging.warning('Not yet implemented: posting task event: %s', message)

  def post_task_error(self, task_id, bot_id, message):
    logging.warning('Not yet implemented: posting task error: %s', message)

  def do_handshake(self, attributes):
    logging.info('Got attributes: %s', attributes)
    ...
    resp = {
        ...
        'bot_group_cfg': {
            'dimensions': self._device_to_new_dimensions(attributes, worker.device),
        },
    }
  ...
  def _attributes_to_worker(self, attributes, worker):
    ...
    self._dimensions_to_device(attributes['dimensions'], worker.device)
    attributes['state']['bot_version'] = attributes['version']
    json_format.ParseDict(attributes['state'], worker.properties)

  def _dimensions_to_device(self, dims, device):
  ...
  def _device_to_new_dimensions(self, attributes, device):
