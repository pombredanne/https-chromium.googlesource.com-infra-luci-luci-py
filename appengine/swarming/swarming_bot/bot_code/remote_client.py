...
class RBELease:
  ...
  def __init__(self, id, state, payload=None, result=None):
    """Constructs a lease given its details.

    Arguments:
      id: a string lease ID.
  ...
  def clone(self):
    ...
    return RBELease(self.id, self.state,
...
class RBESession:
  ...
  def update(self, status, dimensions, poll_token):
    """Updates the state of the session, pick up a new lease.

    Should be called in the outer bot loop, when the bot is waiting for new
    tasks. This method reports the results of the last finished lease to the
    server and picks up a new lease (if any). It also recognizes if the session
    is closed by the server and updates `healthy` property accordingly.
    ...
    """
    if not self.healthy:
      raise RBESessionException('The session is unhealthy')
    ...
    assert not self._finished_lease or self._finished_lease.state == RBELeaseState.COMPLETED
    ...
    # A new lease must be in PENDING state and have a payload.
    if lease:
  ...
  def ping_active_lease(self):
    ...
    Must be called only if `active_lease` is set. This method "pings" the
    lease (making the RBE server know the bot is not dead yet) and polls its
    cancellation status.

    Calling this methods may update `healthy` property as a side-effect.
    A session may become unhealthy if it is gone on the backend side. The
    active lease is considered canceled in that case. If the local session was
    already unhealthy when the method was called, the lease is considered
    canceled as well.
    ...
    # already. Report the latest snapshot of the dimensions though, since the
    # API wants dimensions.
  ...
  def finish_active_lease(self, result):
    ...
    signifying the session is ready to pick up a new lease in update(...).
    ...
    lease.result = result.copy() if result is not None else None
  ...
  def terminate(self):
    ...
    Does nothing if the session is unhealthy. Ignores `active_lease`.

    Retries the call a bunch of times on RPC errors to increase chances of
    successfully reporting results of the last finished lease. If errors are
    still happening, eventually just gives up. Termination usually happens
    when the process is exiting, there's no time time to retry forever.
    """
  ...
  def _update(self,
    ...
    Returns:
      RBELease returned by the backend.
