...
class TestBotBase(net_utils.TestCase):
  ...
  def make_bot(self, auth_headers_cb=None):
    ...
    self.loop = bot_main._BotLoop(self.bot, self.quit_bit)

  def poll_once(self):
    self.quit_bit.reset()
    self.mock(self.loop, 'handler_done', lambda *_args: self.quit_bit.set())
    bot_main._run_bot_loop(self.loop)
