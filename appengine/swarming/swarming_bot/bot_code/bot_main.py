...
def _run_isolated_flags(botobj, is_clean):
  ...
  if is_clean:
    ...
    min_free = min_free + 2 * 1024 * 1024 * 1024
...
def _clean_cache(botobj):
  ...
  cmd.extend(_run_isolated_flags(botobj, True))
...
def _run_manifest(botobj, manifest, start):
  ...
  try:
    ...
    command.append('--')
    command.extend(_run_isolated_flags(botobj, False))
