...
def _run_bot_inner(arg_error, quit_bit):
  ...
  state = _BotLoop(botobj, quit_bit)

  while not _sleep_until_signaled(quit_bit, state.sleep_duration):
    # Note: on_before_poll should always be called before get_state and
    # get_dimensions (i.e. before _update_bot_attributes). It's part of the bot
    # hooks API contract.
    _call_hook_safe(False, botobj, 'on_before_poll')
    _update_bot_attributes(botobj, sleeps)
    cmd, param, error = None, None, None
    try:
      cmd, param = botobj.remote.poll(botobj.attributes)
    ...
    try:
      if cmd == 'sleep':
        state.cmd_sleep(param['duration'], param['rbe'])
      elif cmd == 'terminate':
        state.cmd_terminate(param)
      elif cmd == 'run':
        state.cmd_run(param, last_poll_time)
      elif cmd == 'update':
        state.cmd_update(param)
      elif cmd in ('host_reboot', 'restart'):
        state.cmd_host_reboot(param)
      elif cmd == 'bot_restart':
        state.cmd_bot_restart(param)
      elif cmd:
    ...
    # executes tasks back to back with no idle time in-between, the hook should
    # not be called.
    if state.idle:
      _call_hook_safe(True, botobj, 'on_bot_idle', state.idle_duration)
...
def _sleep_until_signaled(quit_bit, timeout):
  ...
  """
  # TODO(vadimsh): On Python 3.5 and older wait(...) may raise an exception if
  # it is interrupted by a signal. When this happens we should resume sleeping,
  # but this requires a monotonic clock to be precise and not be dangerous.
  # Monotonic clocks also appeared only in Python 3.5. Instead use time.time()
  # and just make sure we never sleep longer than the originally requested
  # `timeout`. That way the function will at least terminate fast enough, even
  # if the clock jumps. It may be grossly inaccurate though, but we don't need
  # accuracy and clock jumps should be rare.
  while timeout > 0:
    start = time.time()
    try:
...
class _BotLoop:
  ...
  def idle(self):
    ...
    return self._consecutive_sleeps or self._consecutive_errors or self._bad_bot_state_errors

  @property
  def idle_duration(self):
    """Duration since the bot finished executing the last task or started."""
  ...
  def handle_error(self, message):
    """Called whenever there's an error polling or processing the response."""
    logging.error('Poll loop error: %s', message)
    self._consecutive_errors += 1

  def handle_success(self):
    """Called whenever a command is processed without internal errors."""
    self._consecutive_errors = 0
  ...
  def cmd_sleep(self, duration, rbe):
    ...
    _maybe_update_lkgbc(self._botobj)
  ...
  def cmd_terminate(self, task_id):
    ...
    try:
      ...
      botobj.remote.post_task_update(task_id, {'duration': 0}, None, 0)
  ...
  def cmd_run(self, manifest):
    ...
    success = _run_manifest(self._botobj, manifest)
    ...
    _clean_cache(self._botobj)
    if success:
      # Completed a task successfully so update swarming_bot.zip if necessary.
      _update_lkgbc(self._botobj)
    # This is used to track how long the bot is idle.
    self._consecutive_sleeps = 0
    self._requested_sleep_duration = 0.0
    self._last_task_time = time.time()
  ...
  def cmd_update(self, version):
    ...
    _update_bot(self._botobj, version)
    _set_quarantined('Failed to self-update the bot')
    self._bad_bot_state_errors += 1

  def cmd_host_reboot(self, message):
    """Called when Swarming asks the bot to reboot the host machine."""
    self._botobj.host_reboot(message)
    _set_quarantined('Failed to reboot the host')
    self._bad_bot_state_errors += 1

  def cmd_bot_restart(self, message):
    """Called when Swarming asks the bot to restart its own process."""
    _bot_restart(self._botobj, message)
    _set_quarantined('Failed to restart the bot process')
    self._bad_bot_state_errors += 1
