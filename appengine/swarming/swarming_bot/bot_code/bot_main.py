...
def _cleanup_purgeable_space(botobj):
  ...
  dim_os = dims.get('os')
  if not dim_os or dim_os[0] != 'Mac' or (len(dim_os) >= 2 and
                                          dim_os[1] == 'Mac-10'):
    return
  ...
  cmd = ['sudo', 'rm', '-f', cache_file]
  output, err = _Popen(botobj, cmd).communicate(None)

  logging.info(
      '_cleanup_purgeable_space: removed %s. %s sec elapsed. output=%s, '
      'error=%s', cache_file,
      time.time() - start, output, err)
