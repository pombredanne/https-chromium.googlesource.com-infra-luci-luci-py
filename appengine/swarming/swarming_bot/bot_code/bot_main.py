...
class _BotLoopState:
  ...
  def on_bot_exit(self):
    ...
    Bot.set_exit_hook, Bot.host_reboot (can be called from hooks, call the
  ...
  def rbe_poll(self, maintenance, blocking):
    ...
    # dead.
    self.rbe_maybe_abandon_session()
    ...
    try:
      ...
      # rbe_maybe_abandon_session).
      if self._rbe_session.terminating and not self._rbe_termination_pending:
  ...
  def rbe_maybe_abandon_session(self):
    """Abandons the RBE session if it looks dead and needs to be recreated."""
    if not self._rbe_session:
      return  # nothing to abandon
    if self._rbe_session.alive and not self._rbe_session.terminating:
    ...
    # if something goes wrong, treat session closure as a transient error to
    # slow down spinning.
    logging.warning('RBE: session %s is gone', self._rbe_session.session_id)
    self._rbe_consecutive_errors += 1
