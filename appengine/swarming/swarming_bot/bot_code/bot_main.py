...
def _update_bot(botobj, version):
  ...
  Returns only in case of failing to get the new bot code.
  """
...
def _maybe_update_lkgbc(botobj):
  """Updates the Last Known Good Bot Code (LKGBC) when LKGBC is older than 1 one
  week.

  This either means:
  - The bot code is particularly hosed, for an extended period of time.
  - All tasks are failing, which may legitimitely happen some times whenthe devs
    don't care about a particular configuration.
  - The bot is completely idle, so it never gets the chance to update LKGKC.

  We decide that the first situation is rare enough that it's preferable to
  explcitly handle the two later. We've seen bots being idle for extended
  ...
  try:
    if not os.path.isfile(THIS_FILE):
      # Don't error out, it's broken but bleh.
      return False
    golden = os.path.join(botobj.base_dir, 'swarming_bot.zip')
    if os.path.isfile(golden):
      org = os.stat(golden)
