...
def _get_disks_quarantine(botobj, disks):
  ...
  for mount, infos in sorted(disks.iteritems(), key=lambda x: -len(x[0])):
    if path.startswith(mount.lower() if case_insensitive else mount):
      if mount != root:
        # Found the bot's partition.
        _check_for_quarantine(mount, infos, 'bot')
      break
  else:
    # The bot's partition == root partition, so apply the bot's settings too,
    # but only if not already quarantined.
    if not errors and root in disks:
      _check_for_quarantine(root, disks[root], 'bot')
  if errors:
    return '\n'.join(errors)
