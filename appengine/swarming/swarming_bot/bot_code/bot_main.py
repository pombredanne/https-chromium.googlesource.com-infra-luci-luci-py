...
def _run_bot_inner(arg_error, quit_bit):
  ...
  # Spin until getting a termination signal. Extracted into a separate function
  # for easier testing.
  _run_bot_loop(_BotLoop(botobj, quit_bit))
...
def _run_bot_loop(state):
  """Spins executing Swarming commands until getting termination signal."""
  while not _sleep_until_signaled(state.quit_bit, state.sleep_duration):
    ...
    # hooks API contract.
    _call_hook_safe(False, state.bot, 'on_before_poll')
    _update_bot_attributes(state.bot, state.consecutive_sleeps)
    cmd, param, error = None, None, None
    try:
      cmd, param = state.bot.remote.poll(state.bot.attributes)
      logging.debug('Swarming server response:\n%s: %s', cmd, param)
    except remote_client_errors.PollError as e:
      error = 'poll RPC failed: %s' % e
    _call_hook_safe(False, state.bot, 'on_after_poll', cmd)
    ...
    # **not** be called.
    if state.idle:
      _call_hook_safe(True, state.bot, 'on_bot_idle', state.idle_duration)
...
class _BotLoop:
  ...
  @property
  def quit_bit(self):
    """Event signaled when the bot should gracefully exit."""
    return self._quit_bit
  ...
  @property
  def consecutive_sleeps(self):
    """Number of consecutive times the server asked the bot to sleep."""
    return self._consecutive_sleeps
