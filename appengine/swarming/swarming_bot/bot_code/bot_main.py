...
def _should_enable_ts_monitoring(config):
  """Check whether the config from the Swarming server instructs the bot to enable monitoring"""
...
def _add_ts_mon_arguments(parser):
...
class _BotLoopState:
  ...
  def run(self, test_hook=None):
    ...
    while not self._quit_bit.is_set():
      ...
      # Swarming ASAP. Bot idleness status surfaces in the UI.
