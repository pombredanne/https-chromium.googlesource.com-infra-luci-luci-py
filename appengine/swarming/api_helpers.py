...
def check_identical_request(namespace, request_uuid, ttl=60 * 60):
  ...
  The decorator passes the cache to the original function, so that the function
  can validate it.
    ...
    def wrapper():
      ...
      if request_idempotency_key:
        result_cache = memcache.get(
            request_idempotency_key, namespace=namespace)
      ...
      if request_idempotency_key:
        memcache.add(
          request_idempotency_key, result, namespace=namespace, time=ttl)
      return result
