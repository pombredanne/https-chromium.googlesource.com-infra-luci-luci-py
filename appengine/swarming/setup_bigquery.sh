...
QUERY="SELECT
  ...
  ROUND(TIMESTAMP_DIFF(event_time, CAST(LEAD(event_time) OVER(PARTITION BY bot.bot_id ORDER BY event_time DESC) AS TIMESTAMP), MICROSECOND)*0.000001, 3) AS since_last,
  ROUND(TIMESTAMP_DIFF(CAST(LAG(event_time) OVER(PARTITION BY bot.bot_id ORDER BY event_time DESC) AS TIMESTAMP), event_time, MICROSECOND)*0.000001, 3) AS until_next,
  *
...
if !(bq mk --use_legacy_sql=false --view "$QUERY" \
  ...
  echo ""
  echo "The view already exist. You can delete it with:"
  echo ""
