...
class TaskBackendAPIService(prpc_helpers.SwarmingPRPCService):
  ...
  def CancelTasks(self, request, _context):
    ...
    ).filter(task_result.TaskResultSummary.key.IN(result_keys))

    query = task_result._filter_query(task_result.TaskResultSummary, query, start=None, end=None, sort='created_ts', state='pending_running')
