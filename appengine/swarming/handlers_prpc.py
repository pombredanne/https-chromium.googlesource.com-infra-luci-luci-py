...
class BotService(prpc_helpers.SwarmingPRPCService):
  DESCRIPTION = swarming_api_prpc_pb2.BotServiceDescription
...
class TaskService(prpc_helpers.SwarmingPRPCService):
  DESCRIPTION = swarming_api_prpc_pb2.TaskServiceDescription
  ...
  def CancelTask(self, request, _context):
    ...
    return swarming_api_pb2.CancelResponse(ok=ok, was_running=was_running)
  ...
  def GetStdout(self, request, _context):
    output, state = api_common.get_output(request.task_id, request.offset,
                                          request.length)
    return swarming_api_pb2.TaskOutputResponse(output=output, state=state)


class SwarmingService:
  DESCRIPTION = swarming_api_prpc_pb2.SwarmingServiceDescription
