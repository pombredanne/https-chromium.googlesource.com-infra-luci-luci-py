...
@ndb.tasklet
def _do_counts(q):
  count = yield q.count_async()
  dead = yield bot_management.filter_availability(q, None, None, True,
                                                  None).count_async()
  quarantined = yield bot_management.filter_availability(
      q, True, None, None, None).count_async()
  maintenance = yield bot_management.filter_availability(
      q, None, True, None, None).count_async()
  busy = yield bot_management.filter_availability(q, None, None, None,
                                                  True).count_async()
  bc = BotsCount(count=count,
                 dead=dead,
...
def count_bots(dimensions):
  ...
  return _do_counts(q).get_result()
