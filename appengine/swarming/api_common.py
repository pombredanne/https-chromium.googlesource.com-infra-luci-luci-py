...
def to_keys(task_id):
  ...
  try:
    ...
    raise handlers_exceptions.BadRequestException('%s is an invalid key.' %
                                                  task_id)
...
def get_task_request_async(task_id, request_key, viewing):
  ...
  Arguments:
    ...
    viewing: Can be either CANCEL or VIEW: determines whether task_cancel_acl
  ...
  if viewing == VIEW:
    ...
    raise handlers_exceptions.InternalException('_get_task_request_async()')
...
def get_request_and_result(task_id, viewing, trust_memcache):
  ...
  Raises:
    handlers_exceptions.NotFoundException: if an invalid task_id is supplied.
  """
  try:
