...
def new_task(task_request, secret_bytes, template_apply, evaluate_only,
             request_uuid):
  """Schedules a new task for a bot with given dimensions.

  Arguments:
    task_request: ndb TaskRequest entity representing the new task.
    secret_bytes: the secret bytes. What are those?
  ...
  Returns:
    NewTaskResult(task_request, task_id, task_result) where
      task_request is the TaskRequest object which would result from this
        routine. Only stored in datastore if evaluate_only=True.
      task_id: task_id of task, will be None if evaluate_only=True.
      task_result initial TaskResultSummary entity in datastore.

  Raises:
    handlers_exceptions.BadRequestException if creating the task fails for an
      expected reason.
  """
  api_helpers.process_task_request(task_request, template_apply)

  # If the user only wanted to evaluate scheduling the task, but not actually
  # schedule it, return early without a task_id.
  if evaluate_only:
    task_request._pre_put_hook()
    return NewTaskResult(request=task_request, task_id=None, task_result=None)
  ...
  def _schedule_request():
    try:
      result_summary = task_scheduler.schedule_request(
          task_request,
          enable_resultdb=(task_request.resultdb
                           and task_request.resultdb.enable),
          secret_bytes=secret_bytes)
    except (datastore_errors.BadValueError, TypeError, ValueError) as e:
      logging.exception("got exception around task_scheduler.schedule_request")
      raise handlers_exceptions.BadRequestException(e.message)

    return NewTaskResult(request=task_request,
  ...
  if cache_hit:
    ...
    if new_task_result.request != task_request:
      logging.warning(
