...
def _convert_known_exception_to_endpoints(e):
  exe_type = type(e)
  if exe_type is handlers_exceptions.NotFoundException:
    return endpoints.NotFoundException(e.message)
  if exe_type is handlers_exceptions.BadRequestException:
    return endpoints.BadRequestException(e.message)
  return e


def convert_to_endpoints_exception(func):
  """Changes any handlers_exceptions functions raised by func to the equivalent
  endpoints.exception.

  Arguments:
    func is any callable.
  ...
  def wrapper(*args, **kwargs):
    try:
      ...
      e = _convert_known_exception_to_endpoints(e)
      raise e
