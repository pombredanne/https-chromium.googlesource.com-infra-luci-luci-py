...
class BackendTest(test_env_handlers.AppTestBase):
    ...
    task_queues = sorted(
      [
        ...
        ('monitoring-bq-tasks-requests',
          ...
          '2020-01-01T01:01'),
        ('monitoring-bq-tasks-results',
          '/internal/taskqueue/monitoring/bq/tasks/results/',
          '2020-01-01T01:01'),
      ],
      key=lambda x: x[1])
  ...
  def test_taskqueue_monitoring_bq_tasks_results(self):
    self.set_as_admin()
    now = datetime.datetime(2020, 1, 2, 3, 4, 0)
    def task_bq(start, end):
      self.assertEqual(start, now)
      self.assertEqual(end, now + datetime.timedelta(seconds=60))
      return 0, 0
    self.mock(task_result, 'task_bq', task_bq)
    self.app.post(
        '/internal/taskqueue/monitoring/bq/tasks/results/2020-01-02T03:04',
        headers={'X-AppEngine-QueueName': 'monitoring-bq-tasks-results'})
