...
class TasksApiTest(BaseTest):
  ...
  def test_new_ok_in_realms_mode(self):
    ...
    task_request = key.get()

    # Make sure associated the task with the correct realm.
    self.assertEqual('test:task_realm', task_request.realm)
    # Correctly initialized the service account state.
    self.assertEqual('service-account@example.com',
                     task_request.service_account)
    self.assertFalse(task_request.service_account_token)
  ...
  def test_new_ok_with_default_task_realm_not_enforced(self):
    ...
    self.assertEqual('service-account@example.com',
                     task_request.service_account)
    self.assertEqual('tok', task_request.service_account_token)
