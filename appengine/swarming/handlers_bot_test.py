...
class BotApiTest(test_env_handlers.AppTestBase):
  ...
  def setUp(self):
    ...
    s.add_service(handlers_task_backend.TaskBackendAPIService())

    # By default requests in tests are coming from bot with fake IP.
    routes = handlers_bot.get_routes() + s.get_routes()
  ...
  def test_push_pubsub_message_for_task_backend_on_task_update(self):
    def _decode(raw, dst):
    ...
    def create_new_run_task_request(**kwargs):
      """Returns an initialized buildbucket.v2.RunTaskRequest.
    ...
    task_id = actual_resp.task.id.id
    ...
    self.set_as_user()
    response = self.client_get_results(task_id)
    expected = self.gen_run_result(name=unicode('bb-42423'),
                                   bot_idle_since_ts=fmtdate(self.now),
                                   costs_usd=[0.1],
                                   created_ts=fmtdate(self.now),
                                   modified_ts=fmtdate(self.now),
