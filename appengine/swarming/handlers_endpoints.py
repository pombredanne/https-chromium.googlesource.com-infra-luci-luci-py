...
class SwarmingTasksService(remote.Service):
  ...
  @gae_ts_mon.instrument_endpoint()
  @auth.endpoints_method(
      TaskStatesRequest, swarming_rpcs.TaskStateList,
      http_method='GET')
  @auth.require(acl.can_view_all_tasks)
  def results(self, request):
    ...
    results = ndb.get_multi(result_keys)
    for task in results:
      task_states.append(task.state)

    return swarming_rpcs.TaskStateList(items=[
        swarming_rpcs.TaskState(task_id, state)
        for task_id, state in zip(request.task_id, task_states)])
