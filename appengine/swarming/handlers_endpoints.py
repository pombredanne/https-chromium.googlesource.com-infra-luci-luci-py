...
def apply_server_property_defaults(properties):
  ...
  settings, pool = (
    config.settings(),
    pools_config.get_pool_config(properties.pool))
  if not settings and not pool:
    return

  iso_server = iso_ns = None
  if pool and pool.default_isolate:
    iso_server = pool.default_isolate.server
    iso_ns = pool.default_isolate.namespace
  else:
  ...
  cipd_server = cipd_vers = None
  if pool and pool.default_cipd:
    cipd_server = pool.default_cipd.server
    cipd_vers = pool.default_cipd.client_version
  else:
