...
class SwarmingServerService(remote.Service):
  ...
  @gae_ts_mon.instrument_endpoint()
  @auth.endpoints_method(
      PermissionsRequest, swarming_rpcs.ClientPermissions,
      http_method='GET')
  @auth.public
  def permissions(self, request):
    ...
    return swarming_rpcs.ClientPermissions(
        ...
        cancel_task=self._can_terminate_bot(request),
        cancel_tasks=realms.can_cancel_tasks(request.tags),
        get_bootstrap_token=acl.can_create_bot())

  def _can_terminate_bot(self, request):
    if not request.task_id:
      return False
    task_key, _ = _to_keys(request.task_id)
    task = task_key.get()
    if not task:
      raise endpoints.NotFoundException('%s not found.' % request.task_id)
