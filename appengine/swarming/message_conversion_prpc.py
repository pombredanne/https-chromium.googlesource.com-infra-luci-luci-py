...
class ProtoFactory(object):
  ...
  def collection(self, proto_field, items, conversion_fn=None):
    field = getattr(self._proto, proto_field)
    if conversion_fn:
...
class ObjectDerivedProtoFactory(ProtoFactory):
  def __init__(self, ndb_entity, proto):
    super(ObjectDerivedProtoFactory, self).__init__(proto)
    self._ndb = ndb_entity

  def _get(self, field_name):
    if hasattr(self._ndb, field_name):
      return getattr(self._ndb, field_name)

  def field(self, field_name, proto_field=None, conversion_fn=None):
  ...
  def date(self, field_name, proto_field=None):
    proto_field = proto_field or field_name
    field = self._get_proto(proto_field)
    date = self._get(field_name)
    if date:
  ...
  def string_list_pairs(self, field_name, proto_field=None):
    proto_field = self._get_proto(proto_field or field_name)
    obj_field = self._get(field_name)
    _string_list_pairs_from_dict(obj_field or {}, proto_field)
    return self

  def message_field(self, field_name, factory_fn):
    obj_field = self._get(field_name)
  ...
  def collection_field(self, field_name, factory_fn=None):
    self.collection(field_name, self._get(field_name) or [], factory_fn)
    return self

  def repeated_message_field(self, field_name, factory_fn):
...
def bot_info_to_proto(bot_info, deleted=False):
  ...
  factory = ObjectDerivedProtoFactory(
      bot_info, swarming_api_pb2.BotInfoResponse()).fields(
          'bot_id', 'external_ip', 'authenticated_as', 'is_dead', 'quarantined',
