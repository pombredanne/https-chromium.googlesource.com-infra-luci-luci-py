...
# Custom bucketer with 2% resolution in the range of 100ms...1000s. Used for
# bot died detection latency.
_detection_bucketer = gae_ts_mon.GeometricBucketer(growth_factor=10**0.01,
                                                   num_finite_buckets=400,
                                                   scale=100)
...
# Instance metric. Metric fields:
# - pool: e.g. 'skia'.
_bot_died_detection_latencies = \
  gae_ts_mon.CumulativeDistributionMetric(
    'swarming/tasks/bot_died_detection_latencies',
...
def on_task_status_change_pubsub_latency(tags, state, http_status_code,
                                         latency):
  fields = _extract_given_job_fields(_tags_to_dict(tags), [
      'pool',
  ])
...
def on_task_status_change_scheduler_latency(summary):
  fields = _extract_given_job_fields(_tags_to_dict(summary.tags), [
      'pool',
  ])
...
def on_bot_died_detection_latency(tags, dead_after_ts):
  latency = utils.utcnow() - dead_after_ts
  fields = _extract_given_job_fields(_tags_to_dict(tags), [
      'pool',
  ])
  _bot_died_detection_latencies.add(round(latency.total_seconds() * 1000),
                                    fields=fields)
