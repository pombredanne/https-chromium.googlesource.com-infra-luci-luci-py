...
/**
 * Uses luci-auth to fetch credentials to be used in the live_demo mode.
 * Function will **crash** entire program if it fails for whatever reason.
 *
 * @returns [email, token] where email is logged in users email and token is accessToken for swarming.
 */
function findLiveDemoTokens() {
  // The default scope for both of these calls is
  // https://www.googleapis.com/auth/userinfo.email
  const tokenResult = childProcess.spawnSync("luci-auth", ["token"]);
...
exit_code: ${tokenResult.status}`);
  ...
  if (infoResult.status != 0) {
    console.error(`Error retrieving luci-auth info: ${infoResult.error}
...
exit_code: ${infoResult.status}`);
  ...
  try {
    ...
    console.log(`Failed to parse email from luci-auth info.
stdout: ${infoResult.stdout}
error: ${e}`);
    process.exit(1);
  }
  return [email, tokenResult.stdout.toString()];
}
...
module.exports = (env, argv) => {
  ...
  if (argv.mode !== "production") {
    ...
    if (demoType === "live") {
      const [email, token] = findLiveDemoTokens();
