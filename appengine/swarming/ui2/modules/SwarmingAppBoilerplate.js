...
export default class SwarmingAppBoilerplate extends HTMLElement {
  ...
  constructor(template) {
    ...
    // _app is a reference to the <swarming-app> element
    this._app = null;
    this._auth_header = '';
  ...
  /** @prop {boolean} loggedIn Indicates if a user is logged in and authorized to
                        see this page.
   *                    Read-only. */
  get loggedInAndAuthorized() { return !!this._auth_header && !this._notAuthorized;}
  ...
  /** Handles a fetch error, possibly signaling the user isn't authorized to see this page.*/
  fetchError(e, loadingWhat) {
    if (e.status === 403) {
      this._message = 'User unauthorized - try logging in with a different account';
      this.setUserNotAuthorized();
      this.render();
    } else {
      console.error(e);
