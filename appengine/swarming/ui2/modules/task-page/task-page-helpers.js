...
/** sliceScheduleDeadline returns a human readable time stamp of when a task
 *  slice expires.
 */
export function sliceScheduleDeadline(slice, request) {
...
/** taskScheduleDeadline returns a human readable time stamp of when a task
 *  expires, which is after any and all slices expire.
 */
export function taskScheduleDeadline(request) {
  if (!request.created_ts) {
    return '';
  }
