...
const cloudLoggingURL = (project, query, start, end) => {
  ...
  if (start) {
    ...
    if (end) {
      url += `;timeRange=${start.toISOString()}%2F${end.toISOString()}`;
    }
  }
  url += `?project=${project}`
  return url;
...
const serverTaskLogsURL = (project, taskId, request, result) => {
  ...
  return encodeURI(cloudLoggingURL(project, query, timeStart, timeEnd));
};
...
const botLogsURL = (botProjectID, botZone, request, result) => {
  ...
  return encodeURI(cloudLoggingURL(botProjectID, query, timeStart, timeEnd));
};
