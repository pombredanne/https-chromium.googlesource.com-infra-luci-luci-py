...
import { cipdLink, humanState, isolateLink, parseRequest, parseResult, sliceExpires, taskCost, taskExpires, wasDeduped, wasPickedUp } from './task-page-helpers'
...
<tr>
  ...
  let requestedPackages = cipdInput.packages || [];
  let actualPackages = (result.cipd_pins && result.cipd_pins.packages) || [];
...
`;
  ...
  for (let dim of botDimensions) {
    for (let d of usedDimensions) {
      if (d.key === dim.key) {
        dim.highlight = true;
      }
    }
