...
<tr>
  ...
  <td>
    <a href=${tagMap.source_repo.replace('%s',
              tagMap.source_revision)}>
...
const execution_block = (properties, env) => html`
<tr>
  <td>Extra Args</td>
...
<tr>
  <td rowspan=${env.length+1}>Environment Vars</td>
</tr>
${env.map(env_row)}
<tr>
  <td>Idempotent</td>
...
const env_row = (env) => html`
<tr>
  <td class="break-all">${env.key}=${env.value}</td>
</tr>`;


const taskInfo = (ele, request, result, currentSlice) => html`
...
<tbody>
  ...
  ${dimension_block(currentSlice.properties.dimensions || [])}
  ${isolate_block('Isolated Inputs', currentSlice.properties.inputs_ref || {})}
  ${outputs_block(currentSlice.properties.outputs || [])}
  ${commit_block(request.tagMap)}

  <tr>
    <td>Show Details</td>
    <td>
      <button @click=${() => console.log('should hide')}>
        <add-circle-icon-sk></add-circle-icon-sk>
      </button>
    </td>
  </tr>
</tbody>
<tbody ?hidden=${false && TODO}>
  ${execution_block(currentSlice.properties, currentSlice.properties.env || [])}
...
<swarming-app id=swapp
  ...
  <main>
    <div class=left>
    ${taskInfo(ele, ele._request, ele._result, ele._currentSlice)}
    </div>
    <div class=right>
    ${taskLogs(ele)}
    </div>
