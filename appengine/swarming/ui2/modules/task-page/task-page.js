...
<div class=id_buttons>
  ...
  <button title="Retry the task"
          @click=${ele._promptRetry}>retry</button>
  <button title="Re-queue the task, but don't run it automatically"
          @click=${ele._promptDebug}>debug</button>
</div>`;
}
  ...
  <dialog-pop-over>
    <div class='retry-dialog content'>
      ...
      <div class="horizontal layout end">
        <button @click=${ele._closePopup}>Cancel</button>
        <button @click=${() => ele._promptCallback()}>OK</button>
      </div>
    </div>
  </dialog-pop-over>
</swarming-app>
