...
const serverLogsURL = (ele, request, result) => {
  ...
  if (request.created_ts) {
    ...
    let timeStart = new Date(request.created_ts - 60*1000);
    let timeEnd = result.completed_ts || result.abandoned_ts;
    if (timeEnd) {
      url += `&interval=CUSTOM`
      url += `&dateRangeStart=${timeStart.toISOString()}`
      url += `&dateRangeEnd=${timeEnd.toISOString()}`;
    } else {
      url += `&interval=JUMP_TO_TIME`
      url += `&timestamp=${timeStart.toISOString()}`
    }
  }
