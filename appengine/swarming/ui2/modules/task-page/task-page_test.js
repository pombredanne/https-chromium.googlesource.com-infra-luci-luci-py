...
describe('task-page', function() {
  ...
  describe('api calls', function() {
    ...
    it('pages stdout', function(done) {
      ...
      const FIRST_LINE_LEN = 30 // 💥 takes 4 bytes
      const SECOND_LINE = 'off on the second log line\r\n';
      const SECOND_LINE_LEN = 28
      const THIRD_LINE = 'third log line\n';
      fetchMock.get(`/_ah/api/swarming/v1/task/${TEST_TASK_ID}/stdout?offset=0&length=102400`, {
        state: 'RUNNING',
        output: FIRST_LINE,
      }, {overwriteRoutes: true});
      fetchMock.get(`/_ah/api/swarming/v1/task/${TEST_TASK_ID}/stdout?offset=${FIRST_LINE_LEN}`+
          '&length=102400', {
        state: 'RUNNING',
        output: SECOND_LINE,
      }, {overwriteRoutes: true});
      fetchMock.get(`/_ah/api/swarming/v1/task/${TEST_TASK_ID}/stdout?`+
          `offset=${FIRST_LINE_LEN + SECOND_LINE_LEN}&length=102400`, {
        state: 'COMPLETED',
        output: THIRD_LINE,
      }, {overwriteRoutes: true});
