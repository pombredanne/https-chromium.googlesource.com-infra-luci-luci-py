...
const secondaryOptions = (ele) => {
  ...
  if (!values) {
    return html` <div class="information_only">
      Hmm... no preloaded values. Maybe try typing your filter like
      ${ele._primaryKey}:foo-bar in the above box and hitting enter.
    </div>`;
  }
  ...
  return values.map(
    (value) =>
      html` <div class="item">
        <span class="value"
          >${applyAlias(value, stripTag(ele._primaryKey))}</span
        >
        <span class="flex"></span>
        <add-circle-icon-sk
          ?hidden=${ele._filters.indexOf(makeFilter(ele._primaryKey, value)) >=
          0}
  ...
  class extends SwarmingAppBoilerplate {
    ...
    connectedCallback() {
      ...
      this.addEventListener(
        "tasks-canceling-started",
        this._startedMassCancelingEvent
      );
      ...
      this.addEventListener(
        "tasks-canceling-finished",
        this._finishedMassCancelingEvent
      );
    }
    ...
    _fetch() {
      ...
      const tags = this._filters
        ...
        .map((f) => f.replace("-tag", ""));
      this.app._fetchPermissions(extra, { tags: tags }).then(() => {
    ...
    _filterSearch(e) {
      ...
      if (key !== "state" && !key.endsWith("-tag")) {
        errorMessage(
          `Filters should only be on state or something-tag.
                   Auto-correcting ${key} to ${key}-tag.`,
          6000
        );
    ...
    _matchingBotsLink() {
      ...
      const dimensions = this._filters
        .map((f) => {
    ...
    _rebuildFilterables() {
      this._filteredPossibleColumns = filterPossibleColumns(
        Object.keys(this._possibleColumns),
        this._columnQuery
      );
    ...
    _refilterPrimaryKeys(e) {
      ...
      if (
        this._filterQuery &&
        this._filteredPrimaryArr.length > 0 &&
