...
// It is tempting to import flatpickr.min.css, however, that can
// cause problems when webpack goes to minify because certain CSS
// things can conflict. One that I noticed was keyframe names
// overlapping, and then spinner-sk animates as if it dropped down.
      ...
      .then((json) => {
        ...
        const maybeLoadMore = (json) => {
          ...
          if ((this._filters.length || this._showAll) && json.cursor) {
            ...
            queryParams = listQueryParams(this._filters, this._limit, {
              cursor: json.cursor,
              limit: this._limit,
              start: this._startTime,
              end: this._now ? Date.now() : this._endTime,
