...
describe("task-list", function () {
          ...
          loggedInTasklist((ele) => {
            ...
            expect(link.href).toContain(
              "/tasklist?at=false" +
                ...
                "&c=state&d=desc" +
                "&et=2018-12-19T16%3A46%3A23.234Z" +
                "&f=state%3AQUERY_COMPLETED_FAILURE" +
                "&k=&n=true&s=createdTs" +
                "&st=2018-12-19T16%3A46%3A23.234Z&v=false"
            );
  ...
  describe("dynamic behavior", function () {
        ...
        mockPrpc(
          ...
          (_url, opts) => {
            expect(opts.body).toContain(
              "valid:filter:gpu:can:have:many:colons"
            );
            return true;
          },
          true
    ...
    it("updates the links with filters and other settings", function (done) {
      loggedInTasklist((ele) => {
        ele._startTime = new Date(1545151583000);
        ele._endTime = new Date(1545237981000);
        ...
        expect(link.href).toContain(
          "tasklist?at=false&c=name&c=createdTs" +
            ...
            "&c=state&d=desc&et=2018-12-19T16%3A46%3A21.000Z" +
            "&f=pool-tag%3AChrome" +
            "&f=state%3AQUERY_COMPLETED_FAILURE" +
            "&k=&n=true&s=completedTs&st=2018-12-19T16%3A46%3A21.000Z&v=false"
        );
        done();
      });
    });
    ...
    it("counts correctly when preparing to cancel", function (done) {
      loggedInTasklist((ele) => {
        ...
        fetchMock.flush(true).then(() => {
          ...
          const start = new Date(Date.UTC(2018, 11, 12, 16, 46, 22, 1234));
          const end = new Date(Date.UTC(2018, 11, 19, 16, 46, 22, 1234));
          checkForRequest({
            tags: ["pool:Chrome"],
            state: "QUERY_PENDING",
            start: start.toJSON(),
