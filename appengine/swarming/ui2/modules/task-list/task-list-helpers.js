...
export function hoursAgo(hours) {
  const d = new Date();
  d.setSeconds(d.getSeconds() - hours * 60 * 60);
  return d;
}
...
export function mutateStateFromLegacyState(newState) {
  ...
  // convert start and end, which were timestamps to Date fields
  if (newState.st && typeof newState.st === "number") {
    newState.st = new Date(newState.st * 1000);
  }
  if (newState.et && typeof newState.et === "number") {
    newState.et = new Date(newState.et * 1000);
  }
