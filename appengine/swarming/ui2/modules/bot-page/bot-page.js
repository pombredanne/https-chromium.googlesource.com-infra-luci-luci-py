...
const statusAndTask = (ele, bot) => {
  ...
  return html` <tr
    ...
    <tr class=${bot.is_dead ? 'dead' : ''}>
      <td>Last Seen</td>
      <td title=${bot.human_last_seen_ts}>
        ${timeDiffExact(bot.last_seen_ts)} ago
      </td>
      <td>
        <button
          class="shut_down ${!bot.is_dead && bot.first_seen_ts ? '' : 'hidden'}"
          ?hidden=${bot.is_dead}
        ...
        <button
          class="delete ${bot.is_dead && !bot.deleted ? '' : 'hidden'}"
...
const dataAndMPBlock = (ele, bot) => html`
  <tr title="IP address that the server saw the connection from.">
    <td>External IP</td>
    <td colspan="2">
      <a href=${'http://' + bot.external_ip}>${bot.external_ip}</a>
    </td>
  </tr>
  <tr
...
const taskRow = (task) => html`
  <tr class=${taskClass(task)}>
    <td class="break-all">
      <a target="_blank" rel="noopener" href=${taskPageLink(task.task_id)}>
        ${task.name}
      </a>
    </td>
...
const eventRow = (event, showAll, serverVersion) => {
  ...
  return html` <tr>
    ...
    <td>
      <a target="_blank" rel="noopener" href=${taskPageLink(event.task_id)}>
        ${event.task_id}
      </a>
    </td>
    ...
    class extends SwarmingAppBoilerplate {
      ...
      _fetch() {
        ...
        if (!this._taskCursor) {
          this.app.addBusyTasks(1);
          fetch(
              `/_ah/api/swarming/v1/bot/${this._botId}/tasks?${TASKS_QUERY_PARAMS}`,
              extra,
          )
              .then(jsonOrThrow)
              .then((json) => {
        ...
        if (!this._eventsCursor) {
          this.app.addBusyTasks(1);
          fetch(
              `/_ah/api/swarming/v1/bot/${this._botId}/events?${EVENTS_QUERY_PARAMS}`,
              extra,
          )
              .then(jsonOrThrow)
              .then((json) => {
      ...
      _moreEvents() {
        ...
        this.app.addBusyTasks(1);
        fetch(
            `/_ah/api/swarming/v1/bot/${this._botId}/events?cursor=${this._eventsCursor}&` +
          EVENTS_QUERY_PARAMS,
            extra,
        )
            .then(jsonOrThrow)
      ...
      _moreTasks() {
        ...
        this.app.addBusyTasks(1);
        fetch(
            `/_ah/api/swarming/v1/bot/${this._botId}/tasks?cursor=${this._taskCursor}&` +
          TASKS_QUERY_PARAMS,
            extra,
        )
            .then(jsonOrThrow)
