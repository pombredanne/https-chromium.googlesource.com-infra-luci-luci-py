    ...
    describe('gpu bot with a running task', function() {
      ...
      it('renders some of the bot data', function(done) {
        loggedInBotPage((ele) => {
          ...
          expect(rows[1]).toHaveClass('hidden', 'not quarantined');
          expect(rows[2]).toHaveClass('hidden', 'not dead');
          expect(rows[3]).toHaveClass('hidden', 'not in maintenance');
          expect(cell(4, 0)).toMatchTextContent('Current Task');
          expect(cell(4, 1)).toMatchTextContent('42fb00e06d95be11');
          expect(cell(4, 1).innerHTML).toContain('<a ', 'has a link');
          expect(cell(4, 1).innerHTML).toContain('href="/task?id=42fb00e06d95be10"');
          expect(cell(5, 0)).toMatchTextContent('Dimensions');
          expect(cell(10, 0)).toMatchTextContent('gpu');
          expect(cell(10, 1)).toMatchTextContent('NVIDIA (10de) | ' +
            'NVIDIA Quadro P400 (10de:1cb3) | NVIDIA Quadro P400 (10de:1cb3-25.21.14.1678)');
          expect(cell(22, 0)).toMatchTextContent('Bot Version');
          expect(rows[22]).toHaveClass('old_version');
          done();
        });
      });
    ...
    describe('quarantined android bot', function() {
      ...
      it('displays a quarantined message', function(done) {
        loggedInBotPage((ele) => {
          ...
          expect(rows[1]).not.toHaveClass('hidden', 'quarantined');
          expect(cell(1, 1)).toMatchTextContent('No available devices.');
          expect(rows[2]).toHaveClass('hidden', 'not dead');
          expect(rows[3]).toHaveClass('hidden', 'not in maintenance');
          expect(cell(4, 0)).toMatchTextContent('Current Task');
          expect(cell(4, 1)).toMatchTextContent('idle');
    ...
    describe('dead machine provider bot', function() {
      ...
      it('displays dead and mp related info', function(done) {
        loggedInBotPage((ele) => {
          ...
          expect(rows[0]).toHaveClass('dead');
          expect(rows[1]).toHaveClass('hidden', 'not quarantined');
          expect(rows[2]).not.toHaveClass('hidden', 'dead');
          expect(rows[3]).toHaveClass('hidden', 'not in maintenance');
          expect(rows[26]).not.toHaveAttribute('hidden');
          expect(cell(26, 0)).toMatchTextContent('Machine Provider Lease ID');
          expect(cell(26, 1)).toMatchTextContent('f69394d5f68b1f1e6c5f13e82ba4ccf72de7e6a0');
          expect(cell(26, 1).innerHTML).toContain('<a ', 'has a link');
          expect(cell(26, 1).innerHTML).toContain('href="https://example.com/leases/'+
