...
const fleetCountsToggleSwitch = (ele) => {
  if (ele._showFleetCounts) {
    return html`<expand-less-icon-sk @click=${(e) => ele._toggleFleetsCount(e)}></expand-less-icon-sk>`;
  } else {
    return html`<expand-more-icon-sk @click=${(e) => ele._toggleFleetsCount(e)}></expand-more-icon-sk>`;
  }
};
...
<swarming-app id=swapp
  ...
  <main @click=${e => ele._showColSelector && ele._toggleColSelector(e)}>
    <h2 class=message ?hidden=${ele.loggedInAndAuthorized}>
      You must sign in to see anything useful.
    </h2>
