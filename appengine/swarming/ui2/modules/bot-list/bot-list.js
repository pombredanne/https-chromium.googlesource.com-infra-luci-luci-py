...
<swarming-app id=swapp
  ...
  <main @click=${(e) => ele._showColSelector && ele._toggleColSelector(e)}>
    ...
    <button ?hidden=${
      !ele.loggedInAndAuthorized || !!ele._filters.length || ele._showAll
    }
  ...
  class extends SwarmingAppBoilerplate {
    ...
    connectedCallback() {
      ...
      this.addEventListener(
        "bots-deleting-started",
        this._startedMassDeletingEvent
      );
      ...
      this.addEventListener(
        "bots-deleting-finished",
        this._finishedMassDeletingEvent
      );
    }
    ...
    _makeSummaryURL(newFilter, preserveOthers) {
      ...
      const withNewState = this._filters.filter((f) => {
        if (filterStr.startsWith("task")) {
          return !f.startsWith("task");
        }
