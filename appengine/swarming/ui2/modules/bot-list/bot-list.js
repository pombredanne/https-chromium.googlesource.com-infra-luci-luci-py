...
import { aggregateTemps, attribute, botLink, column, colHeaderMap,
         devices, extraKeys, fromDimension, fromState, longestOrAll, makeFilter, processBots,
...
<div class=item selected?=${ele._primaryKey === key}>
  ...
  <checkbox-sk checked?=${ele._cols.indexOf(key) >= 0}
               ...
               on-click=${(e) => ele._toggleCol(e, key)}>
  </checkbox-sk>
</div>
`
...
<i>${ele._primaryKey}</i> is a part of the bot's state and is informational only.</div>`
  ...
  <arrow-forward-icon-sk hidden?=${ele._filters.indexOf(makeFilter(ele._primaryKey, value)) >= 0}
                        on-click=${() => ele._addFilter(ele._primaryKey, value)}>
...
<div class=item>
  ...
  <span class=flex></span>
  <remove-circle-outline-icon-sk on-click=${() => ele._removeFilter(filter)}>
...
<select-sk class="selector keys" on-selection-changed=${(e) => ele._primayKeyChanged(e)}>
...
window.customElements.define('bot-list', class extends SwarmingAppBoilerplate {
  ...
  /* TODO(kjlubick) add properties for anything that gets mirrored to
  query params and combine it with stateReflector */
