...
<div class=options>
  ...
  <a href="https://example.com/TODO">View Matching Tasks</a>
...
<table id=query_counts>
  ${summaryQueryRow({label: 'Displayed', value: ele._bots.length})}
...
window.customElements.define('bot-list', class extends SwarmingAppBoilerplate {
  ...
  constructor() {
    ...
    this._columnQuery = '';
    this._filterQuery = '';
    // Allows us to abort fetches that are tied to filters when
    // filters change.
    ...
    fetch('/_ah/api/swarming/v1/bots/list?' + queryParams, extra)
      ...
      .then((json) => {
        ...
        const maybeLoadMore = (json) => {
          ...
          this.render();
          // Special case: Don't load all the bots when filters is
