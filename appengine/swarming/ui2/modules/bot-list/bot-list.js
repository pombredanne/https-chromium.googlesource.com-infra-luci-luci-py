...
<div class=item selected?=${ele._primaryKey === key}>
  ...
  <checkbox-sk checked?=${ele._cols.indexOf(key) !== -1}
...
<i>${ele._primaryKey}</i> is a part of the bot's state and is informational only.</div>`
  ...
  <span class=flex></span>
  <arrow-forward-icon-sk hidden?=${false/* TODO? kjlubick*/}
                        on-click=${(e) => ele._addFilter(e, ele._primaryKey, value)}>
...
<select-sk class="selector values">
  ${secondaryOptions(ele)}
</select-sk>
<!-- filters selector-->
<select-sk class="selector filters">
...
window.customElements.define('bot-list', class extends SwarmingAppBoilerplate {
  ...
  _addFilter(e, key, value) {
    // TODO(kjlubick): port _cantAddFilter
