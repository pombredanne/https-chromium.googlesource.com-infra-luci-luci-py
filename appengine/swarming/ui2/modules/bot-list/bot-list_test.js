...
import { column, processBots, processDimensions, processPrimaryMap } from 'modules/bot-list/bot-list-helpers'
...
//===============TESTS START====================================
  ...
  describe('dynamic behavior', function() {
    ...
    fit('changes columns by clicking on the boxes', function(done) {
      createElement((ele) => {
        userLogsIn(ele, () => {
          ...
          checkbox.click();
          // listen for collumns-changed event ?

          let colHeaders = ele.querySelectorAll('.bot-table thead th');
          expect(colHeaders).toBeTruthy();
          expect(colHeaders.length).toBe(5, '(num colHeaders)');
          expect(colHeaders).toContain(keyToClick);
          done();
        });
      });
    });
