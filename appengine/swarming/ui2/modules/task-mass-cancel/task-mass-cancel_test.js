...
describe('task-mass-cancel', function() {
  ...
  function createElement(test) {
    return window.customElements.whenDefined('task-mass-cancel').then(() => {
      container.innerHTML = `<task-mass-cancel start=10000 end=20000 now=false tags="pool:Chrome,os:Android" auth_header="fake"></task-mass-cancel>`;
  ...
  it('makes an API call to delete after clicking', function(done) {
    createElement((ele) => {
      ...
      ele.addEventListener('tasks-canceling-finished', () => {
        ...
        expect(calls[0][0]).toEqual('/_ah/api/swarming/v1/tasks/list?end=20000&fields=cursor%2Citems%2Ftask_id&limit=200&start=10000&state=PENDING&tags=os%3AAndroid&tags=pool%3AChrome');
        done();
      });
