...
import { hoursAgo } from "../task-list/task-list-helpers";
...
window.customElements.define(
  ...
  class extends HTMLElement {
    ...
    connectedCallback() {
      ...
      // If start / end are loaded from attributes they will be strings
    ...
    _cancelAll() {
      ...
      const maybeCancelMore = (resp) => {
        this._progress += parseInt(resp.matched);
        this.render();
        if (resp.cursor) {
          const payload = {
            limit: CANCEL_BATCH_SIZE,
    ...
    _fetchCount() {
      ...
      const pendingPromise = service
        .count(pendingParams)
        .then((resp) => {
          this._pendingCount = parseInt(resp.count);
        })
      ...
      const runningPromise = service
        .count(runningParams)
        .then((resp) => {
          this._runningCount = parseInt(resp.count);
        })
