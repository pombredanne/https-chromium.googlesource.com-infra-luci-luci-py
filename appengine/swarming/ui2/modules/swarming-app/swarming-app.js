...
window.customElements.define('swarming-app', class extends HTMLElement {
  ...
  /** @prop {Object} server_details The details about the server or a placeholder
                     object if the user is not logged in or not authorized.
   *                 Read-only. */
  ...
  _fetch() {
    ...
    fetch('/_ah/api/swarming/v1/server/details', auth)
      ...
      .then((json) => {
        ...
        this._render();
        this.dispatchEvent(new CustomEvent('server-details-loaded', {bubbles: true}));
        this.finishedTask();
      })
      .catch((e) => {
        if (e.status === 403) {
    ...
    fetch('/_ah/api/swarming/v1/server/permissions', auth)
      ...
      .then((json) => {
        ...
        this._render();
        this.dispatchEvent(new CustomEvent('permissions-loaded', {bubbles: true}));
        this.finishedTask();
      })
      .catch((e) => {
        if (e.status !== 403) {
          console.error(e);
          errorMessage(`Unexpected error loading permissions: ${e.message}`, 5000);
        }
        this.finishedTask();
      });
  }
