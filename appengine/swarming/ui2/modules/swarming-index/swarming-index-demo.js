...
// details are public
fetchMock.get('/api/swarming/v1/server/details', JSON.stringify(details));
...
const respond = function(logged_in, logged_out) {
  return function(url, opts){
    if (opts && opts.headers && opts.headers.authorization) {
      ...
      return {
        status: 200,
        body: JSON.stringify(logged_out),
        headers: {'Content-Type':'application/json'},
      };
    }
  };
...
fetchMock.get('/api/swarming/v1/server/permissions',
              respond(logged_in_permissions, logged_out_permissions));

const logged_in_token = {
  bootstrap_token: '8675309JennyDontChangeYourNumber8675309'
}

const logged_out_token = {}

fetchMock.post('/api/swarming/v1/server/token',
              respond(logged_in_token, logged_out_token));