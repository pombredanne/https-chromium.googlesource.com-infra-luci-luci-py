...
window.customElements.define('swarming-index', class extends HTMLElement {
  ...
  update() {
    ...
    fetch('/api/swarming/v1/server/details', extra)
      .then(jsonOrThrow)
      .then((json) => {
    ...
    fetch('/api/swarming/v1/server/permissions', extra)
      .then(jsonOrThrow)
      .then((json) => {
    ...
    extra.method = 'POST';
    fetch('/api/swarming/v1/server/token', extra)
      .then(jsonOrThrow)
      .then((json) => {
  ...
  connectedCallback() {
    ...
    console.log('adding log-in listener')
    this.addEventListener('log-in', (e) => {
      console.log('Saw login ', e);
