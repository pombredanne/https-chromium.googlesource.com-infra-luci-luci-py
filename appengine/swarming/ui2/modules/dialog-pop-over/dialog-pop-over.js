 ...
 * </p>
 *
 * @fires dialog-negative when a button with class .negative is clicked
 * @fires dialog-positive when a button with class .positive is clicked
 */
...
window.customElements.define('dialog-pop-over', class extends HTMLElement {
  ...
  connectedCallback() {
    ...
    for (let btn of $('.negative', this)) {
      btn.addEventListener('click', () => {
        btn.dispatchEvent(new CustomEvent('dialog-negative', {bubbles: true}));
      });
    }

    for (let btn of $('.positive', this)) {
      btn.addEventListener('click', () => {
        btn.dispatchEvent(new CustomEvent('dialog-positive', {bubbles: true}));
      });
    }
  }
  ...
  /** hide makes the content centered and visible. It also makes the semi-transparent
