 ...
 *   or none. They can be joined together via the current property
...
<arrow-drop-down-icon-sk ?hidden=${ele.key === ele.current && ele.direction === 'asc'}>
</arrow-drop-down-icon-sk>
<arrow-drop-up-icon-sk ?hidden=${ele.key === ele.current && ele.direction === 'desc'}>
...
window.customElements.define('sort-toggle', class extends HTMLElement {
  ...
  /** @prop {string} current - The currently selected sort key for a
   ...
   *                  direction the user indicated. Is ignored if current
