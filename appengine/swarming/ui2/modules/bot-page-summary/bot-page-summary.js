...
const taskRow = (task, ele) => html`
  <tr>
    <td title=${task.full_name} class="break-all">
      ${ele._shortenName(task.full_name)}
    </td>
    <td>${task.total}</td>
    <td>${task.success}</td>
    <td>${task.failed}</td>
  ...
  <div>
    <table>
      <thead>
        <tr>
          <th
            title="How much time passed between the oldest task fetched and now."
          >
            Total Wall Time
          </th>
    ...
    <div class="controls">
      <checkbox-sk ?checked=${ele._fullNames} @click=${ele._toggleName}>
      </checkbox-sk>
      ...
      <span ?hidden=${ele._summarized.length <= SHOW_LIMIT}
        >Show All Tasks</span
      >
    </div>
  </div>
...
window.customElements.define(
     ...
     */
      ...
      _aggregate() {
        ...
        // and now.
        totalStats.wall_time =
        (now - this.tasks[this.tasks.length - 1].started_ts) / 1000;
      ...
      _sortAndLimitTasks() {
        ...
        return this._summarized.slice(
            0,
            Math.min(this._summarized.length, SHOW_LIMIT),
        );
      }
