...
class TaskState(messages.Enum):
  ...
  # overhead phase; mapping inputs files or archiving outputs back to the
  # server.
  RUNNING = 1
  ...
  ALL = 10
  # Query for all tasks that are COMPLETED but that actually didn't ran due to
  # TaskProperties.idempotent being True *and* that a previous task with the
  # exact same TaskProperties had successfully ran before.
  DEDUPED = 11
...
class StateField(messages.Enum):
  ...
  # will stay in this state until either a task reap it or that the expiration
  ...
  KILLED = 0x80
  # The task was never set PENDING and was immediately refused, as the server
