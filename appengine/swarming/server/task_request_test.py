...
class TaskRequestApiTest(TestCase):
  ...
  def test_cron_send_to_bq_empty(self):
    ...
    self.assertEqual(None, task_request.bq.BqState.get_by_id('task_requests'))
  ...
  def test_cron_send_to_bq(self):
    ...
    def json_request(url, method, payload, scopes, deadline):
      ...
      self.assertEqual(task_request.bq.bqh.INSERT_ROWS_SCOPE, scopes)
      self.assertEqual(600, deadline)
      return {'insertErrors': []}
    self.mock(task_request.bq.net, 'json_request', json_request)
    ...
    self.assertEqual(
        expected, task_request.bq.BqState.get_by_id('task_requests').to_dict())
