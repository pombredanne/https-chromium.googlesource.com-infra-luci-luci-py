...
def _gen_request_slices(**kwargs):
  ...
  task_template_deployment = kwargs.pop(
      '_task_template_deployment', swarming_rpcs.PoolTaskTemplate.AUTO)
  now = utils.utcnow()
  args = {
    u'created_ts': now,
  ...
  task_request.init_new_request(req, True, task_template_deployment)
  return req
...
class TestCase(test_case.TestCase):
  ...
  @staticmethod
  def _tt(cache=None, cipd_package=None, env=None):
...
class TaskRequestPrivateTest(TestCase):
  ...
  def test_apply_template_conflicting_cipd_package(self):
    tt = self._tt(cipd_package={('C', 'pkg'): 'latest'})
    p = task_request.TaskProperties(
        cipd_input=task_request.CipdInput(
            packages=[
              task_request.CipdPackage(
...
class TaskRequestApiTest(TestCase):
  def setUp(self):
    ...
    self._pool_configs = {}
    self.mock(
        pools_config, 'get_pool_config', self._pool_configs.get)
  ...
  def test_init_new_request_skip_template(self):
    self._set_pool_config_with_templates(self._tt(env={'hi': 'prod'}))

    request = _gen_request(
      _task_template_deployment=swarming_rpcs.PoolTaskTemplate.SKIP)
    as_dict = request.to_dict()
  ...
  def test_init_new_request_missing_template(self):
    ...
    as_dict = request.to_dict()
    self.assertIn(u'swarming.pool.version:testVersion1', as_dict['tags'])
    self.assertIn(u'swarming.pool.template:missing', as_dict['tags'])

  def test_init_new_request_prod_template(self):
    self._set_pool_config_with_templates(
      self._tt(env={'hi': 'prod'}),
      canary=None,
  ...
  def test_init_new_request_canary_never_template(self):
    ...
    request = _gen_request(
      _task_template_deployment=swarming_rpcs.PoolTaskTemplate.CANARY_NEVER,
    )
    as_dict = request.to_dict()
    self.assertIn(u'swarming.pool.version:testVersion1', as_dict['tags'])
    self.assertIn(u'swarming.pool.template:prod', as_dict['tags'])
  ...
  def test_init_new_request_canary_prefer_template(self):
    ...
    request = _gen_request(
      _task_template_deployment=swarming_rpcs.PoolTaskTemplate.CANARY_PREFER,
    )
    as_dict = request.to_dict()
    self.assertIn(u'swarming.pool.version:testVersion1', as_dict['tags'])
    self.assertIn(u'swarming.pool.template:canary', as_dict['tags'])
