...
class TaskRequestApiTest(TestCase):
  ...
  def test_get_automatic_tags(self):
    ...
    req = mkreq(_gen_request_slices(task_slices=slices))
    expected = set(
        (u'service_account:none', u'pool:default',
          u'hostname:localhost', u'user:Jesus', u'priority:50',
          u'OS:Windows-3.1.1'))
    self.assertEqual(expected, task_request.get_automatic_tags(req, 0))
    expected = set(
        (u'service_account:none', u'pool:GPU', u'user:Jesus', u'priority:50'))
