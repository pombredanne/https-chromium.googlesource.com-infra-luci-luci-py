...
class FakeExternalScheduler(object):
  ...
  def NotifyTasks(self, req, credentials):
    self._test.assertIsInstance(req, plugin_pb2.NotifyTasksRequest), repr(req)
...
class ExternalSchedulerApiTest(test_case.TestCase):
  ...
  def _enqueue(self, url, queue_name, payload=None):
