...
def _fresh_BotTaskDimensions_async(bot_dimensions, bot_root_key, now):
  ...
  fresh = yield [
      _fresh_BotTaskDimensions_slice_async(bot_dimensions_set, bot_root_key, now, r)
      for r in roots
  ]
...
def _assert_task_props_async(properties, expiration_ts):
  ...
  if obj:
    ...
    for task_dims in expand_dimensions_to_flats(properties.dimensions):
      ...
      # contains a list of dimension lists all matching `dimensions_hash`, even
      # if they just happen to hash to the same integer. We need to enqueue a
      # task to add new dimensions to this list.
      if not s:
        logging.info(
