...
class TaskDimensionsSet(ndb.Model):
  ...
  # 'key:value' strings. This is stored to enable _match_bot(). Stores flattened
  # dimension sets as produced by expand_dimensions_to_flats, i.e. dimension
  # values here never have "|" in them. Such dimension sets can directly be
  # matched with bot dimensions (as happens in _match_bot).
  dimensions_flat = ndb.StringProperty(repeated=True, indexed=False)
  ...
  def _equals(self, dimensions_flat_set):
    ...
    # self.dimensions_flat is validated to not have duplicates in _pre_put_hook.
...
def _tidy_stale_BotTaskDimensions(now):
  ...
  Returns:
    Number of deleted BotTaskDimensions entities.
  """
  ...
  try:
    ...
    while (yield qit.has_next_async()):
      ...
      bot_task_key = ndb.Key(
          BotTaskDimensions, task_dimensions_hash, parent=bot_dims_key.parent())
      viable += 1
...
def _refresh_TaskDimensions_async(now, valid_until_ts, task_dimensions_flats,
  ...
  try:
    outcome = yield _ensure_TaskDimensions_async(
...
def rebuild_task_cache_async(payload):
  ...
  try:
    ...
    success = yield _refresh_TaskDimensions_async(now, valid_until_ts,
