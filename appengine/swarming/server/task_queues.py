...
def _refresh_TaskDimensions_async(now, valid_until_ts, task_dimensions_flats,
  ...
  obj = yield task_dimensions_key.get_async()
  if obj and not any(
      map(lambda df: obj.assert_request(now, valid_until_ts, df),
          task_dimensions_flats)):
...
def rebuild_task_cache_async(payload):
  ...
  expanded_task_dimensions_flats = _expand_dimensions_to_dimensions_flat(
      task_dimensions)
  try:
    # Couldn't use lambda because it cannot fit in one line within 80 chars.
    def refresh_bot(dimensions_flat):
      return _refresh_all_BotTaskDimensions_async(
          now, valid_until_ts, dimensions_flat, task_dimensions_hash)

    yield map(refresh_bot, expanded_task_dimensions_flats)
