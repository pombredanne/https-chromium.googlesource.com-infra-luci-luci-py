...
# A single set of task dimensions together with a timestamp when it should be
# removed from the datastore. Stored as a JSON dict inside TaskDimensionsSets
# and TaskDimensionsInfo.
#
...
# matched with bot dimensions using set operation.
#
# Expiry is populated when the set is stored inside TaskDimensionsInfo and None
# when it is stored in TaskDimensionsSets. Expiry is always larger (with some
# wide margin) than the scheduling deadline of the last task that used these
# dimensions and it is randomized to avoid expiration stampedes.
_TaskDimensionsSet = collections.namedtuple('_TaskDimensionsSet', [
    'dimensions', # a list of "key:value" strings
    'expiry',     # an utils.datetime_to_timestamp(...) timestamp or None
])
...
class TaskDimensionsSets(ndb.Model):
  ...
  Root entity. Key ID is
    * `bot:<id>:<dimensions_hash>` - for tasks targeting a bot via its ID.
    * `pool:<id>:<dimensions_hash>` - for tasks targeting a pool via its ID.

  Where `<dimensions_hash>` is a 32 bit integer with the task dimensions hash as
  generated by hash_dimensions(...).
  ...
  rescans that happen in _tq_rescan_matching_tasks), as well as improves caching
  efficiency of direct ndb.get_multi fetches of TaskDimensionsSets in
  assert_bot(...).
  """
  # Disable useless in-process per-request cache to save some RAM.
  _use_cache = False

  # All matching dimensions sets as a list of _TaskDimensionsSet dicts.
  #
  # The order is irrelevant. Never empty. `expiry` in each dict is set to None.
...
class TaskDimensionsInfo(ndb.Model):
  ...
  # All matching dimensions sets as a list of _TaskDimensionsSet dicts.
  #
  # The order is irrelevant. Never empty. `expiry` in each dict is populated.
  sets = datastore_utils.DeterministicJsonProperty(indexed=False)
...
class BotDimensionsMatches(ndb.Model):
  ...
  # Incremented in every transaction, used to skip obsolete TQ tasks.
  #
  # When a task queue task starts, if the current entity revision is different
  # from the revision in the task payload, then this task is no longer relevant
  ...
  # a datastore query, see _tq_assign_tasks_to_bots_async.
  dimensions = ndb.StringProperty(repeated=True)
  ...
  #     for matching bots in _tq_assign_tasks_to_bots_async. This reduces
  #     latency of scheduling of new kinds of tasks.
  #   * Periodically, and on bot dimensions change, there's a full rescan of
  #     all existing TaskDimensionsSets, see _tq_rescan_matching_tasks.
  #
  ...
  #
  # Unmatched entries are also opportunistically deleted by transactions that
  # add new entries, to make sure it the list never grows too big.
  matches = ndb.StringProperty(repeated=True, indexed=False)

  # The last time when this entity was updated.
  #
  # It is also the time when `dimensions` was cleaned of "unmatched" entries.
  last_update_ts = ndb.DateTimeProperty(indexed=False)

  # The last time when the full rescan of TaskDimensionsSets was triggered.
  #
  # Note: it is updated when triggering the asynchronous rescan via a task queue
  # task, not when it completes.
  last_rescan_ts = ndb.DateTimeProperty(indexed=False)
...
@ndb.tasklet
def _cleanup_task_dimension_sets_async():
...
@ndb.tasklet
def _tq_assign_tasks_to_bots_async(task_sets_id, dimensions):
  """A task queue task to assign new task dimension set to all matching bots.

  Arguments:
    task_sets_id: string ID of TaskDimensionsSets entity to match on.
    dimensions: a _TaskDimensionsSet tuple with dimensions to match on.

  Returns:
    True if assigned, False if the TQ task needs to be retried.
  """
  # TODO:
  #
  # * Give up if `dimensions` expired already.
  # * Check that TaskDimensionsSets entity still has `dimensions` inside of it.
...
@ndb.tasklet
def _tq_rescan_matching_tasks(bot_id, revision):
  """A task queue task that finds all matching TaskDimensionsSets for a bot.

  Arguments:
    bot_id: ID of the bot.
    revision: revision of BotDimensionsMatches when the task was enqueued.
...
def assert_bot(bot_root_key, bot_dimensions):
  ...
  # Ignore the result of the new implementation for now.
...
def assert_task_async(request):
  ...
  for i in range(request.num_task_slices):
    ...
    futures.append(_assert_task_dimensions_async(t.properties.dimensions, exp_ts))
  yield futures
