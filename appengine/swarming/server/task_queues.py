...
#
# Expiry is populated when the set is stored inside TaskDimensionsInfo and it is
# None when it is stored in TaskDimensionsSets. Expiry is always larger (with
# some wide margin) than the scheduling deadline of the last task that used
# these dimensions and it is randomized to avoid expiration stampedes.
_TaskDimensionsSet = collections.namedtuple('_TaskDimensionsSet', [
    'dimensions', # a list of "key:value" strings
    'expiry',     # an integer with Unix timestamp in seconds or None
])
