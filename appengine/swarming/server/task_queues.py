  ...
  try:
    while (yield qit.has_next_async()):
      ...
      if s and s.valid_until_ts >= now:
        ...
        logging.debug(
            'crbug.com/1064848: found a matching TaskDimension, but was stale.')
  finally:
    logging.debug(
...
def _rebuild_bot_cache_async(bot_dimensions, bot_root_key):
  ...
  try:
    ...
    # The expirations are distributed to avoid accessing Datastore
    # at the same time.
...
def assert_bot_async(bot_root_key, bot_dimensions):
  ...
  # call and still valids. It rebuilds cache every 20-40 minutes to avoid
  # missing tasks that can be assigined to the bot. Tasks can be missed because
  ...
  if (obj and obj.dimensions_flat == bot_dimensions_to_flat(bot_dimensions) and
    ...
    logging.debug('assert_bot_async: cache hit. bot_id: %s, valid_until: %s\n'
                  'bot_dimensions: %s',
                  bot_root_key.string_id(), obj.valid_until_ts, bot_dimensions)
    raise ndb.Return(None)
