...
class TaskToRunApiTest(test_env_handlers.AppTestBase):
  ...
  def test_set_lookup_cache(self):
    ...
    # By default, the negative cache is false, e.g. it is safe to reap the task.
    ...
    self.assertEqual(False, lookup(to_run_1.key))
    self.assertEqual(False, lookup(to_run_2.key))
    # Mark to_run_1 as unreapable, e.g. a bot is about to reap it.
    ...
    self.assertEqual(False, lookup(to_run_1.key))
    self.assertEqual(False, lookup(to_run_2.key))
    # Mark to_run_2 as unreapable, e.g. a bot is about to reap it.
