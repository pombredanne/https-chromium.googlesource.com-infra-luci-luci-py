...
class TaskToRunApiTest(test_env_handlers.AppTestBase):
  ...
  def test_yield_expired_task_to_run(self):
    ...
    expired_task_to_runs = [r for r in task_to_run.yield_expired_task_to_run()]

    # only to_run_2 and to_run_3 should be yielded
    self.assertEqual(2, len(expired_task_to_runs))
    self.assertIn(to_run_2, expired_task_to_runs)
    self.assertIn(to_run_3, expired_task_to_runs)
