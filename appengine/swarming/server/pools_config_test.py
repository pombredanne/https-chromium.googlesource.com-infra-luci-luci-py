    ...
    pool=[
        pools_pb2.Pool(
            ...
            scheduling_algorithm=(
                pools_pb2.Pool.SchedulingAlgorithm.Value('SA_FIFO')),
        ),
    ],
...
class PoolsConfigTest(test_case.TestCase):
  ...
  def test_no_scheduling_algorithm(self):
    ...
    self.assertEqual(pools_pb2.Pool.SchedulingAlgorithm.Value('SA_FIFO'),
  ...
  def test_lifo_scheduling_algorithm(self):
    cfg = pools_pb2.PoolsCfg(
        pool=[pools_pb2.Pool(name=['abc'],
                             scheduling_algorithm=(
                                 pools_pb2.Pool.
                                 SchedulingAlgorithm.Value('SA_LIFO')))])
    self.mock_config(cfg)
    self.assertEqual(pools_pb2.Pool.SchedulingAlgorithm.Value('SA_LIFO'),
                     pools_config.get_pool_config('abc').scheduling_algorithm)
