...
class PoolsConfigTest(test_case.TestCase):
  ...
  def test_good_bot_monitoring(self):
    cfg = pools_pb2.PoolsCfg(
        ...
        bot_monitoring=[
          pools_pb2.BotMonitoring(name='mon', dimension_keys='a'),
        ])
    self.validator_test(cfg, [])
...
class TestBotMonitoring(TaskTemplateBaseTest):
  ...
  def test_bad_dimension_keys(self):
    bm = self.parse("""
    name: "hi"
    dimension_keys: "first "
    """)
    self.validator_test(
        [bm], ['bot_monitoring u\'hi\': invalid dimension_keys u\'first \''])

  def test_bad_repeated_dimension_keys(self):
    bm = self.parse("""
    name: "hi"
    dimension_keys: "same"
    dimension_keys: "same"
    """)
    self.validator_test(
        [bm], ['bot_monitoring u\'hi\': duplicate dimension_keys'])
