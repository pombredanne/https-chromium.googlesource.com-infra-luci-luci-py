...
class TaskTemplateBaseTest(unittest.TestCase):
  ...
  @staticmethod
  def tt(cache=None, cipd_package=None, env=None):
...
class TestTaskTemplates(TaskTemplateBaseTest):
  ...
  def test_task_template_update_env(self):
    tti = pc.TaskTemplate._Intermediate()
    tti.update(self.tt(env=[pc.Env('VAR', '1', (), True)]))
    ...
    # override existing
    tti.update(self.tt(env=[pc.Env('VAR', '2', (), False)]))
  ...
  def test_task_template_update_env_prefix(self):
    tti = pc.TaskTemplate._Intermediate()
    tti.update(self.tt(env=[pc.Env('PATH', '', ('1',), True)]))
    ...
    # append existing
    tti.update(self.tt(env=[pc.Env('PATH', '', ('2',), False)]))
  ...
  def test_finalize_empty_values(self):
    ...
    self.assertEqual(
      ...
      [
        ...
        'cipd_package[#1]: empty path',
        'cipd_package[#2]: empty pkg',
        'env[#0]: empty name',
        'env[\'VAR\']: prefix contains ""',
  ...
  def test_resolve_inclusions(self):
    poolcfg = self.parse("""
      ...
      task_template: {
        ...
        env: {var: "PATH" prefix: "something" prefix: "else" soft: true}
      }
    """)
    ...
    self.assertEqual(template_map['d'], self.tt(
      ...
      env=(
        pc.Env('PATH', '', ('something', 'else'), True),
        pc.Env('VAR', '1', (), False),
      ),
    ))
...
class TestPoolCfgTaskTemplateDeployments(TaskTemplateBaseTest):
  ...
  def test_resolve_bad_canary(self):
    poolcfg = self.parse("""
      task_template_deployment: {name: "a" canary_chance: 100}
    """)
    ...
    self.assertEqual(
      [x.text for x in self.ctx.result().messages],
      ["deployment['a']: canary_chance out of range `[0,1)`: 100.0"])
  ...
  def test_resolve_bad_canary_2(self):
    ...
    self.assertEqual(
      [x.text for x in self.ctx.result().messages],
      ["deployment['a']: canary_chance out of range `[0,1)`: -1.0"])
  ...
  def test_resolve_single_deployment(self):
    ...
    self.assertEqual(pc.TaskTemplateDeployment(
      prod=self.tt(env=(pc.Env("VAR", "1", (), False),)),
