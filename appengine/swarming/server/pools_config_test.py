...
class TestTaskTemplates(TaskTemplateBaseTest):
  ...
  def test_task_template_update_cache(self):
    ...
    tti.update(self.ctx, self.tt(cache=[self.PCE(name='hi', path='there')]))
    ...
    # override existing
    tti.update(self.ctx, self.tt(cache=[self.PCE(name='hi', path='nerd')]))
  ...
  def test_task_template_update_env(self):
    tti = pools_config.TaskTemplate._Intermediate(self.ctx)
    tti.update(self.ctx, self.tt(
        env=[self.PE(var='VAR', value='1', soft=True)]))
    ...
    # override existing
    tti.update(self.ctx, self.tt(env=[self.PE(var='VAR', value='2')]))
  ...
  def test_task_template_update_env_prefix(self):
    ...
    tti.update(self.ctx, self.tt(env=[
        self.PE(var='PATH', prefix=['1'], soft=True)]))
    ...
    # append existing
    tti.update(self.ctx, self.tt(env=[self.PE(var='PATH', prefix=['2'])]))
  ...
  def test_finalize_overlapping_paths(self):
    ...
    self.assertEqual(
        ...
        [
          ("cache 'other_name' uses 'cache_cipd/path', which conflicts with "
           "cipd['other/pkg2:latest', 'other/pkg:latest'] using"
           " 'cache_cipd'"),
          ("cache 'whatnow' uses 'good/path/b', which conflicts with cache"
           " 'some_name' using 'good/path'"),
        ])
  ...
  def test_finalize_empty_values(self):
    ...
    self.assertEqual(
      ...
      [
        ...
        (
            "'': directory has conflicting owners: cache 'cool_name' and"
            " cipd['some/pkg:latest']"
        ),
      ])
...
class TestPoolCfgTaskTemplate(TaskTemplateBaseTest):
  ...
  def test_inclusion_cycle(self):
    ...
    self.assertEqual(
        [x.text for x in self.ctx.result().messages],
        [
            'template[\'a\']: template[\'b\']: depends on \'a\'' + tail,
            'template[\'a\']: depends on \'b\'' + tail,
        ])
  ...
  def test_bad_include(self):
    ...
    self.assertEqual(
        ...
        [
          ...
          "template['b']: unknown include: 'nope'",
        ])
