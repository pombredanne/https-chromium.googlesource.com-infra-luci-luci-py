...
def get_swarming_bot_zip(host):
  ...
  # TODO(crbug.com/1087981): Compare the bot config revision and cached revision.
...
def cache_swarming_bot_zip(version, content, bot_config_rev):
  ...
  logging.info(
      'bot %s with sig %s with bot_config rev: %s saved in memcached in %d chunks',
      version, h.hexdigest(), bot_config_rev, p)
