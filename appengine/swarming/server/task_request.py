...
def cron_delete_old_task_requests():
  ...
  try:
    # Key ordering is naturally by most recent first. We want the reverse,
    # delete the oldest first. That would require ordering by -TaskRequest.key,
    # which would require a new composite index. We don't want that.
    # So instead use .created_ts directly, which is ordered by what is needed.
