...
def _validate_env_key(prop, value):
  maxlen = 1024
  if not value:
    raise datastore_errors.BadValueError(
  ...
  if len(value) > maxlen:
    raise datastore_errors.BadValueError(
        'key in %s is too long: %d > %d' % (prop._name, len(value), maxlen))
  if not _ENV_KEY_RE.match(value):
    raise datastore_errors.BadValueError(
        'key in %s is invalid: %r' % (prop._name, value))
...
def _validate_env_prefixes(prop, value):
  for k, v in value.iteritems():
    if not isinstance(k, unicode):
      raise TypeError(
          '%s must have string key, not %r' % (prop._name, k))
