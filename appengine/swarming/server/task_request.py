...
class TaskRequest(ndb.Model):
  ...
  # user request.
  bot_ping_tolerance_sec = ndb.IntegerProperty(default=600,
...
def validate_ping_tolerance(value):
  ...
  if value > MAX_TOLERANCE_SECS or value < MIN_TOLERANCE_SECS:
    raise datastore_errors.BadValueError('bot_ping_tolerance_sec must be between'
                                         '%s and %s' % (MIN_TOLERANCE_SECS,
                                                        MAX_TOLERANCE_SECS))
