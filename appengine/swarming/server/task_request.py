...
def _validate_dimensions(_prop, value):
  ...
  for k, values in value.iteritems():
    # Check type, which is more an internal check, the API should enforce this.
    ...
    if u':' in k:
      raise datastore_errors.BadValueError(
          u'dimension key %r cannot contain ":"' % k)
    if k.strip() != k:
      raise datastore_errors.BadValueError(
          u'dimension key %r has whitespace' % k)
    if len(k) > maxlen:
      raise datastore_errors.BadValueError(
          u'dimension key %r is too long; maximum is %d' % (k, maxlen))
    ...
    for v in values:
      if not v:
        raise datastore_errors.BadValueError(
            u'dimension key %r has empty value' % k)
      if len(v) > maxlen:
        raise datastore_errors.BadValueError(
            u'dimension key %r has value %r that is is too long; maximum is %d'
            % (k, v, maxlen))
      if v.strip() != v:
        raise datastore_errors.BadValueError(
            u'dimension key %r has value %r with whitespace' % (k, v))
