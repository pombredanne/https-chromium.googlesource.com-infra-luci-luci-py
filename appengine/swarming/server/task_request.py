...
def init_new_request(request, allow_high_priority, secret_bytes_ent):
  ...
  request.service_account_token = None
  if (request.service_account not in ('none', 'bot') and
      not service_accounts.is_service_account(request.service_account)):
    raise ValueError(
        '"service_account" must be an email, "bot" or "none" string, got %r' %
