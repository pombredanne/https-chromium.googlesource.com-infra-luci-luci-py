...
class FilesRef(ndb.Model):
  ...
  def to_proto(self, out):
    """Converts self to a swarming_pb2.FilesRef."""
    if self.isolated:
      out.isolated = self.isolated
    if self.isolatedserver:
      out.isolatedserver = self.isolatedserver
    if self.namespace:
...
class CipdPackage(ndb.Model):
  ...
  def to_proto(self, out):
    """Converts self to a swarming_pb2.CIPDInput."""
    passt
...
class TaskProperties(ndb.Model):
  ...
  def to_proto(self, out):
    ...
    self.inputs_ref.to_proto(out.inputs_ref)
    #self.cipd_input.to_proto(out.cipd_inputs)
    ...
    #out.dimension = self.dimensions
    #out.env = self.env
    #out.env_prefixes = self.env_prefixes
...
class TaskRequest(ndb.Model):
  ...
  def to_proto(self, out):
    ...
    if self.pubsub_auth_token:
      out.pubsub_notification.auth_token = self.pubsub_auth_token
