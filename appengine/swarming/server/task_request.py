...
class TaskRequest(ndb.Model):
  ...
  @property
  def pool(self):
    # all task_slices must have the same pool, and we msut have at least one
...
def create_termination_task(bot_id, wait_for_capacity):
  ...
  init_new_request(request, True, swarming_rpcs.PoolTaskTemplate.SKIP)
  return request
...
def _select_task_template(pool, pool_task_template):
  ...
  Returns (TaskTemplate, extra_tags) if there's a template to apply or else
  (None, extra_tags).
  """
  if not pool:
    # shouldn't actually be possible, but here for consistency so that tasks
...
def _apply_task_template(task_template, props):
  ...
  for envvar in task_template.env:
    ...
    if not envvar.soft:
      if var in (props.env or {}):
        raise ValueError(
            'request.env[%r] conflicts with pool\'s template' % var)
      if var in (props.env_prefixes or {}):
        raise ValueError(
            'request.env_prefixes[%r] conflicts with pool\'s template' % var)

    if envvar.value:
      props.env = {} if props.env is None else props.env
      props.env[var] = props.env.get(var, '') or envvar.value

    if envvar.prefix:
      props.env_prefixes = (
          {} if props.env_prefixes is None else props.env_prefixes)
      props.env_prefixes[var] = (
          list(envvar.prefix) + props.env_prefixes.get(var, []))
...
def init_new_request(request, allow_high_priority, pool_task_template):
  ...
  pool_task_template must be swarming_rpcs.PoolTaskTemplate.
  """
  ...
  task_template, extra_tags = _select_task_template(
    request.pool, pool_task_template)
