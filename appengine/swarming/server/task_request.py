...
def _validate_dimensions(_prop, value):
  ...
  for k, values in value.iteritems():
    if not k or not values or not isinstance(k, unicode):
      raise datastore_errors.BadValueError(
          'dimensions must be a dict of strings or list of string, not %r' %
          value)
    if isinstance(values, (list, tuple)):
      if not all(isinstance(v, unicode) for v in values):
        raise datastore_errors.BadValueError(
            'dimensions must be a dict of strings or list of string, not %r' %
            value)
      if k in u'id' and len(values) != 1:
        raise datastore_errors.BadValueError(
...
class TaskProperties(ndb.Model):
  ...
  def dimensions(self):
    ...
    # consistent.
    if isinstance(self.dimensions_data.items()[0][1], (list, tuple)):
