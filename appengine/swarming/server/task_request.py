...
# It `projects/{project}/instances/{instance}`
_CAS_INSTANCE_RE = re.compile(r'^projects/(?P<project>[a-z0-9-]+)/'
                              'instances/(?P<instance>[a-z0-9-_]+)$')
...
def _validate_cas_instance(_prop, value):
  ...
  if _CAS_INSTANCE_RE.match(value):
    return value
  raise datastore_errors.BadValueError('invalid cas_instance')
