...
class TaskSchedulerApiTest(test_env_handlers.AppTestBase):
  ...
  def test_bot_reap_task_es_with_nonpending_task(self):
    ...
    calls = []
    self.mock(external_scheduler, 'notify_requests', lambda *_: calls.append(True))

    # It should be notified to external scheduler, but exception won't be raised,
  ...
  def test_cron_handle_bot_died_second(self):
    ...
    now_2 = self.mock_now(
        self.now +
        2 * datetime.timedelta(seconds=_request.bot_ping_tolerance_secs), 3)
    self.assertEqual((['1d69b9f088008912'], 0, 0),
  ...
  def test_ensure_active_slice_nonpending(self):
    ...
    self.assertEqual(task_scheduler._ensure_active_slice(r.request, 1, 1), (None, False))
