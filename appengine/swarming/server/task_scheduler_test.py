...
class TaskSchedulerApiTest(test_env_handlers.AppTestBase):
  ...
  def _quick_schedule(self, num_task, **kwargs):
    ...
    result_summary = task_scheduler.schedule_request(request, False)
  ...
  def test_schedule_request_resultdb(self):
    ...
    with mock.patch(
      ...
      result_summary = task_scheduler.schedule_request(request, True)
  ...
  def test_bot_update_child_with_cancelled_parent(self):
    ...
    child_result_summary = task_scheduler.schedule_request(child_request, False)
    self.assertEqual(0, self.execute_tasks())
