...
class TaskSchedulerApiTest(test_env_handlers.AppTestBase):
  ...
  def test_expire_slice(self):
    ...
    self.assertEqual(len(enqueued), 1)
    task_to_run = enqueued[0][1]
    self.assertEqual(task_to_run.task_slice_index, 0)
    self.assertEqual(task_to_run.dimensions, {
        u'pool': [u'default'],
        u'prop': [u'a']
    })
    del enqueued[:]
    ...
    task_scheduler.expire_slice(task_to_run.key, task_result.State.NO_RESOURCE)
    ...
    self.assertEqual(len(enqueued), 1)
    task_to_run = enqueued[0][1]
    self.assertEqual(task_to_run.task_slice_index, 1)
    self.assertEqual(task_to_run.dimensions, {
        u'pool': [u'default'],
        u'prop': [u'b']
    })
