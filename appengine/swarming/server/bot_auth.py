...
def _get_bot_group_config(bot_id):
  ...
  if cfg:
    logging.debug('Found a bot group cfg for bot_id=%s.\n'
                  'hostname=%s will be used for authentication',
                  bot_id, hostname)
    return hostname, cfg
  ...
  if cfg:
    ...
    logging.debug('Found a bot group cfg for bot_id=%s using hostname=%s\n'
                  'hostname will be used for authentication',
                  bot_id, hostname)
    return hostname, cfg

  logging.error(
      'bot_auth: unknown bot_id, not in the bots.cfg'
