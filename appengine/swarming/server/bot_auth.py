...
def validate_bot_id_and_fetch_config(bot_id, machine_type):
  ...
  public_errs = []
  internal_details = []
  for idx, bot_auth in enumerate(cfg.auth):
    err, details = _check_bot_auth(bot_auth, bot_id, peer_ident, ip)
    if not err:
      logging.debug('Auth method #%d succeeded', idx)
      if internal_details:
        logging.debug('Errors from unsuccessful auth methods:')
  ...
  # In most cases there's only one method, so we can simplify the error message.
  if len(public_errs) == 1:
    for msg in internal_details:
      logging.error('%s', msg)
    raise auth.AuthorizationError(public_errs[0])

  # Otherwise let the callers know errors from each failed method.
  logging.error('All %d auth methods failed:', len(cfg.auth))
  for msg in internal_details:
    logging.error('%s', msg)
  raise auth.AuthorizationError(
      'All %d auth methods failed: %s' %
      (len(cfg.auth), '; '.join(public_errs)))
