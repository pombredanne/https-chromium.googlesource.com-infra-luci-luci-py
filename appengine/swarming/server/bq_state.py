...
def _send_to_bq_raw(dataset, table_name, rows):
  ...
  Arguments:
    ...
    rows: list of (bq_key_id, row) rows to sent to BQ.
...
def _send_to_bq_old(dataset, table_name, rows):
  """Sends the rows to BigQuery.
  ...
  try:
    ...
    while utils.utcnow() < should_stop:
      ...
      if rows:
        ...
        state.failed_db_keys, state.failed_bq_keys = _send_to_bq_old(
