...
def get_task_account_token(task_id, bot_id, scopes):
  ...
  if task_request.realm:
    # import pools_config and realms inline to avoid circular imports
    from server import pools_config
    from server import realms
    # If TaskRequest.realm is set, it calls MintServiceAccountToken to grab
    # a OAuth token.
    # It also re-checks if the service account is still allowed to run in the
    # realm. because it may have changed since the last check.
...
def _mint_service_account_token(service_account, realm, oauth_scopes,
  ...
  Raises:
    ...
    InternalError if the RPC fails unexpectedly.
  """
  # extract LUCI proejct from realm '<project>:<realm>'.
  if not ':' in realm:
      raise MisconfigurationError(
          "Realm doesn't start with '<project>:'. realm: %s", realm)
