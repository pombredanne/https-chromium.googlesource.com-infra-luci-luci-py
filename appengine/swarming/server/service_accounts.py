...
def has_token_server():
  """Returns False if Token Server URL is not configured.
...
def get_oauth_token_grant(service_account, validity_duration):
  ...
  approves the usage of <service_account> by the <current_user>, and this
  assertion is valid for <validity_duration>".
  ...
  This function must not be used if 'has_token_server()' returns False.

  This grant is later passed back to the token server to generate an actual
  OAuth access token. When this happens, the token server rechecks the ACLs, so
  it's fine to have large 'validity_duration' here.
  ...
  Returns:
    Base64-encoded string with the token body.
  ...
  # Does the cached token lives long enough to be useful for the caller?
  ...
  # that the cached one expires just a few moments ago. With 1h extra lifetime
  # we effective cache the token for 1h.
  #
  ...
  if new_exp_ts < now + validity_duration:
    _log_token_grant('Got unexpectedly short-lived', new_grant, new_exp_ts)
...
def _mint_oauth_token_grant(service_account, end_user, validity_duration):
  ...
  assert ts_url.startswith('https://'), ts_url
  ...
  logging.info('Got a reply from token server %s', service_version)
  raise grant_token, expiry


def _common_audit_tags():
  """Returns a list of tags that describe circumstances of RPC call.
...
def _log_token_grant(prefix, token, exp_ts):
