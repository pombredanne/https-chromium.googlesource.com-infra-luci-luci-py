...
class BqState(ndb.Model):
  ...
  # db_key of the rows previously uploaded that had failed and should be
  # retried.
...
def _send_to_bq(table_name, rows):
  ...
  for err in res.get('insertErrors', []):
    db_key, _bq_key, _row = rows[err['index']]
  ...
  try:
    ...
    while utils.utcnow() < should_stop:
      ...
      if state.failed_db_keys:
        backlog = fetch_rows(state.failed_db_keys)
        logging.info(
      ...
      if rows:
        ...
        state.failed_db_keys = _send_to_bq(table_name, rows)
        if state.failed_db_keys:
