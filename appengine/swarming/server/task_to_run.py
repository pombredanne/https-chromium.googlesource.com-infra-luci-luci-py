...
class _TaskToRunBase(ndb.Model):
  ...
  though its slices. Each TaskToRunShard is eventually either picked up by a bot
  for execution or expires. TaskToRunShard picked up for execution each have an
  associated task_result.TaskResult entity.

  A TaskToRunShard can be in two states:
  - reapable: queue_number and expiration_ts are not None.
  - consumed: queue_number and expiration_ts are both None.

  The entity starts it life in reapable state and then transitions to consumed
  state either by being picked up by a bot for execution or when it expires.
  Consumed state is final.

  This entity must be kept small and contain the minimum data (in particular
  ...
  TODO(vadimsh): Try number is always 1 now. Swarming-level retries have been
  removed. Anything related to try numbers is dead code that needs to be cleaned
  up.
  ...
  # task slices) have created_ts set at the time the new entity is created: when
  # the task is reenqueued.
  ...
  # to be indexed.
  # Reset to None when the TaskToRunShard is consumed or expires.
...
class Claim(object):
  """Represents a claim on a TaskToRunShard, see obtain(...)."""
  ...
  def obtain(cls, to_run_key, duration=15):
    ...
    if it was already claimed by someone else. When claimed, the caller will
    have up to `duration` seconds to do anything they want with TaskToRunShard
    until someone else will be able to claim it. The caller may also release the
    claim sooner by calling `release` on the returned Claim object (or using it
    ...
    Returns:
      Claim if the task was claimed by us or None if by someone else.
    """
  ...
  def release(self):
    ...
    In particular, if the TaskToRunShard reached its final state (and won't be
    mutated anymore), it is fine (and faster) to **not** explicitly release
    the claim and let it expire naturally.

    Note: this is a best-effort and may theoretically release "wrong" claim if
    the memcache was flushed recently, but this is fine, since the entire Claim
    ...
    # claim set by someone else after our claim expires. Note: 2 sec was picked
    # arbitrarily.
