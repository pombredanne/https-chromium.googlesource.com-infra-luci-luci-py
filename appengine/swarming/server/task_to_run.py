...
class _ActiveQuery(object):
  ...
  def _fetch_and_filter(self, cursor):
    ...
    if fetched:
      self._log('got %d items (%d stale, %d negative cache, %d mismatch)',
                len(fetched),
...
def yield_next_available_task_to_dispatch(bot_id, pool, bot_dims_matcher):
  ...
  try:
    for ttr in _yield_potential_tasks(bot_id, pool, stats, bot_dims_matcher):
      ...
      # Try to claim this item. Only one bot will pass this. It then will have
      # ~15s to submit a transaction that assigns the TaskToRun to this bot.
