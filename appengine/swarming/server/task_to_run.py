...
class _PriorityQueue(object):
  ...
  def add_heapified(self, ttr):
    heapq.heappush(self.heap, _TaskToRunItem(ttr))

  def add_unordered(self, ttr):
    self.heap.append(_TaskToRunItem(ttr))

  def heapify(self):
    heapq.heapify(self.heap)

  def pop(self):
    return heapq.heappop(self.heap).ttr
...
class _ActiveQuery(object):
  def __init__(self, query, dim_hash):
    self.iter = _yield_pages_async(query, 10)
    self.future = next(self.iter, None)
  ...
  try:
    ...
    def _poll_queries(queries, use_heap):
      ...
      for q in queries:
        ...
        if runs:
          logging.debug(
              '_yield_potential_tasks(%s): got %d items from queue %d', bot_id,
              len(runs), q.dim_hash)
        ...
        if q.advance():
          ...
          logging.debug('_yield_potential_tasks(%s): queue %d is exhausted',
                        bot_id, q.dim_hash)
    ...
    # into the queue in arbitrary order.
