...
def _get_scheduling_algorithm(dimensions):
  ...
  """
  if (u'pool' in dimensions and
      dimensions[u'pool'] and
      dimensions[u'pool'] is not None and
      dimensions[u'pool'][0] is not None):
    pool = pools_config.get_pool_config(dimensions[u'pool'][0])
    if pool is not None and pool.scheduling_algorithm is not None:
...
def _gen_queue_number(dimensions_hash, timestamp, priority, scheduling_algorithm):
  ...
  elif ((scheduling_algorithm == (pools_pb2.Pool.SchedulingAlgorithm.
                                  Value('SCHEDULING_ALGORITHM_FIFO')))
        or (scheduling_algorithm == (pools_pb2.Pool.SchedulingAlgorithm.
                                     Value('SCHEDULING_ALGORITHM_UNKNOWN')))):
