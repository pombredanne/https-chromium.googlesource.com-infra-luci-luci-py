...
def dimensions_matcher(bot_dimensions):
  ...
  def matcher(request_dimensions):
    ...
    if any(key not in bot_dimensions for key in request_dimensions):
      return False
    return any(
        all(x in bot_flat for x in flat)
        for flat in task_queues.expand_dimensions_to_flats(request_dimensions))
