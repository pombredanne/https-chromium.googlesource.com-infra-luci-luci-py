...
def _gen_queue_number(dimensions_hash, timestamp, priority):
  ...
  - timestamp: datetime.datetime when the TaskRequest was filed in. This value
        is used for LIFO ordering with a 100ms granularity; the year is ignored.
...
def _queue_number_lifo_priority(v):
...
def _queue_number_priority(v):
  ...
  for the details.
  """
  return int(_queue_number_lifo_priority(v) >> 22)
...
def _yield_potential_tasks(bot_id):
  ...
  try:
    ...
    items.sort(key=_queue_number_lifo_priority)
