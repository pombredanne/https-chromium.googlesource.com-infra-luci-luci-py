...
N_SHARDS = 10  # the number of TaskToRunShards
...
def _get_task_to_run_query(dimensions_hash):
  """Returns a ndb.Querys of TaskToRunShard within this dimensions_hash queue.
     During migration, it returns two queries.
  """
  # dimensions_hash should be 32 bits but on AppEngine, which is using 32 bits
  # python, it is silently upgraded to long.
  assert isinstance(dimensions_hash, (int, long)), repr(dimensions_hash)
...
def _yield_potential_tasks(bot_id):
  ...
  start = time.time()
  yielders = []
  index_to_dim_hashes = []  # store future/yieler index to dimension hash.
  for d in dim_hashes:
    for q in _get_task_to_run_query(d):
      yielders.append(_yield_pages_async(q, 10))
      index_to_dim_hashes.append(d)
  ...
  def _to_active_yielders(futs):
    return [index_to_dim_hashes[i] for i, f in enumerate(futs) if f]
...
def request_to_task_to_run_key(request,
  ...
  kind = get_shard_kind(h) if use_shard else TaskToRun
...
def new_task_to_run(request, task_slice_index, use_shard=False):
  ...
  kind = get_shard_kind(h) if use_shard else TaskToRun
  return kind(key=request_to_task_to_run_key(request, 1, task_slice_index),
              created_ts=created,
              queue_number=qn,
              expiration_ts=exp)
