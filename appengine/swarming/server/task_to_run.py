...
class _QueryStats(object):
  ...
  def __str__(self):
    return ('%d total, %d exp, %d cache negative, %d stale, '
            '%d dimensions mismatch, %d ignored') % (
                self.total, self.expired, self.cache, self.stale, self.mismatch,
                self.ignored)
...
def _validate_task_async(bot_dims_matcher, stats, now, to_run):
  ...
  if not bot_dims_matcher(props.dimensions):
    # TODO(vadimsh): Should we get rid of broken TaskToRunShard? Otherwise it
    # will be stuck in the queue until the expiration cron kicks it out, slowing
    # down **all** bot polls that scan this queue.
...
class _ActiveQuery(object):
  ...
  def _fetch_and_filter(self, cursor):
    ...
    # is an optimization to skip claimed entries as fast as possible.
    available = []
    if fresh:
      keys = [_memcache_to_run_key(ttr.key) for ttr in fresh]
      taken = yield _MC_CLIENT.get_multi_async(keys, namespace='task_to_run')
      if not taken:
        self._log('memcache error when checking the negative cache, ignoring')
        available = fresh
      else:
        for ttr, key in zip(fresh, keys):
          if not taken[key]:
            available.append(ttr)
        self._stats.cache += len(fresh) - len(available)

    if fetched:
      self._log('fetched %d items, %d stale, %d negative cache', len(fetched),
                len(fetched) - len(fresh),
                len(available) - len(fresh))
    if not more:
      self._log('exhausted')
    raise ndb.Return((available, cursor, more))

  def _log(self, msg, *args):
    logging.debug('_yield_potential_tasks(%s): queue %d: %s', self._bot_id,
                  self._dim_hash, msg % args)
