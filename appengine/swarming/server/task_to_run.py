...
def _yield_potential_tasks(bot_id):
  ...
  Yields:
    TaskToRunShard entities, trying to yield the highest priority one first. To have
  ...
  try:
    ...
    # items is a list of TaskToRunShard. The entities are needed because property
...
def task_to_run_key_slice_index(to_run_key):
  """Returns the TaskRequest.task_slice() index this TaskToRunShard entity represents
  as pending.
  """
