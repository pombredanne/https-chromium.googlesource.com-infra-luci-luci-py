...
def check_task_get_acl(task_request):
  ...
  'swarming.tasks.get' in the task realm or 'swarming.pools.listTasks' in any
  realms associated with either the task or the bot the task was assigned to.
  ...
  Raises:
    auth.AuthorizationError: if the caller is not allowed.
  """
  if not acl.can_view_task(task_request):
...
def check_task_cancel_acl(task_request):
  ...
  any realms associated with either the task or the bot the task was assigned to.
  ...
  Raises:
    auth.AuthorizationError: if the caller is not allowed.
  """
  if not acl.can_edit_task(task_request):
...
def _bot_pool_realms(bot_id):
  ...
  """
  realms = []
  for p in bot_management.get_latest_pools(bot_id):
...
def _check_task_acl(task_request, task_perm_enum, pool_perm_enum):
  ...
  `pool_perm_enum` permission in any realms associated with either the task pool
  or any of the bot pools of the bot the task was assigned to.

  This idea is that the caller can either "own" the task or "own" the bot it
  was running on.

  If the caller "owns" the task, they will get access through `task_perm_enum`
  in the tasks realm.

  If the caller "owns" the bot, they will get access through `pool_perm_enum` in
  reals assigned to the bot pools. In most cases "bot pools" is just one pool.
  ...
  # Next check the pool permission of the pool in task dimensions. This is also
  # relatively fast, since it hits the local config cache.
  ...
  # dimensions. This is slow, since we need to fetch bot dimensions from
  # datastore. We do it last.
