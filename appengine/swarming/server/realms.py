...
def check_bot_tasks_acl(bot_id):
  ...
  # the caller needs to have any permission of the pools.
  perm = get_permission(realms_pb2.REALM_PERMISSION_POOLS_LIST_TASKS)
  _check_permission(perm, realms)
...
def check_tasks_list_acl(tags):
  ...
  pools = _get_pools_from_dimensions_flat(tags)

  # Pool dimension is required if the caller doesn't have global permission.
  if not pools:
    raise auth.AuthorizationError('Pool tag is missing')
...
def _get_pools_from_dimensions_flat(filters):
  """Gets pools from dimensions_flat."""
  return [f.replace('pool:', '') for f in filters if f.startswith('pool:')]
