...
def check_bot_get_acl(bot_id):
  ...
  If the caller has a global permission,
    It doesn't check realm permission.

  Otherwise,
    It checks realm permissions of the pools that the bot belongs to.
    The caller is expected to have any permissions of the pools.
...
def check_bots_list_acl(dimensions):
  """Checks if the caller is allowed to list or count bots.

  Args:
    dimensions: List of dimensions for filtering.
