...
def _ensure_entities_exist(max_concurrent=50):
  ...
  for machine_type in MachineType.query():
    ...
    cfg = machine_types.pop(machine_type.key.id(), None)
    ...
    cfg_ent = _machine_type_pb2_to_entity(cfg)
    cfg_ent.target_size = machine_type.target_size
    if machine_type != cfg_ent:
      logging.info('Updating MachineType: %s', cfg_ent)
      machine_type = cfg_ent
      put = True
