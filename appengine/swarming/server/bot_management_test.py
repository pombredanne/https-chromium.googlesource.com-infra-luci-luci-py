...
class BotManagementTest(test_case.TestCase):
  ...
  def test_bot_event_update_dimensions(self):
    ...
    bot_info = bot_info_key.get()
    self.assertIsNone(bot_info_key.get())

    # 'request_sleep' initializes BotInfo with given dimensions at first polling.
    _bot_event(
        event_type='request_sleep',
        dimensions={'id': ['id1'], 'os': ['Android'], 'pool': ['default']})
    self.assertEqual(
        bot_info_key.get().dimensions_flat, [u'id:id1', u'os:Android', u'pool:default'])
