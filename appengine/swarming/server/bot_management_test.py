...
class BotManagementTest(test_case.TestCase):
  ...
  def assertEventsAreEqual(self, expected, actual):
    self.assertEqualDicts(expected, actual, ["ts"])

  def assertBotsEqualIgnoringTime(self, expected, actual):
    self.assertEqualDicts(expected, actual,
  ...
  def test_correct_query_ordering(self):
    bot_id = "bot1"
    _bot_event(bot_id=bot_id, event_type="bot_connected")
    _bot_event(bot_id=bot_id, event_type="request_sleep")
    expected = ['request_sleep', 'bot_connected']
    actual = bot_management.get_events_query(bot_id).fetch()
    self.assertEqual(expected, [event.event_type for event in actual])
    self.assertTrue(actual[0].ts > actual[1].ts)
  ...
  def test_bot_event_poll_sleep(self):
    ...
    _bot_event(event_type='request_sleep')
    self.assertEventsAreEqual([],
                              bot_management.get_events_query('id1').fetch())
