...
class TaskResultApiTest(TestCase):
  ...
  def test_to_proto(self):
    ...
    # It cannot be both TIMED_OUT and have deduped_from.
    #run_result.deduped_from = u'123410'
    run_result.state = task_result.State.TIMED_OUT
    ...
    expected.started_time.FromDatetime(
        self.now + datetime.timedelta(seconds=20))
    expected.abandoned_time.FromDatetime(
        self.now + datetime.timedelta(seconds=30))
    expected.completed_time.FromDatetime(
        self.now + datetime.timedelta(seconds=40))
  ...
  def test_cron_send_to_bq_empty(self):
    ...
    self.assertEqual(None, task_result.bq.BqState.get_by_id('task_results'))
  ...
  def test_cron_send_to_bq(self):
    ...
    def json_request(url, method, payload, scopes, deadline):
      ...
      self.assertEqual(task_result.bq.bqh.INSERT_ROWS_SCOPE, scopes)
      self.assertEqual(600, deadline)
      return {'insertErrors': []}
    self.mock(task_result.bq.net, 'json_request', json_request)
    ...
    self.assertEqual(
        expected, task_result.bq.BqState.get_by_id('task_results').to_dict())
