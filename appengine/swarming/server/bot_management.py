...
def get_latest_info(bot_id):
  ...
  bot = bot_management.BotInfo(key=info_key,
...
def get_latest_pools(bot_id):
  """Returns pools the bot belongs to.

  Uses the latest BotInfo falling back to the last recorded event for deleted
  bots that don't have BotInfo. If there's no such bot at all returns an empty
  list.
