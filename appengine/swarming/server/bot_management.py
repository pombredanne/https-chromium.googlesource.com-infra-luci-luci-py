...
def cron_send_to_bq():
  ...
  def _convert(e):
    ...
    e.to_proto(out)
    return (e.key.urlsafe(), e.ts.strftime(fmt), out)
  ...
  def get_oldest_key():
    ...
    return (
      oldest.key.urlsafe(),
      (oldest.ts - datetime.timedelta(seconds=1)).strftime(fmt),
    )

  def get_rows(db_key, bq_key, size):
    """Returns a list of tuple(db_key, bq_key, row)."""
    start = datetime.datetime.strptime(bq_key, fmt)
    return [
      _convert(e) for e in
  ...
  def fetch_rows(db_keys):
    """Returns a list of tuple(db_key, bq_key, row)."""
    logging.info('fetch_rows(%s)', db_keys)
    return [
      _convert(e) for e in ndb.get_multi(ndb.Key(urlsafe=k) for k in db_keys)
      if e
    ]

  return bq.cron_send_to_bq('bot_events', get_oldest_key, get_rows, fetch_rows)
