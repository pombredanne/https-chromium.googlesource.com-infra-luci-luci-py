...
def bot_event(event_type,
  ...
  """Update the state of the bot in the datastore.
  ...
  # events from the same bot, therefore there are no transactions here. In worst
  # case some intermediary state changes won't be properly recorded.
  ...
  # Store the previous composite to detect status change.
  prev_composite = bot_info._calc_composite()

  now = utils.utcnow()

  # `bot_missing` event is created by a server (see cron_update_bot_info), not
  ...
  # Override values in BotInfo if we have an update to them.
  ...
  # If dimensions change, we'd need to record BotEvent with new dimensions.
  dimensions_updated = False
  if (register_dimensions and dimensions_flat
