...
_TaskTemplateDeployment = namedtuple('_TaskTemplateDeployment', [
  ...
  'canary',
  # The chance (float (0, 1.0)) of the time that the canary template should
...
class TaskTemplateDeployment(_TaskTemplateDeployment):
  ...
  def from_pb(cls, ctx, d, template_map):
    if not (0 <= d.canary_chance < 1):
      ctx.error('canary_chance out of range `[0,1)`: %.1f', d.canary_chance)
      raise InvalidTaskTemplateError()

    # We resolve this regardless to catach validation errors that occur in
    # canary templates even when canary_chance == 0
  ...
  class _Intermediate(object):
    ...
    def update(self, other):
      ...
      for entry in other.env:
        cur_pfx = self.env[entry.var][1] if entry.var in self.env else ()
        self.env[entry.var] = (
          entry.value,
          (cur_pfx + entry.prefix),
          entry.soft)
    ...
    def finalize(self, ctx):
      ...
      for i, (name, (var, prefix, _soft)) in enumerate(self.env.iteritems()):
        ident = repr(name.encode('utf-8')) if name else '#%d' % i
...
Env = namedtuple('Env', ['var', 'value', 'prefix', 'soft'])
