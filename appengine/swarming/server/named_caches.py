...
"""Swarming bot named cache management, e.g. list of known named cache and their
state on each bot.
...
def _named_cache_key(pool, named_cache):
  ...
  return ndb.Key(NamedCachePool, pool, NamedCache, named_cache)
...
def task_update_pool(pool):
  ...
  found = {}
  for bot in bot_management.filter_dimensions(q, [pool]):
    state = bot.state
  ...
  for name, sizes in sorted(found.iteritems()):
    ...
    sizes.sort()
    v = sizes[int(len(sizes) * 0.95)]
    size = max(sizes)
    if _update_named_cache(pool, name, size):
      logging.debug('Updated %s for %d', name, size)

  # Delete the old ones.
  exp = utils.now() - datetime.timedelta(days=8)
  ancestor = ndb.Key(NamedCachePool, pool)
  q = NamedCache.query(ancestor=ancestor, NamedCache.ts < exp)
