...
def get_rbe_instance_for_task(task_tags, pool_cfg):
  ...
  if 'rbe:disable' in task_tags:
    return None
  if 'rbe:enforce' in task_tags:
...
def enqueue_rbe_task(task_request, task_to_run):
  ...
  assert task_to_run.rbe_instance == task_request.rbe_instance, to_run
  assert task_to_run.key.parent() == task_request.key
  assert task_to_run.is_reapable, task_to_run

  # "TaskToRunShard9" => 9.
  kind = task_to_run.key.kind()
      ...
      json_format.MessageToDict(
          rbe_pb2.EnqueueRBETask(
              payload=rbe_pb2.TaskPayload(
                  task_request_id=task_request.key.integer_id(),
                  task_to_run_shard=shard,
                  ...
                  debug_info=rbe_pb2.TaskPayload.DebugInfo(
                      ...
                      task_slice=task_to_run.task_slice_index,
                  ),
              ),
