...
def _expire_task_tx(now, request, to_run_key, result_summary_key, capacity,
  ...
  _maybe_taskupdate_notify_via_tq(result_summary, request, es_cfg, True)
  for f in futures:
    f.check_success()
...
def schedule_request(request, secret_bytes):
  ...
  if result_summary.state != task_result.State.PENDING:
    # Warning: this may send the PubSub message before this RPC returns, which
    # may confuse the user. Do not send es_cfg since the task was already
    # processed by the external scheduler if it were the case.
    _maybe_taskupdate_notify_via_tq(result_summary, request, None, False)
