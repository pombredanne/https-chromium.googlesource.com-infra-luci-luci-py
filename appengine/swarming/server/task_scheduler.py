...
def cron_handle_desired_callbacks():
  ...
  for pool in known_pools:
    ...
    for es_cfg in pool_cfg.external_schedulers:
      if not es_cfg.enabled:
        continue
      request_ids = external_scheduler.desired_callbacks(es_cfg)
      if not request_ids:
        continue

      items = []
      for task_id in request_ids:
        # TODO(akeshet/maruel): Will this get the latest result for the
        # given task_id? That's what I want.
        request_key, result_key = task_pack.get_request_and_result_keys(task_id)
        request = request_key.get()
        result = result_key.get()
        items.append((request, result))
      external_scheduler.notify_requests(es_cfq, items, True, False)
