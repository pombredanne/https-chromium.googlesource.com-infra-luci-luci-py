...
def _expire_task_tx(now, request, to_run_key, result_summary_key, capacity,
  ...
  if to_run.expiration_delay == 0:
      logging.warning('_expire_task_tx: slice_expiration_delay <= 0. '
                      'now=%s, expiration_ts=%s', now, to_run.expiration_ts)
  ...
  if not new_to_run:
    ...
    if result_summary.expiration_delay == 0:
        logging.warning('_expire_task_tx: expiration_delay <= 0. '
                        'now=%s, expiration_ts=%s', now, request.expiration_ts)
