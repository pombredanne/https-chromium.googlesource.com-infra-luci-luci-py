    ...
    if es_cfg:
      ...
      if task_id:
        ...
        request_key, result_key = task_pack.get_request_and_result_keys(
            task_id)
        logging.info("Determined request_key, result_key %s, %s",
            request_key, result_key)
        request = request_key.get()
        result_summary = result_key.get()
        # TODO(akeshet): Pull try_number out of... result_summary?
        # I was getting None value when doing so naively.
        # try_number = result_summary.try_number
        try_number = 1
        # TODO(akeshet): Pull slice_number out of assign_tasks response.
        slice_number = 0

        logging.info("Determined try_number, slice_number %s %s", try_number, slice_number)
