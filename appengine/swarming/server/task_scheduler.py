...
def bot_update_task(
  ...
  def run():
    ...
    if run_result.state == task_result.State.CANCELED:
      # The task was canceled while running. Do not update the entity, do not
      # log an error, just return this information to the bot.
...
def cancel_task(request, result_key):
  ...
  Warning: The TaskRunResult.state is immediately set to CANCELED, so there is
           no way to know when the bot is done.
