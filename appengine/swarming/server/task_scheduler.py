  ...
  try:
    for task_id, _try_number, task_slice_index in task_to_runs:
      ...
      to_runs = [
          ...
          # task_to_run.get_task_to_runs() may include expired TaskToRunShards or
          # TaskToRunShards for other silce indexes. _expire_task() will ignore them,
          # but it's better to filter them out here.
          if r.expiration_ts and r.task_slice_index == task_slice_index
      ]
      if len(to_runs) != 1:
        # There should not be multiple TaskToRunShards. But it needs to expire all
        # of them.
