...
def schedule_request(request,
  ...
  - request_id: Optional string. Used to pull TaskRequestID from datastore.
             If task_request_id is not provided, there is no
  ...
  def get_task_result_summary_from_task_id(task_id):
    _, result_summary_key = task_pack.get_request_and_result_keys(task_id)
    return result_summary_key.get()

  if request_id:
    task_req_to_id = task_request.TaskRequestID.create_key(request_id)
    task_exists = task_req_to_id.get()
    if task_exists:
      return get_task_result_summary_from_task_id(task_exists.task_id)
  ...
  def txn():
    ...
    if request_id:
      ...
      task_req_id = task_request.TaskRequestID(request_id=request_id,
