...
def _get_task_from_external_scheduler(es_cfg, bot_dimensions):
  ...
  to_run, raise_excep = _ensure_active_slice(request, try_number, slice_number)
  if not to_run:
...
def _ensure_active_slice(request, try_number, task_slice_index):
  ...
  Returns:
    TaskToRun: A saved TaskToRun instance corresponding to the given request, try_number,
