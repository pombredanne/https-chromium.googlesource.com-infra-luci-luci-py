...
from bb.client import BuildBucketClient
...
def _reap_task(bot_dimensions,
  ...
  def run():
    ...
    if result_summary.state != orig_summary_state:
      buildbucket_update = True
      if build_task and result_summary.state == task_result.State.RUNNING:
        ...
        bb_client = BuildBucketClient(hostname=build_task.buildbucket_host,
                                      secret_bytes=secret_bytes.secret_bytes)
        resp = bb_client.StartBuildTask(req)
        if resp:
      ...
      _maybe_taskupdate_notify_via_tq(result_summary,
                                      ...
                                      es_cfg,
                                      transactional=True,
                                      buildbucket_update=buildbucket_update)
      state_changed = True
