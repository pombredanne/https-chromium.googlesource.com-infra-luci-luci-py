...
def _reap_task(bot_dimensions,
  ...
  # Fetch SecretBytes as well if the slice use secrets.
...
def bot_claim_slice(bot_dimensions, bot_details, to_run_key, claim_id):
  ...
  if not to_run.is_reapable:
    ...
    request_key = to_run.request_key
    run_result_key = task_pack.result_summary_key_to_run_result_key(
        task_pack.request_key_to_result_summary_key(to_run.request_key))
    request, run_result = ndb.get_multi([request_key, run_result_key])
    if not run_result:
