...
def schedule_request(request, secret_bytes):
  ...
  if request.properties.idempotent:
    ...
    if dupe_summary:
      ...
      # Zap irrelevant properties. PerformanceStats is not copied over, since
      # it's not relevant.
      ...
      # and TaskResultSummary. correctly.
      #
...
def cancel_task(request, result_key):
  ...
  # reapable.
  for i in (1, 2):
    # We don't know in advance if it's try number 1 or 2, so do both.
