...
def _reap_task(bot_dimensions, bot_version, to_run_key, request,
  ...
  try:
    run_result, secret_bytes, summary, state_changed = datastore_utils.transaction(
        run, retries=0, deadline=30)
  except datastore_utils.CommitError:
...
def _cancel_task_tx(request, result_summary, kill_running, bot_id, now, es_cfg,
  ...
  Returns:
    tuple(bool, bool, bool, ResultSummary)
    ...
    - True if the task's state has changed and the run result has been committed
      to the datastore
    - result_summary which is the same as the input argument
  """
  was_running = result_summary.state == task_result.State.RUNNING
  if not result_summary.can_be_canceled:
    return False, was_running, False, result_summary
  ...
  return True, was_running, state_changed, result_summary
