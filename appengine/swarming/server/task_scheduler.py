...
def _detect_dead_task_async(run_result_key):
  ...
  def run():
    ...
    Returns tuple(ndb.Key, bool, datetime.timedelta, List[str])
      run_result_key if task summary is updated, otherwise None
...
def bot_terminate_task(run_result_key, bot_id, start_time):
  ...
  def run():
    ...
    latency = utils.utcnow() - actual_state_change_time
    ts_mon_metrics.on_dead_task_detection_latency(
        result_summary.tags, round(latency.total_seconds() * 1000), False)
    return None
...
def cron_handle_bot_died():
  ...
  def _handle_future(f):
    ...
    if state_changed:
      ts_mon_metrics.on_dead_task_detection_latency(
          tags, round(latency.total_seconds() * 1000), True)
