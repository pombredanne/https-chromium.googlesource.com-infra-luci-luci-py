...
def _check_schedule_request_acl_v2(request):
  ...
  if not pool_cfg:
    ...
    # pools.
    logging.warning('Pool %s is not in pools.cfg, using default acl', pool)
    if has_service_account:
      raise auth.AuthorizationError(
          'Can\'t use service account %s on pool %s not defined in pools.cfg' %
          (request.service_account, pool))
    return

  logging.info('Looking at pool %s in pools.cfg at rev %s', pool, pool_cfg.rev)

  # Verify the caller can use the pool at all.
  if not _is_allowed_to_schedule(pool_cfg):
    raise auth.AuthorizationError(
        'User %s is not allowed to schedule tasks in pool %s, see pools.cfg' %
...
def _is_allowed_to_schedule(pool_cfg):
  ...
  logging.info(
      ...
      peer_id.to_bytes(), sorted(token_tags))
  ...
  if cross:
    logging.info(
        ...
        caller_id.to_bytes(), pool_cfg.name, peer_id.to_bytes(), sorted(cross))
    return True

  logging.warning(
      'Expecting any of %s tags, got %s, forbidding the call',
      sorted(trusted_delegatee.required_delegation_tags), sorted(token_tags))
  return False
...
def check_schedule_request_acl(request):
  ...
  try:
    ...
    logging.error('Pool ACL v2 check failed (but ignored): %s', err)
