...
def _expire_task_tx(now, request, to_run_key, result_summary_key, capacity,
  ...
  to_run.expiration_delay = max(0, now - to_run.expiration_ts)
  ...
  if not new_to_run:
    ...
    result_summary.expiration_delay = max(0, now - request.expiration_ts)
...
def _expire_task(to_run_key, request, inline):
  ...
  if summary:
    ...
    ts_mon_metrics.on_task_expired(summary, task_to_run.key.get())
