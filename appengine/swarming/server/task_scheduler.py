...
def _reap_task(bot_dimensions, bot_version, to_run_key, request,
  ...
  def run():
    ...
    # user-provided keep-alive value. This would update after every ping from
    # bot whenever .modified_ts updates.
...
def _handle_dead_bot(run_result_key):
  ...
  def run():
    ...
    if run_result.modified_ts > now - datetime.timedelta(
        seconds=request.bot_ping_tolerance_secs):
