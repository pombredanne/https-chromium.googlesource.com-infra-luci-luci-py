...
def _reap_task(bot_dimensions, bot_version, to_run_key, request,
  ...
  task_to_run.set_lookup_cache(task.key, task_to_run.REAPING)
  now = utils.utcnow()
  ...
  try:
    ...
    task_to_run.set_lookup_cache(task.key, task_to_run.REAPING_FAILED)
  ...
  if run_result:
    ...
    task_to_run.set_lookup_cache(task.key, task_to_run.REAPED)
  else:
    task_to_run.set_lookup_cache(task.key, task_to_run.REAPING_FAILED)
  return run_result, secret_bytes
