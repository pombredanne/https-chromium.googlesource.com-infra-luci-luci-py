...
def _expire_task_tx(now, request, to_run_key, result_summary_key, capacity,
  ...
  if delay <= 0:
    logging.warning(
        '_expire_task_tx: slice_expiration_delay is negative. '
        'run_id=%s, delay_sec=%s, now=%s, expiration_ts=%s',
        to_run.task_id, delay, now, to_run.expiration_ts)
  ...
  if not new_to_run:
    ...
    if delay <= 0:
      logging.warning(
          '_expire_task_tx: expiration_delay is negative.'
          'task_id=%s, delay_sec=%s, now=%s, expiration_ts=%s',
          result_summary.task_id, delay, now, request.expiration_ts)
