...
def _terminate_previous_task(bot_id, previous_task_id):
  ...
  logging.warning(
      'Bot %s attempting to reap task but already running task %s. Will now'
      'terminate this task...', bot_id, previous_task_id)
  _, prev_run_result_key = task_pack.get_request_and_result_keys(
      previous_task_id)
  ...
  if f and f.done():
    try:
      ...
      logging.error('Failed to abandoned task %s. error=%s', previous_task_id,
                    e)
    if state_changed:
...
def bot_reap_task(bot_dimensions, queues, bot_details, deadline,
  ...
  # Bot should terminate its currently running task if its trying to reap a new
  # one
  if previous_task_id:
