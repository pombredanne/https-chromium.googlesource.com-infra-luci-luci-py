...
"""
_BOT_ID_SORTS = {'created_ts', 'started_ts', 'completed_ts'}
...
"""
_DISALLOWED_STATES = {'pending', 'pending_running', 'deduped'}
...
class _TaskResultCommon(ndb.Model):
  ...
  def pending_now(self, now):
    ...
    Returns timedelta(0) for timedelta < 0.
    """
    if self.deduped_from:
      current = now
    else:
      current = self.started_ts or now
...
def get_run_results_query(start, end, sort, state, bot_id):
  ...
  if sort not in _BOT_ID_SORTS:
    raise ValueError('sort must be one of %s' % _BOT_ID_SORTS)

  if state in _DISALLOWED_STATES:
    raise ValueError("invalid state %s for this query" % state)
