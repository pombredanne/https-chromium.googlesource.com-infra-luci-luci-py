...
class _TaskResultCommon(ndb.Model):
  ...
  def to_proto(self, out):
    ...
    # TODO(maruel): I don't think we enforce the same server for all TaskSlice.
    ...
    if self.cipd_pins:
      ...
      for pkg in self.cipd_pins.packages:
        # Do not copy dst_path, it is not necessary.
        dst = out.cipd_pins.packages.add()
        dst.package_name = pkg.package_name
        dst.version = pkg.version
