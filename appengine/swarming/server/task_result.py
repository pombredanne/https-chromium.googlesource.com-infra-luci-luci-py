...
class _TaskResultCommon(ndb.Model):
  ...
  def to_proto(self, out):
    ...
    # TODO(maruel): I don't think we enforce the same CIPD server for all
    # TaskSlice.
...
def _yield_done_tasks(earliest, size_hint):
  ...
  # of higher DB RPC usage than strictly necessay.
  batch_size = max(2, size_hint/4)
  queries = [
      TaskRunResult.query(
          TaskRunResult.completed_ts > earliest).order(
              TaskRunResult.completed_ts).iter(batch_size=batch_size),
      TaskRunResult.query(
          TaskRunResult.abandoned_ts > earliest).order(
              TaskRunResult.abandoned_ts).iter(batch_size=batch_size),
  ]
...
def cron_send_to_bq():
  ...
  def get_oldest_key():
    ...
    oldest = TaskRunResult.query().order(TaskRunResult.modified_ts).get()
    if not oldest:
      return None, None
  ...
  def get_rows(db_key, bq_key, size):
  ...
  def fetch_rows(db_keys):
    """Returns a list of tuple(db_key, bq_key, row)."""
    return [
      _convert(e)
      for e in ndb.get_multi(
          task_pack.unpack_run_result_key(k) for k in db_keys)
      if e
    ]

  return bq.cron_send_to_bq(
