...
def _generate_stats_bots(_start_time, end_time):
  ...
  s = _BotsSnapshot()
  for pool, _bots in pools.iteritems():
...
STATS_HANDLER = stats_framework.StatisticsFramework('stats_bot', _BotsSnapshot,
                                                    _generate_stats_bots)


def cron_generate_stats():
  """Returns the number of minutes processed."""
  # We optimistically hope that DB queries are consistent after 2 minutes. This
  # is not true all the time, but good enough most of the time.
  return STATS_HANDLER.process_next_chunk(2)
