...
class RBETest(test_case.TestCase):
  ...
  def test_get_rbe_instance_for_task(self, uniform):
    ...
    self.assertEqual(call(['rbe:enforce'], self.pool_config('inst', 0)), 'inst')
    self.assertIsNone(call(['rbe:disable'], self.pool_config('inst', 100)))
    self.assertIsNone(
        call(['rbe:enforce', 'rbe:disable'], self.pool_config('inst', 100)))
...
class EnqueueTest(test_case.TestCase):
  ...
  def test_works(self, getrandbits, utcnow, enqueue_task):
    ...
    self.assertEqual(
        args,
        ('/internal/tasks/t/rbe-enqueue/2ed6c6804c8002a10/33', 'rbe-enqueue'))
    self.assertEqual(
        kwargs, {
            'payload': {
                u'body': {
                    u'payload': {
                        u'taskRequestId': u'5848268811752439134',
                        u'taskToRunId': u'33',
                        u'taskToRunShard': u'15',
                        u'debugInfo': {
