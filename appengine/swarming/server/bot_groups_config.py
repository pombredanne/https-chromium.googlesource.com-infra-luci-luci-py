...
class _DigestBuilder(object):
  ...
  def update(self, kind, key, val):
    assert isinstance(kind, str), type(kind)
    assert isinstance(key, basestring), type(key)
    assert isinstance(val, basestring), type(val)
    assert '\x00' not in kind, kind
    assert '\x00' not in key, key
    if isinstance(key, unicode):
      key = key.encode('utf-8')
    if isinstance(val, unicode):
      val = val.encode('utf-8')
    self._h.update('%s\x00%s\x00%d%s\x00' % (kind, key, len(val), val))
...
def _fetch_and_expand_bots_cfg(ctx):
  ...
  for idx, gr in enumerate(cfg.bot_group):
    if gr.bot_config_script_content or not gr.bot_config_script:
      continue
    script_rev, content = config.get_self_config(
        'scripts/' + gr.bot_config_script,
        store_last_good=True)
    if content:
      logging.info(
          'Using bot config script "%s" at rev %s', gr.bot_config_script,
          script_rev)
      gr.bot_config_script_content = content
      digest.update('BOT_CONFIG_SCRIPT_REV', str(idx), script_rev)
    else:
...
class _BotGroupsCache(object):
  ...
  def __init__(self):
    ...
    self.cfg = None  # instance of _BotGroups with the last loaded config
    self.exp = None  # unix timestamp of when it expires
    self.fetcher_thread = None  # a thread that fetches the config now

  def get_cfg_if_fresh(self):
    """Returns cfg if it is still fresh or None if not."""
    # We allow this to be executed outside the lock. 'exp' and 'cfg' may end up
    # not matching each other, but this is fine, since 'cfg' will always be not
    # older than what 'exp' indicates (or None if cleared concurrently). We
    # assume here that when a change to self.cfg field is visible to other
    # threads, all changes to cfg tuple itself are also already visible. This is
    # safe in Python, there's no memory write reordering there.
    exp = self.exp
    cfg = self.cfg
    if cfg and exp is not None and exp > time.time():
      return cfg
    return None

  def set_cfg(self, cfg):
    """Updates cfg and bumps the expiration time."""
    # Called under the lock, but the order is still important, since we read
    # 'cfg' and 'exp' outside the lock in 'get_cfg_if_fresh'.
    self.cfg = cfg
    self.exp = time.time() + _IN_PROCESS_CACHE_EXP_SEC
...
def _fetch_bot_groups():
  ...
  with _cache.lock:
    ...
    # Nothing is know yet? Block everyone (by holding the lock) until we get
    # a result, there's no other choice.
    known_cfg = _cache.cfg
    if not known_cfg:
    ...
    if _cache.fetcher_thread is not None:
      logging.warning(
          ...
          known_cfg.rev, time.time() - _cache.exp)
      return known_cfg
...
def _do_fetch_bot_groups(known_cfg=None):
  ...
  Returns:
    _BotGroups instances (perhaps same as 'known_cfg').
