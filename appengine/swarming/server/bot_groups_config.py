...
def refetch_from_config_service():
  ...
  Returns:
    ...
    None if the config is missing or invalid.
  """
  ctx = validation.Context.logging()
  cfg = _fetch_and_expand_bots_cfg(ctx)
  if ctx.result().has_errors:
    logging.error('Refusing the invalid config, see logs above')
    return None
...
# Bump this to trigger bots.cfg cache refresh, even if the config itself didn't
# change.
_BOT_CFG_CACHE_VER = 'v1'


class BotsCfgHead(ndb.Model):
  """Contains digest of the latest expanded bots.cfg, but not the config itself.

  Root singleton entity with ID == 1.
...
def _fetch_and_expand_bots_cfg(ctx):
  ...
  return ExpandedBotsCfg(cfg, rev, '%s:%s' % (_BOT_CFG_CACHE_VER, rev))


def _get_expanded_bots_cfg(known_digest):
  """Fetches expanded bots.cfg from the datastore cache.

  If the cache is not there (may happen right after deploying the service),
  falls back to fetching the config directly right here. This situation is rare.
  ...
  if not head:
    ...
    # the fetch cron runs. We manually refresh the cache in this case, not
    # waiting for the cron.
    logging.warning('No bots.cfg cached, forcing the refresh')
    expanded = refetch_from_config_service()
    if expanded and expanded.digest == known_digest:
      return False, None
    return True, expanded

  if head.digest == known_digest:
    return False, None
  if head.empty:
    return True, None
...
def _fetch_bot_groups():
  ...
  _, expanded_cfg = _get_expanded_bots_cfg(known_digest='unused yet')
  if not expanded_cfg:
