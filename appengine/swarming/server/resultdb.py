...
def create_invocation_async(task_run_id):
  ...
  response_headers = {}
  _call_resultdb_recorder_api_async(
      'CreateInvocation', {
...
def finalize_invocation_async(task_run_id, interrupted):
  ...
  try:
    _call_resultdb_recorder_api_async('FinalizeInvocation', {
        'name': _get_invocation_id(task_run_id),
        'interrupted': interrupted,
    })
  except net.Error as ex:
    if 'X-Prpc-Grpc-Code' not in ex.headers:
      logging.exception('No X-Prpc-Grpc-Code in response headers: %s',
                        ex.headers)
      return
    ...
    logging.info('Got FAILED_PRECONDITION in response headers')
