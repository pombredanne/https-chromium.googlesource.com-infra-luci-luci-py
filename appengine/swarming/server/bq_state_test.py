...
class BotManagementTest(test_case.TestCase):
  ...
  def test_cron_trigger_tasks(self):
    ...
    expected =  {
      ...
      'last_bq_key': None,
      'last_db_key': None,
      # Values are exclusives; they are the next values to process.
  ...
  def test_cron_trigger_tasks_follow_up(self):
    ...
    # 2020-01-02T03:04:05.678900
    now = datetime.datetime(2020, 1, 2, 3, 4, 5, 678900)
    now_cut = datetime.datetime(*now.timetuple()[:5] + (0,))
    self.mock_now(now)
    bq_state.BqState(
        id='mytable',
        ts=now,
        oldest=now_cut - datetime.timedelta(seconds=600),
        recent=now_cut - datetime.timedelta(seconds=300)).put()
    urls = []
    def enqueue_task(url, task_name):
      urls.append(url)
      self.assertEqual('tqname', task_name)
    ...
    expected =  {
      ...
      'last_bq_key': None,
      'last_db_key': None,
      'oldest': datetime.datetime(2020, 1, 2, 2, 47),
      'recent': datetime.datetime(2020, 1, 2, 3, 2),
      'ts': now,
    }
    self.assertEqual(expected, bq_state.BqState.get_by_id('mytable').to_dict())
    ...
    expected = [
        ...
        '/internal/taskqueue/foo/2020-01-02T02:49',
        '/internal/taskqueue/foo/2020-01-02T02:48',
    ]
    self.assertEqual(expected, urls)
