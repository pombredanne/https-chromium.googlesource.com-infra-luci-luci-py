...
class RealmsTest(test_case.TestCase):
  ...
  def setUp(self):
    ...
    self._auth_mock = mock.patch('components.auth').start()
    self.mock(auth, 'has_permission', self._auth_mock.has_permission)
    self.mock(auth, 'has_permission_dryrun',
              self._auth_mock.has_permission_dryrun)
  ...
  def test_check_pools_create_task_legacy_allowed(self):
    ...
    self._auth_mock.has_permission_dryrun.assert_called_once_with(
  ...
  def test_check_pools_create_task_legacy_allowed_no_pool_realm(self):
    ...
    self.assertIsNone(realms.check_pools_create_task('test_pool'))
    self._auth_mock.has_permission_dryrun.assert_not_called()
  ...
  def test_check_pools_create_task_enforced_allowed(self):
    ...
    self._auth_mock.has_permission.return_value = True
    self.assertIsNone(realms.check_pools_create_task('test_pool'))
    self._auth_mock.has_permission.assert_called_once_with(
        'swarming.pools.createTask', [u'test:pool'])

  def test_check_pools_create_task_enforced_not_allowed(self):
    self._mock_for_check_pools_create_task()
    self._auth_mock.has_permission.return_value = False
    with self.assertRaises(auth.AuthorizationError):
      realms.check_pools_create_task('test_pool')
    self._auth_mock.has_permission.assert_called_once_with(
        'swarming.pools.createTask', [u'test:pool'])
