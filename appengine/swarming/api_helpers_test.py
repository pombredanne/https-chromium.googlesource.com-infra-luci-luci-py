...
class TestCheckIdenticalRequest(test_case.TestCase):
  ...
  def test_cache_hit(self):
    ...
    @api_helpers.check_identical_request('test_request',
                                         'cf60878f-8f2a-4f1e-b1f5-8b5ec88813a9')
    def request(cache):
      if cache:
        return cache
      return func()
  ...
  def test_namespace(self):
    ...
    @api_helpers.check_identical_request('test_request_1', request_uuid)
    def request1(cache):
      if cache:
        return cache
      return func1()

    @api_helpers.check_identical_request('test_request_2', request_uuid)
    def request2(cache):
      if cache:
        return cache
      return func2()

    self.assertEqual(request1(), 'ok')
    func1.assert_called_once()

    # the cache won't hit because this is in a different namespace.
    self.assertEqual(request2(), 'great')
