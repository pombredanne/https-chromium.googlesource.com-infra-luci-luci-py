...
class SwarmingClient(object):
  ...
  def task_collect(self, task_id, timeout=TIMEOUT_SECS, nowait=False):
  ...
  def _run_isolateserver(self, command, args):
    ...
    logging.debug('SwarmingClient._run_isolateserver: executing command. %s', cmd)
    with fs.open(self._rotate_logfile(), 'wb') as f:
      f.write('\nRunning: %s\n' % ' '.join(cmd))
      f.flush()
      p = subprocess42.Popen(
  ...
  def _capture_swarming(self, command, args, stdin):
    ...
    logging.debug('SwarmingClient._capture_swarming: executing command. %s', cmd)
...
class Test(unittest.TestCase):
  ...
  def test_cancel_pending(self):
    ...
    with self._make_wait_task('test_cancel_pending'):
      ...
      actual, actual_files = self.client.task_collect(task_id, nowait=True)
