...
RESULT_HEY2_ISOLATED_OUT = {
  u'isolated': u'0bf6c4f80e18c909a3775c39eef5db74ab74c005',
  u'isolatedserver': u'http://localhost:10050',
  u'namespace': u'default-gzip',
  u'view_url':
    u'http://localhost:10050/browse?namespace=default-gzip'
    '&hash=0bf6c4f80e18c909a3775c39eef5db74ab74c005',
}
...
class Test(unittest.TestCase):
  ...
  def test_isolated_command(self):
    ...
    hello_world = '\n'.join((
        ...
        'with open(os.path.join(sys.argv[1], \'PATH.txt\'), \'wb\') as f:',
        '  f.write(os.path.realpath(path[0]).replace(cwd, "$CWD"))',
    ))
    ...
    expected_summary = self.gen_expected(
        ...
        performance_stats={
          ...
          u'isolated_upload': {
            u'items_cold': [3, 4, 15, 266],
            u'items_hot': [],
            u'num_items_cold': u'4',
            u'total_bytes_items_cold': u'288',
          },
        },
        outputs=[u'hi\n'],
    ...
    expected_files = {
      ...
      os.path.join('0', 'PATH.txt'): '$CWD/local/path',
    }
    self._run_isolated(
