...
class Test(unittest.TestCase):
  ...
  def test_cas(self):
    ...
    digest, _ = self._archive(name,
                              ...
                              DEFAULT_ISOLATE_HELLO,
                              use_cas=True)
    items_in = [sum(len(c) for c in content.values())]
    ...
    _, performance_stats = self._run_with_cas(digest,
                                              name, ['--'] + DEFAULT_COMMAND +
                                              ['${ISOLATED_OUTDIR}'],
                                              expected_summary,
                                              expected_files,
                                              deduped=False)
    items_out = [len('test_isolated')]
  ...
  def _run_with_cas(self, digest, name, args, expected_summary, expected_files,
    ...
    return task_id, performance_stats
  ...
  def _archive(self, name, contents, isolate_content, use_cas=False):
    ...
    isolated_size = None
    if not use_cas:
      isolated_size = fs.stat(isolated_path).st_size
    return hash_or_digest, isolated_size
...
def main():
  ...
  try:
    ...
    bot = start_bot.LocalBot(servers.swarming_server.url,
                             servers.cas_server_address, True, botdir,
                             args.bot_python)
    Test.bot = bot
    bot.start()
    namespace = 'default'
