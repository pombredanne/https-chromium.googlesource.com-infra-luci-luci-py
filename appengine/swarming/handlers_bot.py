...
def send_build_task_update(task, build_id):
  ...
  buildbucket = "cr-buildbucket"
  if "dev" in project_id:
    buildbucket += "-dev"
  topic = 'projects/%s/topics/%s-backend' % (buildbucket, project_id)
...
class BotTaskUpdateHandler(_BotApiHandler):
  ...
  def post(self, task_id=None):
    ...
    if build_task:
      if build_task.task_status != bb_task.status:
        build_task.task_status = bb_task.status
        build_task.put()
        retries = 3
