...
def gen_props_prpc(props):
  ...
  props.cipd_input.packages.extend([
      swarming.CipdPackage(package_name='rm',
                           path='bin',
                           version='git_revision:deadbeef')
  ])
  props.cipd_input.server = 'https://pool.config.cipd.example.com'
  props.command[:] = ['python', '-c', 'print(1)']
  props.containment.containment_type = swarming.ContainmentType.AUTO
  props.dimensions.extend([
      swarming.StringPair(key='os', value='Amiga'),
      swarming.StringPair(key='pool', value='default')
  ])
...
class PrpcTest(test_env_handlers.AppTestBase):
  ...
  def apply_defaults_for_result_summary(self, result):
    ...
    result.bot_dimensions.extend([
        swarming.StringListPair(key='id', value=['bot1']),
        swarming.StringListPair(key='os', value=['Amiga']),
        swarming.StringListPair(key='pool', value=['default'])
    ])
    result.bot_id = 'bot1'
    ...
    result.state = swarming.TaskState.COMPLETED
    result.tags[:] = [
        'a:tag',
        'os:Amiga',
        'pool:default',
  ...
  def apply_defaults_for_run_result(self, result):
    ...
    result.state = swarming.TaskState.RUNNING
...
class BotServicePrpcTest(PrpcTest):
  ...
  def test_bot_get(self):
    ...
    request = swarming.BotRequest(bot_id="bot1", )
    resp = self.post_prpc("GetBot", request)
    actual_info = swarming.BotInfo()
  ...
  def test_get_deleted_bot(self):
    ...
    self.set_as_admin()
    request = swarming.BotRequest(bot_id=bot_id, )
    resp = self.post_prpc("DeleteBot", request)
    expected = swarming.DeleteResponse(deleted=True)
    actual = swarming.DeleteResponse(deleted=True)
  ...
  def test_get_ok_realm(self, api):
    ...
    resp = self.post_prpc(api,
                          swarming.BotRequest(bot_id='id1'),
                          expect_errors=True)
    self.assertEqual(resp.status, '403 Forbidden')
  ...
  def test_get_forbidden(self, api):
    ...
    with mock.patch('server.acl._is_admin', return_value=True):
      self.post_prpc('DeleteBot', swarming.BotRequest(bot_id='id1'))
    resp = self.post_prpc(api,
                          swarming.BotRequest(bot_id='id1'),
                          expect_errors=True)
    self.assertEqual(resp.status, '403 Forbidden')
  ...
  def test_delete_ok(self):
    ...
    # delete the bot
    resp = self.post_prpc('DeleteBot', swarming.BotRequest(bot_id='id1'))
    actual = swarming.DeleteResponse()
    _decode(resp.body, actual)
    self.assertEqual(swarming.DeleteResponse(deleted=True), actual)
  ...
  def test_events(self):
    ...
    self.set_as_privileged_user()
    request = swarming.BotEventsRequest(bot_id="bot1", limit=200)
    ...
    expected = swarming.BotEventsResponse()
    _decode_dict(dict(items=items), expected)
    actual = swarming.BotEventsResponse()
    _decode(resp.body, actual)
  ...
  def test_terminate_admin(self):
    ...
    actual = swarming.TerminateResponse()
    _decode(resp.body, actual)
    expected = swarming.TerminateResponse(task_id='5cee488008810')
    self.assertEqual(expected, actual)
  ...
  def test_tasks_ok(self):
    ...
    self.set_as_privileged_user()
    request = swarming.BotTasksRequest()
    request.bot_id = 'bot1'
    request.start.FromDatetime(start)
    request.end.FromDatetime(end)
    request.sort = swarming.QUERY_CREATED_TS
    request.state = swarming.QUERY_ALL
    request.include_performance_stats = True
    request.limit = 100
    response = self.post_prpc('ListBotTasks', request)
    actual = swarming.TaskListResponse()
    _decode(response.body, actual)

    expected = swarming.TaskResultResponse()
    ...
    expected.state = swarming.TaskState.COMPLETED
    ...
    for sort in (swarming.QUERY_COMPLETED_TS, swarming.QUERY_STARTED_TS):
      request = swarming.BotTasksRequest()
      request.bot_id = 'bot1'
      request.sort = sort
      request.state = swarming.QUERY_ALL
...
class TaskServicePrpcTest(PrpcTest):
  ...
  def test_result_unknown(self):
    ...
    resp = self.post_prpc(
        'GetResult',
        swarming.TaskIdWithPerfRequest(task_id='12310'),
        expect_errors=True)
    self.assertEqual(resp.status, '404 Not Found')
  ...
  def test_result_long(self):
    ...
    resp = self.post_prpc(
        'GetResult',
        swarming.TaskIdWithPerfRequest(task_id='12310' * 10),
        expect_errors=True)
  ...
  def test_result_ok(self):
    ...
    response = self.post_prpc(
        'GetResult', swarming.TaskIdWithPerfRequest(task_id=task_id))
    expected = swarming.TaskResultResponse()
    ...
    expected.state = swarming.TaskState.PENDING
    expected.tags[:] = [
        u'a:tag',
        u'authenticated:user:user@example.com',
        u'os:Amiga',
    ...
    actual = swarming.TaskResultResponse()
    _decode(response.body, actual)
    self.assertEqual(expected, actual)
    ...
    response = self.post_prpc(
        'GetResult',
        swarming.TaskIdWithPerfRequest(task_id=run_id),
        expect_errors=True)
    ...
    self.set_as_user()
    response = self.post_prpc(
        'GetResult', swarming.TaskIdWithPerfRequest(task_id=run_id))
    expected = swarming.TaskResultResponse()
    self.apply_defaults_for_run_result(expected)
  ...
  def test_result_completed_task(self):
    ...
    response = self.post_prpc(
        'GetResult',
        swarming.TaskIdWithPerfRequest(task_id=task_id,
    ...
    expected.created_ts.FromDatetime(self.now)
    expected.started_ts.FromDatetime(self.now)
    expected.state = swarming.COMPLETED
    expected.costs_usd[:] = [0.1]
  ...
  def test_request_unknown(self):
    ...
    response = self.post_prpc('GetRequest',
                              swarming.TaskIdRequest(task_id='12310'),
                              expect_errors=True)
    self.assertEqual(response.status, '404 Not Found')
  ...
  def test_request_ok(self):
    ...
    expected_props = swarming.TaskProperties()
    gen_props_prpc(expected_props)
    expected_props.secret_bytes = b'<REDACTED>'
    expected_props.command[:] = ['python', 'run_test.py']
    expected = swarming.TaskRequestResponse(properties=expected_props,
                                            task_slices=[
                                                swarming.TaskSlice(
    ...
    response = self.post_prpc('GetRequest',
                              swarming.TaskIdRequest(task_id=task_id))
    actual = swarming.TaskRequestResponse()
    _decode(response.body, actual)
    self.assertEqual(expected, actual)
  ...
  def test_cancel_pending(self):
    ...
    expected = swarming.CancelResponse(canceled=True, was_running=False)
    response = self.post_prpc(
        'CancelTask',
        swarming.TaskCancelRequest(task_id=task_id, kill_running=False))
    actual = swarming.CancelResponse()
    _decode(response.body, actual)
    self.assertEqual(expected, actual)
  ...
  def test_cancel_forbidden(self):
    ...
    response = self.post_prpc('CancelTask',
                              swarming.TaskCancelRequest(task_id=task_id,
                                                         kill_running=False),
                              expect_errors=True)
  ...
  def test_cancel_with_realm_permission(self):
    ...
    self.post_prpc(
        'CancelTask',
        swarming.TaskCancelRequest(task_id=task_id_with_realm,
                                   kill_running=False))
    self.post_prpc(
        'CancelTask',
        swarming.TaskCancelRequest(task_id=task_id_without_realm,
                                   kill_running=False))
  ...
  def test_cancel_running(self):
    ...
    expected.modified_ts.FromDatetime(self.now)
    expected.started_ts.FromDatetime(self.now)
    expected.state = swarming.TaskState.RUNNING
    expected.tags[:] = [
    ...
    actual = swarming.CancelResponse()
    _decode(response.body, actual)
    self.assertEqual(swarming.CancelResponse(canceled=False, was_running=True),
                     actual)
    ...
    actual = swarming.CancelResponse()
    _decode(response.body, actual)
    self.assertEqual(swarming.CancelResponse(canceled=True, was_running=True),
                     actual)
    ...
    expected.modified_ts.FromDatetime(self.now)
    expected.started_ts.FromDatetime(self.now)
    expected.state = swarming.TaskState.KILLED
    expected.tags[:] = [
        u'a:tag',
        u'authenticated:user:user@example.com',
        u'os:Amiga',
  ...
  def test_stdout_ok(self):
    ...
    self.set_as_privileged_user()
    run_id = task_id[:-1] + '1'
    expected = swarming.TaskOutputResponse(output=u'rÉsult string',
                                           state=swarming.TaskState.COMPLETED)
    for i in (task_id, run_id):
      response = self.post_prpc('GetStdout',
                                swarming.TaskIdWithOffsetRequest(task_id=i))
      actual = swarming.TaskOutputResponse()
      _decode(response.body, actual)
      self.assertEqual(expected, actual)
    ...
    response = self.post_prpc(
        'GetStdout',
        swarming.TaskIdWithOffsetRequest(task_id=task_id, offset=1, length=2))
    actual = swarming.TaskOutputResponse()
    _decode(response.body, actual)
    # This is because it's counting in bytes, not in unicode characters:
    expected = swarming.TaskOutputResponse(output=u'É',
                                           state=swarming.TaskState.COMPLETED)
    self.assertEqual(expected, actual)

    response = self.post_prpc(
        'GetStdout',
        swarming.TaskIdWithOffsetRequest(task_id=task_id, offset=3, length=5))
    actual = swarming.TaskOutputResponse()
    _decode(response.body, actual)
    expected = swarming.TaskOutputResponse(output=u'sult ',
                                           state=swarming.TaskState.COMPLETED)
    self.assertEqual(expected, actual)
  ...
  def test_stdout_empty(self):
    ...
    _, task_id = self.client_create_task_raw()
    response = self.post_prpc('GetStdout',
                              swarming.TaskIdWithOffsetRequest(task_id=task_id))
    actual = swarming.TaskOutputResponse()
    _decode(response.body, actual)
    expected = swarming.TaskOutputResponse(state=swarming.TaskState.NO_RESOURCE)
    self.assertEqual(expected, actual)
  ...
  def test_result_run_not_found(self):
    ...
    run_id = task_id[:-1] + '1'
    response = self.post_prpc('GetStdout',
                              swarming.TaskIdWithOffsetRequest(task_id=run_id),
                              expect_errors=True)
    self.assertEqual(response.status, '404 Not Found')
  ...
  def test_task_deduped(self):
    ...
    response = self.post_prpc(
        'GetStdout', swarming.TaskIdWithOffsetRequest(task_id=task_id_2))
    actual = swarming.TaskOutputResponse()
    _decode(response.body, actual)
    expected = swarming.TaskOutputResponse(output=u'rÉsult string',
                                           state=swarming.TaskState.COMPLETED)
  ...
  def test_get_with_realm_permission(self, api):
    ...
    def create_request(task_id):
      if api == 'GetRequest':
        return swarming.TaskIdRequest(task_id=task_id)
      if api == 'GetResult':
        return swarming.TaskIdWithPerfRequest(task_id=task_id)
      if api == 'GetStdout':
        return swarming.TaskIdWithOffsetRequest(task_id=task_id)
