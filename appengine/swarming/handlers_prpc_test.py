...
class PRPCTest(test_case.TestCase):
  ...
  def _assert_stats(self, resolution, page_size, time, expected):
    self._gen_stats()
    msg = swarming_pb2.StatsRequest()
    if time:
      msg.latest.FromDatetime(time)
    msg.resolution = resolution
    msg.page_size = page_size
    raw_resp = self.app.post(
        '/prpc/swarming.Swarming/Stats', _encode(msg), self._headers)
    resp = swarming_pb2.StatsResponse()
  ...
  def test_stats_bad_request_page_size(self):
    msg = swarming_pb2.StatsRequest()
    msg.resolution = swarming_pb2.MINUTE
    raw_resp = self.app.post(
        '/prpc/swarming.Swarming/Stats', _encode(msg), self._headers,
        expect_errors=True)
    self.assertEqual(
  ...
  def test_stats_day(self):
    ...
    expected = (
      ...
      u'  requests: 104\n'
      u'}\n')
    self._assert_stats(swarming_pb2.DAY, 5, None, expected)
  ...
  def test_stats_hour(self):
    ...
    expected = (
      ...
      u'  requests: 19\n'
      u'}\n')
    self._assert_stats(swarming_pb2.HOUR, 20, None, expected)
  ...
  def test_stats_minute(self):
    ...
    self._assert_stats(swarming_pb2.MINUTE, 1, now, expected)
