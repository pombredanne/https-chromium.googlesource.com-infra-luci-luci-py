...
def gen_props_prpc(props):
  ...
  props.cipd_input.packages.extend([
      swarming_api_pb2.CipdPackage(package_name='rm',
                                   path='bin',
                                   version='git_revision:deadbeef')
  ])
  props.cipd_input.server = 'https://pool.config.cipd.example.com'
  props.command[:] = ['python', '-c', 'print(1)']
  props.containment.containment_type = swarming_api_pb2.ContainmentType.AUTO
  props.dimensions.extend([
      swarming_api_pb2.StringPair(key='os', value='Amiga'),
      swarming_api_pb2.StringPair(key='pool', value='default')
  ])
...
class PrpcTest(test_env_handlers.AppTestBase):
  ...
  def apply_defaults_for_result_summary(self, result):
    ...
    result.state = swarming_api_pb2.TaskState.COMPLETED
    result.tags[:] = [
        'a:tag',
        'os:Amiga',
        'pool:default',
...
class TaskServicePrpcTest(PrpcTest):
  ...
  def test_request_unknown(self):
    ...
    response = self.post_prpc('GetRequest',
                              swarming_api_pb2.TaskIdRequest(task_id='12310'),
                              expect_errors=True)
    self.assertEqual(response.status, '404 Not Found')
  ...
  def test_request_ok(self):
    ...
    expected_props = swarming_api_pb2.TaskProperties()
    gen_props_prpc(expected_props)
    expected_props.secret_bytes = b'<REDACTED>'
    expected_props.command[:] = ['python', 'run_test.py']
    expected = swarming_api_pb2.TaskRequestResponse(
        properties=expected_props,
        task_slices=[
            swarming_api_pb2.TaskSlice(expiration_secs=86400,
                                       properties=expected_props,
    ...
    response = self.post_prpc('GetRequest',
                              swarming_api_pb2.TaskIdRequest(task_id=task_id))
    actual = swarming_api_pb2.TaskRequestResponse()
    _decode(response.body, actual)
    self.assertEqual(expected, actual)
  ...
  def test_cancel_pending(self):
    ...
    expected = swarming_api_pb2.CancelResponse(ok=True, was_running=False)
    response = self.post_prpc(
        'CancelTask',
        swarming_api_pb2.TaskCancelRequest(task_id=task_id, kill_running=False))
    actual = swarming_api_pb2.CancelResponse()
    _decode(response.body, actual)
    self.assertEqual(expected, actual)
  ...
  def test_cancel_forbidden(self):
    ...
    response = self.post_prpc('CancelTask',
                              swarming_api_pb2.TaskCancelRequest(
  ...
  def test_cancel_with_realm_permission(self):
    ...
    self.post_prpc(
        'CancelTask',
        swarming_api_pb2.TaskCancelRequest(task_id=task_id_with_realm,
                                           kill_running=False))
    self.post_prpc(
        'CancelTask',
        swarming_api_pb2.TaskCancelRequest(task_id=task_id_without_realm,
                                           kill_running=False))
  ...
  def test_cancel_running(self):
    ...
    expected.modified_ts.FromDatetime(self.now)
    expected.started_ts.FromDatetime(self.now)
    expected.state = swarming_api_pb2.TaskState.RUNNING
    expected.tags[:] = [
    ...
    _decode(response.body, actual)
    self.assertEqual(
        swarming_api_pb2.CancelResponse(ok=False, was_running=True), actual)

    # Works if kill_running == True.
    response = self.post_prpc('CancelTask',
                              swarming_api_pb2.TaskCancelRequest(
                                  task_id=task_id, kill_running=True),
                              expect_errors=True)
    actual = swarming_api_pb2.CancelResponse()
    _decode(response.body, actual)
    self.assertEqual(swarming_api_pb2.CancelResponse(ok=True, was_running=True),
                     actual)
    ...
    expected.modified_ts.FromDatetime(self.now)
    expected.started_ts.FromDatetime(self.now)
    expected.state = swarming_api_pb2.TaskState.KILLED
    expected.tags[:] = [
        u'a:tag',
        u'authenticated:user:user@example.com',
        u'os:Amiga',
  ...
  def test_stdout_ok(self):
    ...
    self.set_as_privileged_user()
    run_id = task_id[:-1] + '1'
    expected = swarming_api_pb2.TaskOutputResponse(
        output=u'rÉsult string', state=swarming_api_pb2.TaskState.COMPLETED)
    for i in (task_id, run_id):
      response = self.post_prpc(
          'GetStdout', swarming_api_pb2.TaskIdWithOffsetRequest(task_id=i))
      actual = swarming_api_pb2.TaskOutputResponse()
      _decode(response.body, actual)
      self.assertEqual(expected, actual)
    ...
    response = self.post_prpc(
        'GetStdout',
        swarming_api_pb2.TaskIdWithOffsetRequest(task_id=task_id,
                                                 offset=1,
                                                 length=2))
    actual = swarming_api_pb2.TaskOutputResponse()
    _decode(response.body, actual)
    ...
    expected = swarming_api_pb2.TaskOutputResponse(
        output=u'É', state=swarming_api_pb2.TaskState.COMPLETED)
    self.assertEqual(expected, actual)
    ...
    _decode(response.body, actual)
    expected = swarming_api_pb2.TaskOutputResponse(
        output=u'sult ', state=swarming_api_pb2.TaskState.COMPLETED)
    self.assertEqual(expected, actual)
  ...
  def test_stdout_empty(self):
    ...
    _, task_id = self.client_create_task_raw()
    response = self.post_prpc(
        'GetStdout', swarming_api_pb2.TaskIdWithOffsetRequest(task_id=task_id))
    actual = swarming_api_pb2.TaskOutputResponse()
    _decode(response.body, actual)
    expected = swarming_api_pb2.TaskOutputResponse(
        state=swarming_api_pb2.TaskState.NO_RESOURCE)
    self.assertEqual(expected, actual)
  ...
  def test_result_run_not_found(self):
    ...
    response = self.post_prpc(
        'GetStdout',
        swarming_api_pb2.TaskIdWithOffsetRequest(task_id=run_id),
        expect_errors=True)
    self.assertEqual(response.status, '404 Not Found')
  ...
  def test_task_deduped(self):
    ...
    response = self.post_prpc(
        'GetStdout',
        swarming_api_pb2.TaskIdWithOffsetRequest(task_id=task_id_2))
    actual = swarming_api_pb2.TaskOutputResponse()
    _decode(response.body, actual)
    expected = swarming_api_pb2.TaskOutputResponse(
        output=u'rÉsult string', state=swarming_api_pb2.TaskState.COMPLETED)
  ...
  def test_get_with_realm_permission(self, api):
    ...
    def create_request(task_id):
      if api == 'GetRequest':
        return swarming_api_pb2.TaskIdRequest(task_id=task_id)
      if api == 'GetResult':
        return swarming_api_pb2.TaskIdWithPerfRequest(task_id=task_id)
      if api == 'GetStdout':
        return swarming_api_pb2.TaskIdWithOffsetRequest(task_id=task_id)
