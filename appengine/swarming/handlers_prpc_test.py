...
class TaskBackendAPIServiceTest(test_env_handlers.AppTestBase):
  ...
  def test_cancel_tasks(self):
    ...
    self.mock_default_pool_acl([])
    utils.enqueue_task = mock.Mock(return_value=True)
    ...
    expected_response = backend_pb2.CancelTasksResponse(tasks=[
      backend_pb2.Task(
            id=backend_pb2.TaskID(target='swarming://', id=first_id),
            status=common_pb2.SUCCESS),
      # Task to cancel this should be enqueued
      backend_pb2.Task(
          id=backend_pb2.TaskID(target='swarming://', id=second_id),
          status=common_pb2.SCHEDULED),
      backend_pb2.Task(
          id=backend_pb2.TaskID(target='swarming://', id='1d69b9f088008810'),
