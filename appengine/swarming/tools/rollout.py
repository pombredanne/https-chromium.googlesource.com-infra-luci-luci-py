...
"""Rolls out new versions of Swarming safely to AppEngine."""
...
group.add_argument('--rollback', default=False, action='store_true')
...
def set_traffic_splits(application, services, splits):
  """
  Change the traffic split on the given AppEngine services.

  This function uses the gcloud CLI tool, described on
  https://cloud.google.com/sdk/gcloud/reference/app/services/set-traffic

  Args:
    application: The name of the AppEngine project.
    services: A list of service names to modify.
    splits: A dict mapping versions to traffic weights.
            (see the --splits flag on the above URL for more detail)

  Raises:
    subprocess.CalledProcessError: The traffic splits failed to be set.
  """
  split = ','.join('{version}={weight}'.format(version, weight)
                   for version, weight in splits.iteritems())
...
def rollforward(args, new_version):
  ...
  current_version, expected_new_version = get_inferred_rollout_state(
      args.application, args.services)

  if expected_new_version is not None and expected_new_version != new_version:
    # Nope, not doing that.
    raise InvalidStateError(
  ...
  set_traffic_splits(args.application,
                     args.services,
                     {current_version: non_canary, new_version: canary})

  # Start measuring the canary soak time from now.
  canary_soak_sleep = sleep(args.canary_seconds)
  next(canary_soak_sleep)

  if not check_health(args, new_version):
  ...
  set_traffic_splits(args.application, args.services, {new_version: 1})
...
def rollback(args, version=None):
  if not version:
    ...
    current_version, new_version = get_inferred_rollout_state(args.application,
                                                              args.services)
    if not new_version:
      print 'No rollback in progress, doing nothing.'
      return
    version = current_version

  # Roll back to the current version.
  set_traffic_splits(args.application, args.services, {version: 1})
