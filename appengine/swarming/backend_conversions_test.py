...
class TestBackendConversions(test_case.TestCase):
  ...
  def test_convert_results_to_tasks(self):
    ...
    expected_tasks = [
        test_task(common_pb2.SCHEDULED, res_sum_1.task_id),
        test_task(
            common_pb2.INFRA_FAILURE, run_res_2.task_id,
            summary='Task expired.', set_timeout=True, set_exhaustion=True),
        test_task(common_pb2.CANCELED, run_res_3.task_id),
        test_task(
            common_pb2.INFRA_FAILURE, summary='Swarming task 4 not found'),
    ]

    self.assertEqual(
        expected_tasks,
        backend_conversions.convert_results_to_tasks(results, [1, 2, 3, 4]))
