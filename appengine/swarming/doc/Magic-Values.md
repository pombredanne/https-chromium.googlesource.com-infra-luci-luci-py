...
*   `quarantined`: if present, independent of its value like `0` or `false`, it
    specifies the bot is self-quarantined. This means that the [self-health
    check](Bot.md#health-self_check) failed and the bot considers is it not in
    good shape to run tasks. This normally means the bot needs manual sysadmin
...
*   `maintenance`: if present, independent of its value like `0` or `false`, is
    similar to `quarantined` except that it means the bot is in temporary and
    expected self-maintenance state and doesn't need to be looked at. An example
    is that puppet is running.
...
*   `id`: **must** be a list of a single value, which also must be unique across
    ...
    important.
*   `pool`: **must** be a list of at least one value.
