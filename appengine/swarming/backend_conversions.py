...
def _compute_task_slices(run_task_req):
  ...
  for cache in run_task_req.caches:
    ...
    if cache.wait_for_warm_caches.seconds:
      dims_by_exp[cache.wait_for_warm_caches.seconds]['caches'].append(
          cache.name)
  ...
  # TODO(crbug/1236848): Add command field.
  # TODO(crbug/1236848): Validate backend_config.fields have the expected
  # value types.
  # NOTES:
  #  - `secret_bytes` moved to bbagent command line
  #  - `env_prefix` handled in bbagent
  base_slice = task_request.TaskSlice(
      wait_for_capacity=run_task_req.backend_config.fields['wait_for_capacity'],
