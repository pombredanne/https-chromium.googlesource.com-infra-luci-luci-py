...
"""Contains model and adaptor functions to transition Auth Service v1
to use permissions.cfg from LUCI Config service, instead of calling
permissions.db(). This is necesaary to deprecate
...
class PermissionsConfigMissingError(Exception):
  """Raised if there is no valid PermissionsConfig entity."""
  pass
...
def as_db():
  ...
  if not e or not isinstance(e.config, config_pb2.PermissionsConfig):
    raise PermissionsConfigMissingError()
  cfg = e.config
