...
def expand_realms(db, project_id, realms_cfg):
  ...
  # Prepopulate `conds_set` with all conditions mentioned in the config to
  ...
  realms_expander = RealmsExpander(roles_expander, conds_set, realms_cfg.realms)
...
class ConditionsSet(object):
  ...
  Also assumes the set of all possible *objects* ever passed indexes(...) was
  also passed to add_condition(...) first (so it could build id => index map).

  This makes hot indexes(...) function fast by allowing to compare ids instead
  of (potentially huge) values.
  """

  def __init__(self):
    # A mapping from realms_pb2.Condition value to its unique index.
    self._normalized = {}
  ...
  def add_condition(self, cond):
    ...
    norm = realms_config_pb2.Condition()
    if cond.HasField('restrict'):
      norm.restrict = realms_config_pb2.Condition.Restrict(
          attribute=cond.attribute,
          values=sorted(set(cond.values)))
    else:
      # Note: this should not be happening, we validated all inputs already.
      raise ValueError('Invalid empty condition %r' % cond)

    # Append it to the list of unique conditions if not already there.
    idx = self._normalized.setdefault(norm, len(self._normalized))
  ...
  def finalize(self):
    ...
    conds = sorted(
        ...
        key=lambda (cond, _): cond.SerializeToString())
    self._normalized = None   # won't need anymore

    # Here `conds` is a list of pairs (cond, its old index). We'll need change
    # self._mapping to use new indexes (matching new order in `conds`). Build
    # the mapping {old index => new index}.
  ...
  def indexes(self, conds):
    """Given a set of realms_config_pb2.Condition, returns an indexes tuple.

    Can be called only after finalize(). All given conditions must have
    previously been but into the set via add_condition(...).

    The returned value is essentially a compact encoding of the overall AND
    condition expression on a binding.
    """
    assert self._finalized
    return tuple(sorted(self._mapping[id(cond)] for cond in conds))


class RealmsExpander(object):
  """Helper to traverse the realm inheritance graph."""

  def __init__(self, roles, conds_set, realms):
    self._roles = roles
...
def to_normalized_bindings(perms_to_principals, index_map):
  ...
  Conditions are specified as indexes in ConditionsSet, we use them as their
  are, since by construction of ConditionsSet, all conditions there are in use
  and we don't need any extra filtering or normalization as we do for
  permissions.
  ...
  return [
      ...
      for perms, conds, principals in sorted(normalized, key=lambda x: x[0])
  ]
