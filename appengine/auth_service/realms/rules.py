...
def expand_realms(db, project_id, realms_cfg):
  ...
  for name in realms_expander.realm_names:
    ...
    for principal, perms in realms_expander.elementary_bindinds(name):
...
class RolesExpander(object):
  ...
  def permissions(self, iterable):
    """Returns a tuple of indexes of given permission strings."""
  ...
  def role(self, role):
    ...
    if role.startswith(permissions.BUILTIN_ROLE_PREFIX):
      perms = self.permissions(self._builtin_roles[role].permissions)
