...
"""Dealing with realms.cfg config files."""
...
# TODO(vadimsh): Move the constants to a more appropriate place when it exists.
ROOT_REALM = '@root'
LEGACY_REALM = '@legacy'
...
def validate_realms_cfg(cfg, ctx):
  """Validates realms.cfg config file using the current version of roles DB."""
...
def validate_custom_roles_list(db, roles, ctx):
  ...
  for i, role in enumerate(roles):
    with ctx.prefix('Custom role #%d ("%s"): ', i+1, role.name):
      ...
      # All referenced permission must be known.
  ...
  for name, role in sorted(graph.items()):
    ...
    cycle = find_shortest_cycle(name, graph)
    if cycle:
      ctx.error(
...
def validate_realm_name(name, ctx):
  ...
  if name == ROOT_REALM or name == LEGACY_REALM:
    return True
  if name.startswith('@'):
    ctx.error(
        'unknown special realm name, only "%s" and "%s" are allowed',
        ROOT_REALM, LEGACY_REALM)
    return False
  if not REALM_NAME_RE.match(name):
    ctx.error('the name must match "%s"', REALM_NAME_RE.pattern)
    return False
  return True
...
def find_shortest_cycle(start, graph):
  """Finds the shortest path from `start` to itself in a directed graph.
