...
class JobQueue(object):
  ...
  def execute(self, txn_sleep_time):
    ...
    This gives the datastore some time to "land" transactions. It's no a
    guarantee, but rather a best effort attempt to avoid contention. If it
    ...
    for idx, job in enumerate(self.jobs):
      if idx:
        time.sleep(txn_sleep_time)  # let the datastore to chill a bit
      try:
        job()
      except (
...
def refetch_config():
  """Called periodically in a cron job to import changes into AuthDB.
  ...
  # If db.permissions has changed, we need to propagate changes into AuthDB.
  ...
  # realms representation in AuthDB as well.
  latest = get_latest_revs_async()
  stored = get_stored_revs_async()
...
def check_permission_changes(db, jobs):
  """Enqueues a job to update permissions list stored in AuthDB, if necessary.
  ...
  In particular it may have entries that are not yet used anywhere. Downstream
  services are still interested in seeing them all (for example, to compare with
  the list of permissions the service is interested in checking).
...
def check_config_changes(db, jobs, latest, stored):
  """Enqueues jobs to update the AuthDB based on detected config changes.

  Args:
    db: a permissions.DB instance with current permissions and roles.
    jobs: a JobQueue instance to enqueue jobs into.
    latest: a list of RealmsCfgRev with all fetched configs.
    stored: a list of RealmsCfgRev representing last applied configs.
  """
  ...
  # List of RealmsCfgRev we'll need to reevaluate due to db.revision changing.
  reeval = []
  ...
  # it may end up being too big, hitting various datastore limits. So we
  # "heuristically" split it into DB_REEVAL_REVISIONS revisions, hoping for
  # the best. Before splitting, shuffle the list to avoid unlucky batching that
...
def get_latest_revs_async():
  """Returns a list of all RealmsCfgRev by querying LUCI Config."""
...
def get_stored_revs_async():
  """Returns a list of all RealmsCfgRev based on data in the datastore."""
  # TODO(vadimsh): Implement via a strongly consistent AuthDB datastore query.
  raise ndb.Return([])


def update_realms(db, revs):
  """Performs an AuthDB transaction that updates realms of a bunch of projects.

  It reevaluates expanded realms form using (potentially) new expansion rules.
