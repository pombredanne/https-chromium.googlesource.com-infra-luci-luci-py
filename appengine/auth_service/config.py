...
def _validate_ip_allowlist_config(conf):
  ...
  for ip_allowlist in conf.ip_allowlists:
    if not model.IP_ALLOWLIST_NAME_RE.match(ip_allowlist.name):
...
def _update_ip_allowlist_config(root, rev, conf):
  ...
  existing_ip_allowlists = {
    e.key.id(): e
    for e in model.AuthIPAllowlist.query(ancestor=model.root_key())
  }
  ...
  ip_allowlist_assignments = (
      model.ip_allowlist_assignments_key().get() or
  ...
  for a in conf.assignments:
    ...
    if key in existing:
      ...
      new_one = model.AuthIPAllowlistAssignments.Assignment(
          identity=model.Identity.from_bytes(a.identity),
          ip_allowlist=a.ip_allowlist_name,
...
_CONFIG_SCHEMAS = {
  ...
  'ip_allowlist.cfg': {
    ...
    'revision_getter': lambda: _get_authdb_config_rev_async('ip_allowlist.cfg'),
