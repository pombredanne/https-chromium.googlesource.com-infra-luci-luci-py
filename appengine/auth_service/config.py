...
def _update_imports_config(root, rev, conf):
...
def _update_authdb_configs(configs):
  ...
  if root.to_dict() != orig:
    logging.info('Global config has been updated')
    root.record_revision(
        modified_by=model.get_service_self_identity(),
        modified_ts=utils.utcnow(),
        comment='Importing configs at rev %s' % rev.revision)
    root.put()
    some_dirty = True
...
def _update_oauth_config(root, rev, conf):
  assert ndb.in_transaction(), 'Must be called in AuthDB transaction'
  assert isinstance(root, model.AuthGlobalConfig), root
  existing_as_dict = {
...
def _update_security_config(root, rev, conf):
  assert ndb.in_transaction(), 'Must be called in AuthDB transaction'
  assert isinstance(root, model.AuthGlobalConfig), root
