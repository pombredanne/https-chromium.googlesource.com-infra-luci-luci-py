...
class ValidationTestCase(test_case.TestCase):
  ...
  def test_validate_acl_cfg(self):
    ...
    '''
    result = validation.validate_config(config.self_config_set(), 'acl.cfg',
                                        cfg)
  ...
  def test_validate_schemas(self):
    ...
    '''
    result = validation.validate_config(config.self_config_set(), 'schemas.cfg',
                                        cfg)
  ...
  def test_validation_by_service_async(self):
    ...
    self.assertEqual(result.messages, [
        validation_context.Message(
            ...
            text=('Error during external validation: invalid response: '
        ...
        validation_context.Message(
            ...
            text=('Error during external validation: invalid response: '
                  ...
                  'path: qux.cfg\n'
                  'response: %r' % res)),
        validation_context.Message(severity=logging.INFO, text='%s'),
    ])

  def test_validate_json_files(self):
    with self.assertRaises(ValueError):
      config.validation.DEFAULT_RULE_SET.validate('services/luci-config',
                                                  'a.json', '[1,]')
