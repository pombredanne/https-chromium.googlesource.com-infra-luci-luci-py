...
def _import_revision(config_set, base_location, commit, update_version=False):
  ...
  if rev_key.get() and not update_version:
  ...
  @ndb.transactional
  def txn():
    if not rev_key.get() or update_version:
...
def _import_config_set(config_set, location):
  ...
  try:
    ...
    update_version = config_set_entity.version < storage.ConfigSet.CUR_VERSION
    if (config_set_entity and config_set_entity.latest_revision == commit.sha
        and not update_version):
      save_attempt(True, 'Up-to-date')
      logging.debug('Config set %s is up-to-date', config_set)
      return

    _import_revision(config_set, location,
                     commit, update_version=update_version)
