...
def _update_import_success_metrics(config_set, commit):
  ...
  currently_imported_version_timestamp.set(
      timestamp,
      fields = {"config_set": config_set}
  )
  last_import_success_timestamp.set(
      int(time.time() * 10**6),
      fields = {"config_set": config_set}
  )
  currently_imported_version.set(
      commit.sha,
      fields = {"config_set": config_set}
  )
...
def import_config_set(config_set):
  last_import_attempt_timestamp.set(
    int(time.time() * 10**6),  # microseconds
    fields = {"config_set": config_set},
  )
