...
class IsolateServiceLogIfPythonClientTest(IsolateServiceTestBase):
  """Test the IsolateService's hanlders can log for python clients."""
  ...
  def mocked_cls():
    ...
    @staticmethod
    def _log(a, m, i):
      IsolateServiceFindPythonClientTest._agent_method_identity = (a, m, i)

    cls._log_agent_method_identity = _log
    return cls

  api_service_cls = mocked_cls()
  ...
  def test_log_agent_method_identity(self):
    ...
    response = self.call_api('preupload', message_to_dict(collection), 200)
    self.assertEqual(
        IsolateServiceFindPythonClientTest._agent_method_identity,
        ('python-requests/4.2', 'preupload', 'user:admin@example.com'))
