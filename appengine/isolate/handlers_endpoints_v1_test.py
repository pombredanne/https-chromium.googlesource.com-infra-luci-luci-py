...
class IsolateServiceTestBase(test_case.EndpointsTestCase):
  ...
  def setUp(self):
    ...
    # reigster all the possible identities being used in the tests.
    all_authed_ids = [
...
class IsolateServiceLogIfPythonClientTest(IsolateServiceTestBase):
  ...
  def test_log_serviceaccount(self, logwarn):
    ...
    auth_testing.mock_get_current_identity(self, admin)
    self._send_request()
    logwarn.assert_called_once_with(
        'Python isolate client: user_agent=python-requests/1.0 method=preupload'
        ' identity=user:admin@appspot.gserviceaccount.com')
