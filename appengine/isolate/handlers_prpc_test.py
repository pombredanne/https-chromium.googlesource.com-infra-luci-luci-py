...
class PRPCTest(test_case.TestCase):
  ...
  def _assert_stats(self, resolution, limit, time, expected):
    self._gen_stats()
  ...
  def test_stats_bad_request_limit(self):
    msg = isolated_pb2.StatsRequest()
    ...
    raw_resp = self.app.post(
        ...
        expect_errors=True)
    self.assertEqual(
        raw_resp.body, 'Invalid limit; must be between 1 and 1000')
  ...
  def test_stats_hour(self):
    # There's less entries than the requested limit.
    expected = (
      u'measurements {\n'
      u'  ts {\n'
