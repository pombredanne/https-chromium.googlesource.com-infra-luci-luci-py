...
def _log_identity_if_is_python_client(func):
  ...
  def wrapped(service, *args, **kwargs):
    ...
    if 'python' in agent and (not identity.is_user or
                              identity.name.endswith('.gserviceaccount.com')):
      service._log_agent_method_identity(agent, func.__name__,
                                         identity.to_bytes())
...
class IsolateService(remote.Service):
  ...
  @staticmethod
  def _log_agent_method_identity(agent, method, identity):
    logging.warn('Python isolate client: user_agent=%s method=%s identity=%s' %
                 (agent, method, identity))
