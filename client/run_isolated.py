...
def map_and_run(data, constant_run_path):
  ...
  try:
    with data.install_packages_fn(run_dir, isolated_client_dir,
                                  cas_client_dir, nsjail_dir) as cipd_info:
      if cipd_info:
...
def install_client_and_packages(run_dir, packages, service_url,
  ...
  Args:
    ...
    nsjail_dir (str): where to download nsjail.
  """
  assert cache_dir
  ...
  with client_manager as client:
    ...
    # Install nsjail on Linux hosts to |nsjail_dir|.
    # TODO(https://crbug.com/1227833): Remove amd64 filter once nsjail builds
    # on arm64.
    if sys.platform == "linux" and cipd.get_platform() == "amd64":
...
def main(args):
  ...
  if options.cipd_enabled:
    ...
    install_packages_fn = (
        lambda run_dir, isolated_dir, cas_dir, nsjail_dir: install_client_and_packages(
            run_dir,
