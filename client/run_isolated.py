...
ISOLATED_REVISION = 'git_revision:8b1bab062e641355d3ee12ba81f74c40111decd7'
...
def upload_out_dir(storage, out_dir, leak_temp_dir, go_isolated_client):
...
def map_and_run(data, constant_run_path):
  ...
  try:
    with data.install_packages_fn(run_dir, isolated_client_dir) as cipd_info:
      ...
      isolated_stats = result['stats'].setdefault('isolated', {})
      # This could use |go_isolated_client|, so make sure it runs when the CIPD
      # package still exists.
      result['outputs_ref'], isolated_stats['upload'] = (
          upload_out_dir(data.storage, out_dir, data.leak_temp_dir,
                         go_isolated_client))
  ...
  # Clean up
    ...
    try:
      ...
      if data.leak_temp_dir:
        ...
        for directory in (run_dir, tmp_dir, isolated_client_dir, out_dir):
