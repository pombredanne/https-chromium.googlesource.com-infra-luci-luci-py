  ...
  # Clean up
    ...
    try:
      ...
      if data.leak_temp_dir:
        ...
        for dir in (run_dir, tmp_dir, isolated_client_dir):
          if not fs.isdir(dir):
            continue
          try:
            success = file_path.rmtree(dir)
          except OSError as e:
            logging.error('rmtree(%r) failed: %s', dir, e)
            success = False
          if not success:
            sys.stderr.write(OUTLIVING_ZOMBIE_MSG % (dir, data.grace_period))
