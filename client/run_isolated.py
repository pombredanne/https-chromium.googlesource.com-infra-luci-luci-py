...
def create_option_parser():
  ...
  group.add_option(
      ...
      default=True,
      help='Whether to use the Go isolated to upload the output to'
      'isolateserver. If true, CIPD must be enabled.')
  parser.add_option_group(group)
...
def parse_args(args):
  ...
  def to_bool(arg):
    if isinstance(arg, (bool, int)):
      return bool(arg)
    return distutils.util.strtobool(arg)

  options.cipd_enabled = to_bool(options.cipd_enabled)
  options.use_go_isolated_to_upload = to_bool(options.use_go_isolated_to_upload)
