...
def trigger_task_shards(swarming, task_request, shards):
  ...
  if len(tasks) != len(requests):
    if tasks:
      print(
          'Only %d shard(s) out of %d were triggered' % (len(tasks),
                                                         len(requests)),
          file=sys.stderr)
...
def decorate_shard_output(swarming, shard_index, metadata, include_stdout):
  ...
  if metadata.get('started_ts') and not metadata.get('deduped_from'):
    pending = '%.1fs' % (parse_time(metadata['started_ts']) - parse_time(
        metadata['created_ts'])).total_seconds()
  elif (metadata.get('state') in ('BOT_DIED', 'CANCELED', 'EXPIRED') and
        metadata.get('abandoned_ts')):
    pending = '%.1fs' % (parse_time(metadata['abandoned_ts']) - parse_time(
        metadata['created_ts'])).total_seconds()
  else:
    pending = 'N/A'
...
def collect(swarming, task_ids, timeout, decorate, print_status_updates,
  ...
  try:
    for index, metadata in yield_results(
      ...
      if decorate:
        ...
        print('%s: %s %s' % (metadata.get('bot_id', 'N/A'), metadata['task_id'],
                             shard_exit_code))
  ...
  if len(seen_shards) != len(task_ids):
    ...
    print(
        'Results from some shards are missing: %s' % ', '.join(
  ...
  def convert_value(self, opt, value):
    if value not in self.choices:
      raise optparse.OptionValueError(
          "%s must be one of %s not %r" % (self.get_opt_string(), self.choices,
                                           value))
    stdout_to = []
    if value == 'all':
...
def CMDrun(parser, args):
  ...
  try:
    ...
    on_error.report('Failed to trigger %s(%s): %s' % (task_request.name,
                                                      args[0], e.args[0]))
    return 1
  if not tasks:
