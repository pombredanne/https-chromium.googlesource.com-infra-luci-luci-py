...
class NamedCache(Cache):
  ...
  def __init__(self, cache_dir, policies, time_fn=None):
    ...
    if policies.__class__ is NamedCachePolicies:
      self._policies = policies
    else:
      self._policies = NamedCachePolicies(
          max_cache_size=policies.max_cache_size,
          min_free_space=policies.min_free_space,
          max_items=policies.max_items,
    ...
    self.state_file = os.path.join(cache_dir, self.STATE_FILE)
    self._lru = lru.LRUDict()
    self._dont_evict = set(self._policies.dont_evict)
    if not fs.isdir(self.cache_dir):
      fs.makedirs(self.cache_dir)
  ...
  def trim(self):
    ...
    def unexcempt_items_remain():
      remaining_items = set(key for key, _ in self._lru.items())
      no_excempt_items = self._dont_evict.issuperset(remaining_items)
      if no_excempt_items:
