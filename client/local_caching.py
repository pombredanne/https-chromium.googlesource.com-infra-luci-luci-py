...
class NamedCache(Cache):
  ...
  def _oldest_evictable_ts(self):
    self._lock.assert_locked()
    oldest = self._oldest_evictable_item()
    # oldest = (name, ts)
    if oldest:
      return oldest[1]
  ...
  def oldest_evictable_item(self):
    with self._lock:
      return self._oldest_evictable_item()

  def oldest_evictable_ts(self):
    with self._lock:
      return self._oldest_evictable_ts()
