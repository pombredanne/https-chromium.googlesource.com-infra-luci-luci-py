...
def _get_recursive_size_with_scandir(path):
  ...
  logging.debug(
      '_get_recursive_size_with_fswalk: traversed %s. files: %d, links: %d, directories: %d',
      path, n_files, n_links, n_dirs)
  return total
...
def _get_recursive_size(path):
  ...
  try:
    if _use_scandir():
      return _get_recursive_size_with_scandir(path)
    else:
      return _get_recursive_size_with_fswalk(path)
