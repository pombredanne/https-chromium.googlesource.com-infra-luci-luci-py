  ...
  def async_fetch(self, channel, priority, digest, size, sink):
    ...
    def fetch():
      try:
        ...
        stream = self._storage_api.fetch(digest, size)
        if self._use_zip:
  ...
  with cache:
    ...
    with tools.Profiler('GetIsolateds'):
      ...
      if not isolated_format.is_valid_hash(isolated_hash, algo):
        logging.debug('%s is not a valid hash, assuming a file (algo was %s, hash size was %d)', isolated_hash, algo(), algo().digest_size)
