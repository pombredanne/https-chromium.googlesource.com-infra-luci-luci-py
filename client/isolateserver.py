...
class Storage(object):
  ...
  def upload_items(self, items):
    ...
    _print_upload_stats(items, uploaded)
    return uploaded
...
def _print_upload_stats(items, missing):
  ...
  logging.info(
      'Total:      %6d, %9.1fkb', total, total_size / 1024.)
...
def _enqueue_dir(dirpath, blacklist, tempdir, hash_algo, hash_algo_name):
  ...
  # Look for optimistic batching. It's a replication of
...
def archive_files_to_storage(storage, files, blacklist):
  ...
  # TODO(maruel): Stop needing a temporary direc.
