...
class LocalAuthServerTest(auto_stub.TestCase):
    ...
    with local_auth_server(token_gen, 'acc_1'):
      ...
      self.assertEqual({
          'accounts': [
              ...
              {
                  'email': 'acc_2@example.com',
                  'id': 'acc_2'
              },
              {
                  'email': 'acc_3@example.com',
                  'id': 'acc_3'
              },
          ],
          'default_account_id':
      ...
      self.assertEqual({
          u'access_token': u'tok_acc_1',
          u'expiry': self.epoch + 300
      }, resp)
      ...
      self.assertEqual({
          u'access_token': u'tok_acc_2',
          u'expiry': self.epoch + 360
      }, resp)
  ...
  def test_handles_token_errors(self):
    ...
    with local_auth_server(token_gen, 'acc_1'):
      self.assertEqual({
          u'error_code': 123,
          u'error_message': u'error message'
  ...
  def test_validation(self):
    ...
    with local_auth_server(token_gen, 'acc_1'):
      ...
      cases = [
          ...
          ({
              'account_id': 123
          }, '"account_id" must be a string', 400),

          # scopes
          ({
              'account_id': 'acc_1'
          }, '"scopes" is required', 400),
          ({
              'account_id': 'acc_1',
              'scopes': []
          }, '"scopes" is required', 400),
...
class LocalAuthHttpServiceTest(auto_stub.TestCase):
  ...
  def test_works(self):
    ...
    def handle_request(request):
      ...
      self.assertEqual(u'Bearer %s' % token, request.headers['Authorization'])
