...
DISABLE_CIPD_FOR_TESTS = '--no-cipd-enabled'
...
class RunIsolatedTest(unittest.TestCase):
  ...
  def test_minimal_lower_priority(self):
    cmd = [
        DISABLE_CIPD_FOR_TESTS, '--lower-priority', '--raw-cmd', '--',
        sys.executable, '-c'
    ]
    if sys.platform == 'win32':
      cmd.append(
  ...
  def test_named_cache(self):
    ...
    now = time.time() - 2
    cmd = [
        DISABLE_CIPD_FOR_TESTS, '--named-cache-root', self._named_cache_dir,
        '--named-cache', 'cache1', 'a', '100', '--raw-cmd', '--',
        sys.executable, '-c',
