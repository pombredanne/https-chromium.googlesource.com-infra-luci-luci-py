...
class RunIsolatedTest(unittest.TestCase):
  def setUp(self):
    ...
    self._cas_instance = 'projects/chromium-swarm-dev/instances/default_instance'
  ...
  def test_cas(self):
    ...
    client_manager = cipd.get_client(cipd.DEFAULT_CIPD_SERVER,
                                     cipd.DEFAULT_CIPD_CLIENT_PACKAGE,
                                     cipd.DEFAULT_CIPD_CLIENT_VERSION,
                                     self._cipd_cache_dir)
    with client_manager as cipd_client:
      packages = [('', run_isolated.CAS_PACKAGE, run_isolated.CAS_REVISION)]
      ...
      stats_file = os.path.join(inputs_dir, 'archive_stats.json')
      ...
      with open(digest_file) as f:
        cas_digest = f.read()
      expected_files = os.listdir(inputs_root)

    # Runs run_isolated with cas options. It connects to the remote dev instance.
    cmd = [
        '--cas-instance',
        self._cas_instance,
        '--cas-digest',
