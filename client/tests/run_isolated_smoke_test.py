...
class RunIsolatedTest(unittest.TestCase):
  def setUp(self):
    ...
    logging.info("cwd: %s %s %s", os.getcwd(), __file__, sys.argv)
  ...
  def _test_dir(self, dirname):
    ...
    logging.info("dirname %s", __file__)
    logging.info("dirname %s", os.path.abspath(__file__))

    p = os.path.join(test_env.TESTS_DIR, 'data', dirname)
    logging.info("dirname %s", p)
    return p
  ...
  def test_isolated_normal(self):
    ...
    cas_digest = self._upload_to_cas(self._test_dir('repeated_files'))
    expected = [
        'state.json',
        '01d003989115d52bfb87decb2bbff387916aa3e967bc29c94fceff7da631e82f',
