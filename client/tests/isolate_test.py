...
class IsolateLoad(IsolateBase):
  ...
  def test_chromium_split(self):
    ...
    actual_isolated_client = tools.read_json(
        os.path.join(self.isolated_dir, 'foo.isolated'))
    expected_isolated_client = {
        u'algo': u'sha-1',
    ...
    self._cleanup_isolated(expected_isolated_client)
    self.assertEqual(expected_isolated_client, actual_isolated_client)
