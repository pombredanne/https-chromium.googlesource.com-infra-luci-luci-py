...
class FindExecutableTest(auto_stub.TestCase):
  ...
  def test_add_python(self):
    ...
    self.assertEqual([self._abs('SYSTEM', 'python')] + cmd,
                          self._fe(cmd))

  def test_WIN_add_python(self):
    cmd = ['not_a_real_path.py']
    self.assertEqual([self._abs('SYSTEM', 'python.exe')] + cmd,
                          self._fe(cmd))
  ...
  def test_implicit_cwd(self):
    self.assertEqual([self._touch_exe('python')],
                          self._fe(['python']))

  def test_WIN_implicit_cwd(self):
    self.assertEqual([self._touch_exe('python.bat')],
                          self._fe(['python']))

  def test_relative_PATH_entry(self):
    self.PATH.insert(0, 'LOCAL')
    self.assertEqual([self._touch_exe('LOCAL', 'thingy')],
                          self._fe(['thingy']))
  ...
  def test_explicit_relative_missing(self):
    self.assertEqual(['./not_real'], self._fe(['./not_real']))
