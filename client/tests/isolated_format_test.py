...
class SymlinkTest(unittest.TestCase):
  ...
  if sys.platform == 'darwin':
    def test_expand_symlinks_path_case(self):
      ...
      # system.
      fs.symlink('dest', os.path.join(self.cwd, 'link'))
      fs.mkdir(os.path.join(self.cwd, 'Dest'))
      fs.open(os.path.join(self.cwd, 'Dest', 'file.txt'), 'w').close()

      result = isolated_format._expand_symlinks(unicode(self.cwd), 'link')
