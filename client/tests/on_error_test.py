...
class OnErrorServerTest(OnErrorBase):
  ...
  def assertRequestParams(expected, actual):
    # Exclude PATH on Windows because PATH in a child process include
    # duplicated pywin32_system32 paths, which makes assertion fail.
    if sys.platform == 'win32':
      expected['env'].drop('PATH', None)
      actual['env'].drop('PATH', None)
