...
class CacheManagerTest(TestCase):
  def setUp(self):
    ...
    self.policies = local_caching.CachePolicies(
        ...
        max_items=50,
        max_age_secs=21*24*60*60)
    self.root_dir = os.path.join(self.tempdir, 'cache')
  ...
  def test_get_oldest(self):
    cache = local_caching.CacheManager(self.root_dir, self.policies)
  ...
  def test_get_timestamp(self):
    now = 0
    time_fn = lambda: now
    cache = local_caching.CacheManager(
        self.root_dir, self.policies, time_fn=time_fn)
    for i in xrange(10):
  ...
  def test_corrupted(self):
    ...
    with open(os.path.join(self.root_dir, u'state.json'), 'w') as f:
      f.write('}}}}')
    fs.makedirs(os.path.join(self.root_dir, 'a'), 0777)
