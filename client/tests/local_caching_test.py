...
class CacheTestMixIn(object):
  ...
  def _add_one_item(self, cache, size):
    """Adds one item of |size| bytes and returns the created name."""
    raise NotImplementedError()
...
class DiskContentAddressedCacheTest(TestCase, ContentAddressedCacheTestMixIn):
  ...
  def test_trim_policies_trim_old(self):
    ...
    self.assertEqual([hashlib.sha1('recent').hexdigest()], list(cache))
...
class FnTest(TestCase):
  ...
  def _prepare_isolated_cache(self, isolated_cache):
    now = self._now
    # Add items to these caches.
    small = '0123456789'
    big = small * 1014
    small_digest = unicode(hashlib.sha1(small).hexdigest())
    big_digest = unicode(hashlib.sha1(big).hexdigest())
    self._now = now + 1
    isolated_cache.write(big_digest, [big])
    self._now = now + 2
    isolated_cache.write(small_digest, [small])
    self.assertEqual([], isolated_cache.trim())

    # Isolated cache verification.
    expected = {
      big_digest: big,
      small_digest: small,
      isolated_cache.STATE_FILE: _gen_state(
          [
            [big_digest, [10140, now+1]],
            [small_digest, [10, now+2]],
          ]),
    }
    self.assertEqual(expected, read_tree(isolated_cache.cache_dir))
    self._now = now

  def _prepare_named_cache(self, named_cache):
    now = self._now
    # Add items to these caches.
    small = '0123456789'
    big = small * 1014
    self._now = now + 1
    self.put_to_named_cache(named_cache, u'first', u'big', big)
    self._now = now + 3
    self.put_to_named_cache(named_cache, u'second', u'small', small)
    self.assertEqual([], named_cache.trim())

    # Named cache verification. Ensures the cache contain the expected data.
    actual = read_tree(named_cache.cache_dir)
    ...
    expected = {
      ...
      named_cache.STATE_FILE: _gen_state(
          [
  ...
  def test_clean_caches_disk(self):
    ...
    # Setup caches.
    policies = _get_policies(min_free_space=10240)
    named_cache = local_caching.NamedCache(
        os.path.join(self.tempdir, 'named_cache'), policies)
    cache_small = self._prepare_named_cache(named_cache)

    isolated_cache = local_caching.DiskContentAddressedCache(
        os.path.join(self.tempdir, 'isolated_cache'), policies, hashlib.sha1,
        trim=False)
    self._prepare_isolated_cache(isolated_cache)

    # Request triming.
    self._free_disk = 1020
    actual = local_caching.trim_caches(
        [isolated_cache, named_cache],
        self.tempdir,
        min_free_space=policies.min_free_space,
        max_age_secs=policies.max_age_secs)
    self.assertEqual([10140, 10140, 10, 10], actual)
    ...
    self.assertEqual(expected, read_tree(named_cache.cache_dir))
  ...
  def test_clean_caches_memory(self):
    caches = [
    ...
    for size in xrange(100):
      data = (string.digits*((size+9)/10))[:size]
      i = random.randint(0, len(caches)-1)
      caches[i].write(hashlib.sha1(data).hexdigest(), [data])
      self._now += 1
    ...
    actual = local_caching.trim_caches(
        ...
        min_free_space=1024,
        max_age_secs=100)
    self.assertEqual(range(100), actual)
