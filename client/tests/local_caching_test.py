...
class DiskContentAddressedCacheTest(TestCase, ContentAddressedCacheTestMixin):
  ...
  def test_save_disk(self):
    ...
    with open(os.path.join(cache.cache_dir, cache.STATE_FILE)) as f:
      content = json.load(f)
    self.assertEqual([[h, [2, 1000]]], content['items'])
...
class NamedCacheTest(TestCase, CacheTestMixin):
  ...
  def test_save_named(self):
    ...
    p = content['items'][0][1][0][0]
    self.assertEqual(
        sorted([p, cache.NAMED_DIR, cache.STATE_FILE]),
        sorted(os.listdir(cache.cache_dir)))
...
class FnTest(TestCase):
  """Test functions that leverages both DiskContentAddressedCache and
  NamedCache.
  """
  def setUp(self):
