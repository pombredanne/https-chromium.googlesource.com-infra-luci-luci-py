...
class TraceInputs(unittest.TestCase):
  ...
  def test_process_quoted_arguments(self):
    ...
    for actual, expected in test_cases:
      self.assertEqual(expected,
                       trace_inputs.strace_process_quoted_arguments(actual))
  ...
  def test_variable_abs(self):
    value = trace_inputs.Results.File(None, u'/foo/bar', None, None,
                                      trace_inputs.Results.File.READ)
  ...
  def test_variable_rel(self):
    value = trace_inputs.Results.File(u'/usr', u'foo/bar', None, None,
                                      trace_inputs.Results.File.READ)
    actual = value.replace_variables({'$FOO': 'foo'})
    self.assertEqual('$FOO/bar', actual.path)
  ...
  def test_CsvReader(self):
    test_cases = {
        ...
        u'   Foo,  ""Who the hell thought delimiters are great as escape too""':
            [
                u'Foo',
                u'"Who the hell thought delimiters are great as escape too"'
            ],
        (u'  "remoting.exe", ""C:\\Program Files\\remoting.exe" '
         u'--host="C:\\ProgramData\\host.json""'): [
            u'remoting.exe', u'"C:\\Program Files\\remoting.exe" '
            u'--host="C:\\ProgramData\\host.json"'
        ],
        ...
        u'"cmd.exe", ""C:\\\\Winz\\\\cmd.exe" /k ""C:\\\\MSVS\\\\vc.bat"" x86"':
            [
                u'cmd.exe',
                u'"C:\\\\Winz\\\\cmd.exe" /k "C:\\\\MSVS\\\\vc.bat" x86'
            ],
    }
...
if sys.platform != 'win32':
  ...
  class StraceInputs(unittest.TestCase):
    ...
    def test_execve(self):
      lines = [
          (self._ROOT_PID, 'execve("/home/foo_bar_user/out/unittests", '
    ...
    def test_empty(self):
      try:
        ...
        expected = ('Found internal inconsitency in process lifetime detection '
                    'while finding the root process', None, None, None, None,
                    [])
    ...
    def test_clone(self):
      ...
      lines = [
          (self._ROOT_PID, 'clone(child_stack=0, flags=CLONE_CHILD_CLEARTID'
           '|CLONE_CHILD_SETTID|SIGCHLD, child_tidptr=0x7f5350f829d0) = %d' %
           self._CHILD_PID),
          (self._CHILD_PID, 'clone(child_stack=0, flags=CLONE_CHILD_CLEARTID'
           '|CLONE_CHILD_SETTID|SIGCHLD, child_tidptr=0x7f5350f829d0) = %d' %
    ...
    def test_clone_chdir(self):
      # Grand-child with relative directory.
      lines = [
          (self._ROOT_PID, 'execve("../out/unittests", '
           '["../out/unittests"...], [/* 44 vars */])         = 0'),
          (self._ROOT_PID, 'clone(child_stack=0, flags=CLONE_CHILD_CLEARTID'
           '|CLONE_CHILD_SETTID|SIGCHLD, child_tidptr=0x7f5350f829d0) = %d' %
           self._CHILD_PID),
          (self._CHILD_PID, 'chdir("/home_foo_bar_user/path1") = 0'),
          (self._CHILD_PID, 'clone(child_stack=0, flags=CLONE_CHILD_CLEARTID'
           '|CLONE_CHILD_SETTID|SIGCHLD, child_tidptr=0x7f5350f829d0) = %d' %
           self._GRAND_CHILD_PID),
          (self._GRAND_CHILD_PID, 'execve("../out/unittests", '
           '["../out/unittests"...], [/* 44 vars */])         = 0'),
          (self._ROOT_PID, 'chdir("/home_foo_bar_user/path2") = 0'),
          (self._GRAND_CHILD_PID, 'open("random.txt", O_RDONLY)       = 76'),
      ]
      expected = {
          'root': {
              'children': [
                  {
                      'children': [{
                          ...
                          'executable':
                              '/home_foo_bar_user/out/unittests',
                          'files': [
                              {
                                  'mode': trace_inputs.Results.File.READ,
                              ...
                              },
                              {
                                  'mode':
                                      trace_inputs.Results.File.READ,
                                  'path':
                                      u'/home_foo_bar_user/path1/random.txt',
                                  'size':
                                      -1,
                              },
                          ],
                          'initial_cwd':
                              u'/home_foo_bar_user/path1',
                          'pid':
                              self._GRAND_CHILD_PID,
                      },],
              ...
              'files': [{
                  ...
                  'path':
                      os.path.join(
                          os.path.dirname(test_env.CLIENT_DIR), 'out',
                          'unittests'),
                  'size':
                      -1,
              },],
    ...
    def test_openat(self):
      lines = [
          ...
          (self._ROOT_PID, 'openat(AT_FDCWD, 0xa23f60, '
    ...
    def test_openat_died(self):
      lines = [
          ...
          (self._ROOT_PID, 'openat(AT_FDCWD, "/tmp/random_dir/Plugins", '
    ...
    def test_truncate(self):
      lines = [
          (self._ROOT_PID, 'execve("../out/unittests", '
           '["../out/unittests"...], [/* 44 vars */])         = 0'),
          (self._ROOT_PID, 'truncate("file.exe", 0) = 0'),
      ]
      files = [
          {
              'mode': trace_inputs.Results.File.READ,
