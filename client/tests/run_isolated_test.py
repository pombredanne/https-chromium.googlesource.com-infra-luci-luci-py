...
class RunIsolatedTest(RunIsolatedTestBase):
  ...
  def test_main(self):
    ...
    cmd = self.DISABLE_CIPD_FOR_TESTS + [
        '--no-log', '--isolated', isolated_hash, '--cache',
        os.path.join(self.tempdir, 'isolated_cache'), '--named-cache-root',
        os.path.join(self.tempdir,
                     'named_cache'), '--isolate-server', 'https://localhost',
        '--root-dir', self.tempdir, '--', 'foo.exe', 'cmd with space'
    ]
  ...
  def test_main_naked(self):
    ...
    cmd = self.DISABLE_CIPD_FOR_TESTS + [
        '--no-log', '--isolated', isolated_hash, '--cache',
        os.path.join(self.tempdir, 'isolated_cache'), '--isolate-server',
        'https://localhost', '--named-cache-root',
  ...
  def test_modified_cwd(self):
    _ = self._run_tha_test(
        command=['../out/some.exe', 'arg'], relative_cwd='some')
    self.assertEqual(
        [
          (
