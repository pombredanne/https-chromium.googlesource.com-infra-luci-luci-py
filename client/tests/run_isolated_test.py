...
class RunIsolatedTestCase(RunIsolatedTestRun):
  ...
  def test_bad_cipd_package(self):
    def emulate_bad_cipd(_run_dir, cas_dir, _nsjail_dir):
      raise run_isolated.InvalidCipdPackageException("foo_package",
                                                     "not/found/foo",
                                                     "deadbeef")
    ...
    self.assertEqual(1, ret)
    self.assertEqual('foo_package', result['missing_cipd']['package_name'])
    self.assertEqual('not/found/foo', result['missing_cipd']['path'])
    self.assertEqual('deadbeef', result['missing_cipd']['version'])
