...
def parse_args(use_isolate_server, use_swarming):
  ...
  if use_swarming:
    task_name = (u'%s-%s-%s' % (getpass.getuser(), datetime.datetime.now(
    ).strftime('%Y-%m-%d-%H-%M-%S'), _hello_world)).encode('utf-8')
...
def run(cmd, verbose):
  ...
  print(u'Running: %s%s%s' % (colorama.Fore.GREEN, u' '.join(cmd),
                              colorama.Fore.RESET))
...
def archive(tempdir, isolate_server, verbose, which):
  ...
  run([
      ...
      'archive',
      '--isolate',
      os.path.join(u'payload', which + u'.isolate').encode('utf-8'),
      '--isolated',
