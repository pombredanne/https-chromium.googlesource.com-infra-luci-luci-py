...
def get_all_by_tags(server, tags):
  ...
  # buildnumber:516399
  tasks = query_swarming(
      'tasks/list?' + '&'.join('tags=' + t for t in tags))['items']
...
def reproduce(server_old, server_new, task_id):
  ...
  cmd = [
    ...
    '--hard-timeout', prop['hard_timeout'],
    '--io-timeout', prop['io_timeout'],
  ]
  if request.get('service_account'):
    cmd.extend(('--service-account', p))
  for d in prop['dimensions']:
    cmd.extend(('-d', d['key'], d['value']))
  for k, v in prop.get('env', []):
    cmd.extend(('--env', d['key'], d['value']))
  for k, v in prop.get('env_prefix', []):
