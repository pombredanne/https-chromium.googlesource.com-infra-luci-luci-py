...
def generate_command(task, duration):
  """Generats a command that sleep and prints the original command."""
  original = get_swarming_args_from_task(task)
  return [
    'python', '-c',
    'import os,time,sys;' +
    ('print \'Visit %s for more information.\\n\';' % URL) +
    'print \'Files are mapped into: %s\' % os.getcwd();' +
    ('print \'Original command: %s\';' % ' '.join(original)) +
    ('print \'Bot leased for %d seconds\';' % duration) +
    'sys.stdout.flush();'
...
def trigger(swarming, taskid, task, duration, reuse_bot):
  ...
  cmd = [
    ...
    '-S', swarming,
    '--hard-timeout', str(duration), '--io-timeout', str(duration),
  ...
  cmd.append('--')
  cmd.extend(generate_command(task, duration))
  try:
...
def main():
  ...
  parser.add_argument(
      '-l', '--lease', type=int, default=60, metavar='SECS',
