...
class HttpError(NetError):
  ...
  Contains the response in full (as HttResponse object) and the original
  exception raised by the underlying network library (if any). The type of
  the original exception may depend on the network engine and should not be
  relied upon.
  """
  ...
  def description(self, verbose=False):
    ...
    Maybe called multiple times, results of the first call are cached.
    "Destroys" the response by reading from it.
    """
    ...
    ct = self.response.get_header('Content-Type')
...
class HttpResponse(object):
  ...
  def content_type(self):
    """Response content type of None if not presented by the server."""
