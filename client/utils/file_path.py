...
def set_read_only(path_or_entry, read_only):
  """Sets or resets the write bit on a file or directory.

  Zaps out access to 'group' and 'others'.
  """
  if isinstance(path_or_entry, six.binary_type) or isinstance(
      path_or_entry, six.text_type):
    orig_mode = fs.lstat(path_or_entry).st_mode
    path = path_or_entry
  else:
    orig_mode = path_or_entry.stat().st_mode
    path = path_or_entry.path
  mode = orig_mode
...
def set_read_only_swallow(path_or_entry, read_only):
  """Returns if an OSError exception occurred."""
  try:
    set_read_only(path_or_entry, read_only)
  except OSError as e:
    return e
  return None
...
def make_tree_deleteable_win(root):
  ...
  while dirs:
    for entry in os.scandir(dirs.popleft()):
      if entry.is_file():
        e = set_read_only_swallow(entry, False)
        if not err:
          err = e
