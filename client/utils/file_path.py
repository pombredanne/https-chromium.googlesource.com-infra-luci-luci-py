...
def atomic_replace(path, body):
  ...
  try:
    ...
    if sys.platform != 'win32':
      ...
      MoveFileEx(tmp_name, str(path), 0x1 | 0x8)
...
def make_tree_deleteable(root):
  ...
  On Win/Mac, use the os.scandir based implementations.
  On Linux, use the fs.walk based implementation.
  """
  if sys.platform == 'win32':
...
def rmtree(root):
  ...
  assert isinstance(root, str) or sys.getdefaultencoding() == 'utf-8', (
