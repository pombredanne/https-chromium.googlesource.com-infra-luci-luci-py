...
def find_executable(cmd, env=None):
  ...
  explicitly include it. This function also normalizes the slashes in cmd[0]
  to be backslashes.
  ...
  Returns:
    A copy of |cmd| with its arg0 executable changed to the absolute path for the
  ...
  if sys.platform == 'win32':
    ...
    cmd = [cmd[0].replace(os.path.altsep, os.path.sep)] + cmd[1:]
