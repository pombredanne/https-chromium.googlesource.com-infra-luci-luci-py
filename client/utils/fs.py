...
if sys.platform == 'win32':
  ...
  class GENERIC_REPARSE_BUFFER(ctypes.Structure):
    _fields_ = (('DataBuffer', wintypes.UCHAR * _MAX_WIN_PATH_LENGTH),)

  class _REPARSE_BUFFER(Union):
    _fields_ = (
      ('SymbolicLinkReparseBuffer', _SYMBOLIC_LINK_REPARSE_BUFFER),
      ('MountPointReparseBuffer', _MOUNT_POINT_REPARSE_BUFFER),
      ('GenericReparseBuffer', _GENERIC_REPARSE_BUFFER),
    )
  ...
  def readlink(path):
    ...
    try:
      ...
      ret = DeviceIoControl(
          ...
          ctypes.sizeof(buf), ctypes.byref(returned),
          None)
      if not ret:
        # pylint: disable=undefined-variable
        err = ctypes.GetLastError()
      ...
      if buf.ReparseTag == IO_REPARSE_TAG_SYMLINK:
        off = buf.SymbolicLinkReparseBuffer.PrintNameOffset
        l = buf.SymbolicLinkReparseBuffer.PrintNameLength
        return buf.SymbolicLinkReparseBuffer.PrintName[off:l]
      if buf.ReparseTag == IO_REPARSE_TAG_MOUNT_POINT:
        off = buf.MountPointReparseBuffer.PrintNameOffset
        l = buf.MountPointReparseBuffer.PrintNameLength
        return buf.MountPointReparseBuffer.PrintName[off:l]
