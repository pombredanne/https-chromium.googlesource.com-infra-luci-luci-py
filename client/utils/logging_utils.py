...
def set_user_level_logging(logger=None):
  ...
  Some scripts (run_isolated.py) have all their output displayed to users in the swarming UI.
  As a result we may want to display some logging as more important than others. These are not
  errors so should not be logged to stderr.

  USER_LOGS are seen as more important than INFO logs but less important than error logs.
  ...
  user_logs_formatter = UTCFormatter(
      "%(filename)s:%(lineno)s %(asctime)s %(severity)s: %(message)s")
