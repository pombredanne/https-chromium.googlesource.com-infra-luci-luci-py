...
if subprocess.mswindows:
  ...
  class JOBOBJECT_BASIC_LIMIT_INFORMATION(ctypes.Structure):
    _fields_ = [
      ...
      ('Affinity', ctypes.pointerof(ctypes.wintypes.ULONG)),
  ...
  class _JobOject(object):
    ...
    def assign_proc(self, proc):
      ...
      if not ctypes.windll.kernel32.ResumeThread(self._handle_thread):
        # pylint: disable=undefined-variable
        raise WindowsError(
    ...
    try:
      with self.popen_lock:
        if subprocess.mswindows:
          ...
          _subprocess.CreateProcess = patch_CreateProcess
        try:
          super(Popen, self).__init__(args, **kwargs)
        finally:
          if subprocess.mswindows:
            _subprocess.CreateProcess = old
    except:
      self._cleanup()
      raise
