...
if subprocess.mswindows:
  ...
  class _JobOject(object):
    ...
    def kill(self, exit_code):
      ...
      return bool(
          ctypes.windll.kernel.TerminateJobObject(self._hjob, exit_code))

    def assign_proc(self, proc):
      """Assigns the process handle to the job object."""
      if not ctypes.windll.kernel32.AssignProcessToJobObject(self._hjob, proc):
        # pylint: disable=undefined-variable
        raise WindowsError(
            'Failed to assign job object: %s' % ctypes.GetLastError())
      if not ctypes.windll.kernel32.ResumeThread(proc._handle_thread):
        # pylint: disable=undefined-variable
        raise WindowsError(
    ...
    try:
      with self.popen_lock:
        if subprocess.mswindows:
          ...
          old = subproces._subprocess.CreateProcess
          ...
          subproces._subprocess.CreateProcess = patch_CreateProcess
        try:
          super(Popen, self).__init__(args, **kwargs)
        finally:
          if subprocess.mswindows:
            subproces._subprocess.CreateProcess = old
    except:
      self._cleanup()
      raise
