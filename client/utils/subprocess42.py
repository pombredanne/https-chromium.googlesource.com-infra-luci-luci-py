...
if subprocess.mswindows:
  ...
  # http://msdn.microsoft.com/en-us/library/windows/desktop/ms684342.aspx
  ...
  class OVERLAPPED(ctypes.Structure):
    _fields_ = [
        ("Internal", wintypes.POINTER(wintypes.ULONG)),
        ("InternalHigh", wintypes.POINTER(wintypes.ULONG)),
        ("u", _U),
  ...
  ctypes.windll.kernel32.CreateEventA.argtypes = (
      # No need to support DACL here.
      wintypes.LPVOID,
      wintypes.BOOL,
      wintypes.BOOL,
      wintypes.LPCSTR)
  ctypes.windll.kernel32.CreateEventA.restype = wintypes.HANDLE

  windll.kernel32.PeekNamedPipe.argtypes = (wintypes.HANDLE, wintypes.LPVOID,
                                            wintypes.DWORD,
                                            wintypes.POINTER(wintypes.DWORD),
                                            wintypes.POINTER(wintypes.DWORD),
                                            wintypes.POINTER(wintypes.DWORD))
  windll.kernel32.PeekNamedPipe.restype = wintypes.BOOL

  windll.kernel32.ReadFile.argtypes = (
      wintypes.HANDLE, wintypes.LPVOID, wintypes.DWORD,
      wintypes.POINTER(wintypes.DWORD),
      wintypes.LPVOID)  # wintypes.POINTER(OVERLAPPED))
  windll.kernel32.ReadFile.restype = wintypes.BOOL

  windll.kernel32.WaitForMultipleObjects.argtypes = (wintypes.DWORD,
                                                     wintypes.POINTER(
                                                         wintypes.HANDLE),
                                                     wintypes.BOOL,
                                                     wintypes.DWORD)
  windll.kernel32.WaitForMultipleObjects.restype = wintypes.DWORD

  def ReadFile(handle, desired_bytes):
    """Calls kernel32.ReadFile()."""
    c_read = wintypes.DWORD()
    buff = wintypes.create_string_buffer(desired_bytes + 1)
    result = windll.kernel32.ReadFile(handle, buff, desired_bytes,
  ...
  def WaitForMultipleObjects(handles, timeout_ms):
    if timeout_ms is None:
      timeout_ms = 0xFFFFFFFF
    arrtype = wintypes.HANDLE * len(handles)
    return windll.kernel32.WaitForMultipleObjects(
        len(handles), arrtype(*handles), False, int(timeout_ms))

  def create_overlapped_pipe():
    """Creates a pair of readonly/writeonly pipes to be used in asynchronous
    I/O.

    Similar to CreatePipe() except that the pipes support asynchronous I/O.

    Calls random.random() internally, which could affect programs that depends
    on deterministic random number generation.
    """
    # Use random.random() first for faster default path. If the pipe already
    # exists, fall back to os.urandom().
    space = len(_RANDOM_LETTERS)
    unique = ''.join(
        _RANDOM_LETTERS[random.randrange(space)] for _ in xrange(8))
    for _ in xrange(3):
      pipename = r'\\\\.\\Pipe\\PythonPipe_%s' % unique
      # It's fine to not set a DACL since the pipe only accept a single
      # connection. Once it accepted its first connection, it must not accept
      # any other.
      read = ctypes.windll.kernel32.CreateNamedPipeA(
          pipename, PIPE_ACCESS_INBOUND | FILE_FLAG_OVERLAPPED,
          PIPE_TYPE_BYTE | PIPE_WAIT, 1, 0, 0, 120 * 1000, None)
      if read:
        break
      # The pipe existed already, use a more random source of randomness.
      for i in os.urandom(8):
        # TODO(maruel): Lame. Reduces randomness.
        unique += _RANDOM_LETTERS[i % space]
      unique += ''.join(_RANDOM_LETTERS[os.urandom(space)] for _ in xrange(8))
    else:
      raise OSError('Impossible to create a pipe')
    write = ctypes.windll.kernel32.CreateFileA(pipename, GENERIC_WRITE, 0, None,
                                               OPEN_EXISTING,
                                               FILE_FLAG_OVERLAPPED, None)
    if not write:
      raise OSError('Failed to open the pipe just created')
    return read, write
  ...
  def recv_multi_impl(conns, maxsize, timeout):
    ...
    if timeout:
      start = time.time()
    #logging.debug('Handles1: %s', [conn.fileno() for conn in conns])
    handles = [
        (i, msvcrt.get_osfhandle(c.fileno())) for i, c in enumerate(conns)
    ]
    events = [CreateEvent(False, None) for _ in handles]
    overlapped = [OVERLAPPED(e) for e in events]
    while handles:
      if timeout:
        remaining = max(0, (timeout - (time.time() - start)) * 1000)
      else:
        remaining = timeout
      start = time.time()
      result = WaitForMultipleObjects(events, remaining)
      logging.debug('WaitForMultipleObjects(%s, %s) = %s in %1.3f',
                    len(handles), remaining, result,
                    time.time() - start)
      if 0 <= result < len(conns):
        #logging.debug('A')
        try:
          avail = min(PeekNamedPipe(handles[result]), maxsize)
          #logging.debug('B %s', avail)
          if avail:
            data = ReadFile(handles[result], avail)
            #logging.debug('C %s', data)
            return result, data
          #return result, ''
          continue
        except OSError as e:
        ...
        logging.debug('WAIT_TIMEOUT')
        return None, None
      elif result == WAIT_FAILED:
        logging.debug('WAIT_FAILED')
        raise OSError(result)
      else:
        logging.error('No idea why WaitForMultipleObjects failed')
        raise OSError(result)
...
class Popen(subprocess.Popen):
  ...
  def __init__(self, *args, **kwargs):
    ...
    # Named pipes must be used with asynchronous reads.
    # subprocess.Popen._get_handles() creates anonymous pipes which don't
    # support asynchronous I/O.
    # The problem is how to push the tuple to _get_handles.

    # Also, we're screwed if the user passes a file object.
    if kwargs.get('stdout') == subprocess.PIPE:
      kwargs['stdout'] = create_overlapped_pipe()
    if kwargs.get('stderr') == subprocess.PIPE:
      kwargs['stderr'] = create_overlapped_pipe()
    super(Popen, self).__init__(*args, **kwargs)
