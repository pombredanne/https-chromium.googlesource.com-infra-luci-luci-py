...
def main():
  ...
  if sys.platform == 'win32':
    ...
    elapsed = time.time() - start
    print('fs.rmtree_powershell() took %s sec' % elapsed)

    start = time.time()
    fs.rmtree_rmdir(dir3)
    elapsed = time.time() - start
    print('fs.rmtree_rmdir() took %s sec' % elapsed)

    start = time.time()
    fs.rmtree_del_rmdir(dir4)
    elapsed = time.time() - start
    print('fs.rmtree_del_rmdir() took %s sec' % elapsed)
  else:
    dir2 = os.path.join(THIS_DIR, 'blink_web_tests_del2')
    ...
    elapsed = time.time() - start
    print('fs.rmtree_native_posix() took %s sec' % elapsed)
